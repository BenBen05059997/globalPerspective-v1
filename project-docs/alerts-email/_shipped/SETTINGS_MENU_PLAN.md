...
**Date:** 2026-06-10 · **Status:** ✅ SHIPPED — `NotificationsPanel` is live in `Account.jsx`, wired to `usePreferences` (`get_prefs`/`set_prefs` on `newsRecommend`); later plans (`FOLLOW_SURFACING_PLAN.md`) depend on it as live. [Corrected 2026-09-08 doc-staleness sweep — prior "pending a signed-in browser click-through before commit/push" was stale.] Function URL: `https://snkxw6dllquntzyyur2cb4eu6u0cnzqr.lambda-url.ap-northeast-1.on.aws/` (wired into `docs/config.js`).
