...
> ⚠️ **Prod-deploy gap (verified 2026-09-08 doc-staleness sweep):** the Stage 1 code (`RISK_CAP=50`, risk weight `2.0→1.0`) is on `main`/in source, but the **deployed `newsBreakingAlert` Lambda still runs the OLD uncapped, country-risk-dominant scorer** (byte-diffed against `CodeSha256`; the 07-07 LastModified touched config only, not code). So real alerts fire today with the pre-fix logic. Deploy is user-gated (below) and has not happened. Routed to the backlog.
