...
export function regionToCountryCode(regionName) {
    ...
    // Handle possessive forms (e.g., "China's" -> "China")
    if (normalized.endsWith("'s")) {
        const withoutPossessive = normalized.slice(0, -2);
        return regionToCountryCode(withoutPossessive);
    }
 ...
 * @param {string} code - Potential country code
 * @returns {boolean}
 */
export function isValidCountryCode(code) {
    if (!code || typeof code !== 'string') return false;
    const upperCode = code.toUpperCase();
    return Object.values(COUNTRY_NAME_TO_CODE).includes(upperCode);
}
...
export function countryCodeToName(code) {
    ...
    for (const [name, countryCode] of Object.entries(COUNTRY_NAME_TO_CODE)) {
        if (countryCode === upperCode) {
            // Return the first match (usually the official name)
            return name;
        }
    }
