...
2. ~~**⚠️ Breaking-alert scorer runs STALE logic in prod.**~~ **✅ DEPLOYED 2026-09-08** (`update-function-code` on `newsBreakingAlert`; live bytes re-diffed = repo, `CodeSha256` `fH11Usau…`). Prod now runs the fixed scorer (`RISK_CAP=50`, risk weight `2.0→1.0` + `axisForCategory`). **Note — the deploy also shipped the repo's unshipped AUTO-SEND path** (operator chose it): with `DRY_RUN=false` + `RESEND_API_KEY` set, each detection now auto-emails **only** the operator (`ALERT_EMAIL_TO=benlai310@gmail.com`, "audience of one" self-eyeball) — **not subscribers.** Subscribers still receive breaking alerts only via the separate `newsEmailSender` `mode:'breaking'` path (`TriggerBreakingEmailSend`, every 15 min), which broadcasts **human-confirmed** alerts to `breakingOptIn` recipients. Going subscriber-auto is deliberately deferred until precision is eyeballed on the new scorer. See `BREAKING_ALERT_V2_BUILD_PLAN.md` Stage 1.
3. ~~**🧨 Landmine — `newsImpactAudit/src/index.js:30`** defaulted to the retired `deepseek-chat` alias.~~ **✅ FIXED in source 2026-09-08** — default → `deepseek-v4-flash` (verified current per DeepSeek API), **and** added `thinking:{type:'disabled'}` to the repo source. ⚠️ **Correction (deployed-vs-repo diff):** the DEPLOYED function already had this guard (added 2026-07-26, deployed-only, never in repo) — **no live bug**; the fix reconciles repo→deployed so a future deploy can't silently remove it. **No deploy warranted** (live env already correct; deploying repo would be a no-op at best and could regress deployed-only drift). **Related — ✅ SWEPT 2026-09-08:** the 7 other Lambda sources that defaulted to the dead `grok-4-1-fast-non-reasoning` are each set to their live deployed model (per `aws get-function-configuration`): `NewsProjectInvokeAgentLambda`/`newsInvokeGemini`/`newsPairIntelligence`/`newsPostDevTo` → `deepseek-v4-flash`; `newsCountryIntelligence`/`newsSystemsAnalysis` → `deepseek-v4-pro`; `newsThreadAnalysis` → `gemini-2.5-flash`. No `grok-4-1` default remains in the fleet (all source-only — deploy separately).
