...
4. Writes each note TWICE via `putDriftNote` (idempotent, country + thread): `…/DRIFT#{date}` (`DRIFT_TTL_DAYS=60`, powers the live band + ledger) **and `…/DRIFTLOG#{date}` (NO TTL, the permanent member-history archive — added 2026-07-07, `MEMBER_GATING_PLAN.md` P3)**. Never overwrites; the `alreadyNoted` GetItem guard means a note is written once, so the `emailedAt` stamp `newsEmailSender` adds (drift alerts) is never clobbered.
