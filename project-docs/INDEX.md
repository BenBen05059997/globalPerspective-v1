...
- **[SCORING_STANDARDS.md](architecture/SCORING_STANDARDS.md)** — Rubric+benchmark audit of every score the system assigns (formula vs LLM-judgment; rubric quality; benchmark exists?). Verdict: predictions = gold standard; severity tier = most relied-on, least benchmarked (zero ground truth); ties to the never-built P0 reference set. 2026-09-14.
...
- **[ONE_TRUTH_EXECUTION_PLAN.md](architecture/ONE_TRUTH_EXECUTION_PLAN.md)** — Executable plan for the one-source-of-truth first slice (Phases 0/1/3; Phase 2 significance-replacement DROPPED as new-infra-for-cosmetic-gain). Phases 0-1 + 3.1-3.3 EXECUTED 2026-09-15; operator labeling (3.4) + baseline (3.5) pending. 2026-09-15.
