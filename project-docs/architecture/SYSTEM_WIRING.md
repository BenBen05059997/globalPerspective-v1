...
> ⚠️ **PARTIALLY REFRESHED 2026-09-08 — read first.** This is a code-grounded snapshot originally from **2026-05-18** with known drift. **`ARCHITECTURE.md` (last verified 2026-08-01) is now the authoritative current inventory; where this doc disagrees, trust `ARCHITECTURE.md`.** Corrections applied inline 2026-09-08: billing is **Polar** (`newsPolarBilling`, LIVE 2026-06-22), not Paddle/`newsStripeWebhook` (removed 2026-06-01); the fleet is **~33 live Lambdas** (not 14); the LLM provider is **DeepSeek v4-flash/v4-pro** (migrated 2026-07-26, not `deepseek-chat`); routes `/cli` and `/upgrade/success` no longer exist. Sub-sections not re-verified this pass (frontend hooks/components counts, §6 caching) may still be stale.

**Last verified:** 2026-05-18 (billing/inventory/routes/provider corrected 2026-09-08), from on-disk source under `amplify/backend/function/*` and `global-perspectives-starter/frontend/src/*`.

This is a code-grounded companion to ARCHITECTURE.md.
...
## §1 Backend Lambda inventory (~33 live — 14 original core shown; see ARCHITECTURE.md for full list)

> The 14 rows below are the original 2026-05-18 core set. The fleet has since grown to ~33 live Lambdas — see `ARCHITECTURE.md` §Lambda Functions for the authoritative current list (adds `newsAnalyze`, `newsBreakingAlert`, `newsRecommend`, `newsEmailSender`, `newsWeeklyBrief`, `newsWeeklyMarkets`, `newsEconomicImpact`, `newsEconomicQuality`, `newsDriftCorrector`, `newsPredictionResolver`, `newsSourceAudit`, `newsSignals`/`newsImpactAudit`/`newsGdacsIngest`/`newsGdeltConflict`, `newsPolarBilling`, `newsModelGuard`, and the observability trio `newsClientErrors`/`newsFreshnessMonitor`/`newsErrorDigest`).
...
| 9 | `newsSensitiveData` | 1453 | API Gateway HTTP | Google certs (JWT), Mapbox, Loops (Paddle portal removed 2026-06-01) | Users (auto-create on first sign-in) |
| 10 | `newsSavedItems` | 354 | Lambda Function URL | Google certs (JWT) | `SAVED_ITEMS_TABLE` `{uid, savedKey}` no TTL |
| 11 | ~~`newsStripeWebhook` (Paddle)~~ **REMOVED 2026-06-01** → replaced by `newsPolarBilling` (Polar checkout + webhook, Function URL, LIVE 2026-06-22; verifies Polar signature → writes `tier` in Users) | — | — | Users `{uid}` |
...
| `USERS_DDB_TABLE` | `uid` | newsSensitiveData, newsPolarBilling | newsSensitiveData, newsRecommend |
...
Event-driven: `newsPolarBilling` (Polar checkout + webhook, Function URL), `newsSensitiveData`, `newsSavedItems`, `newsAnalyze`, `newsRecommend` (HTTP/Function URL).
...
Live in nav (corrected 2026-09-08 vs `App.jsx`): `/`, `/daily`, `/daily/:dateKey`, `/map`, `/economy`, `/analyze`, `/membership`, `/track-record`, `/weekly-brief`, `/weekly-markets`, `/breaking`, `/breaking/:id`, `/weekly`, `/weekly/thread/:threadId`, `/weekly/countries`, `/weekly/country/:countryName`, `/signin`, `/auth/callback`, `/account`, `/about`, `/contact`, `/privacy`, `/disclosures`, `/whitepaper`, `/spider-demo`. (`/cli` and `/upgrade/success` no longer exist.)
...
~~Routed but not in nav: `/weekly-map`, `/intelligence-map`, `/map-v2`, `/test/briefing-card`.~~ (Corrected 2026-09-08: none of these routes exist in the current `App.jsx`.)
...
2. **ARCHITECTURE.md** — system overview (**authoritative; last verified 2026-08-01**).
3. **BACKEND_GUIDE.md** — Lambda quick reference (also a 2026-05-18 snapshot; verify against ARCHITECTURE.md).
4. **SYSTEM_WIRING.md** (this file) — code-grounded wiring, 2026-05-18 snapshot with 2026-09-08 corrections; trust ARCHITECTURE.md where they differ.
...
- TIERS.md, PADDLE_SETUP.md (Paddle-era; tier checks removed 2026-04-11, Paddle stack removed 2026-06-01) — **both archived in `_legacy/`.** Polar billing is now **LIVE** (`newsPolarBilling` + `/membership`, shipped 2026-06-22); [`POLAR_BILLING_PLAN.md`](../billing/_active/POLAR_BILLING_PLAN.md) documents the shipped system, not a future plan.
...
|---|---|---|
| ~~xAI Grok~~ → **DeepSeek v4-flash / v4-pro** (`api.deepseek.com`; legacy `GROK_*`/`XAI_API_KEY` env names, see §0) | 7+ Lambdas (see §1) | — |
...
| Polar | newsPolarBilling (checkout + webhook) | — |
