...
Changes: `features/map/api/worldData.js`, `features/map/hooks/useWorld.js` (useWorld/useSituationDetail, 5-min visible-tab poll, asOf/stale); **new** `features/map/components/SituationMap.jsx` (dark D3 map, hue=axis/radius=tier/ring=escalating) + `features/map/SituationHome.jsx`+css (freshness grey-out, lede, ranked list, detail panel, `?focus=`); routed at `/map` (old map → `/map-legacy`). Did NOT wire into WorldMapV2 (built a clean new map instead — WorldMapV2 retired to /map-legacy, deleted in S6)
...
Changes: `src/features/map/assets/countries-110m.json` (pinned world-atlas 2.0.2, imported — no CDN fetch); `?focus=<id>` URL state (replace mode). Scrubber `?t=` deferred to S5.
...
Changes: `SituationHome.jsx`, `SituationMap3D.jsx`, `SituationHome.css` (rewrites), `SituationMap.jsx` (palette), new `features/map/lib/situationLabels.js`; design target HTML added to repo.
...
Operator review of the deployed port found pins unclickable; a Sonnet audit found 12 more fixes; a patch-design round returned 6 boards (in-repo `MAP_HOME_DESIGN_TARGET_PATCH.html`; decisions in `MAP_UI_FIX_QUEUE.md`). Shipped across three deploys to unlisted /map: **slice 1+2a** (`41a3e3f`) — click fix (pickingRadius + static escalating halo; 4/4 headless clicks), manual tour (autoplay removed, tour bar in the card, never writes URL), mobile pass (~60vh, Key chip→legend sheet, peek row, 44px targets), since-you-last-looked (new-only via opened_at + localStorage); **slice 2b** (`aec70d6`) — spread arcs + affected-country fill as a selection state (`features/map/lib/countryGeo.js` iso3→geometry join); **slice 3** (`aec70d6`) — flat/globe **toggle** (default flat, "Globe" button persists per visitor; `_GlobeView`/`_GlobeController`, callout via `_GlobeViewport` w/ far-side cull, arcs = great circles). Plus audit fixes (lede error/expired states, WCAG `--sh-dim`, cadence-from-`next_expected_at`, stable fly-to). **Frontend map programme fully shipped.** Verify green (235 tests), browser-verified on live data, 0 console errors.
...
### S5.5 · T3 — Feed quality + tracker correctness (backend "slice 5" bundle) — ✅ DONE + DEPLOYED
Status: ✅ **Option A DEPLOYED to prod 2026-09-10** (commit `ffd80cc`). Three Lambdas patch-zip deployed (all clean baselines — live bytes == repo-before-edits, no node_modules, no deployed-only defaults): `newsSituationTracker` (Sha `8r/8rbocp0…`), `newsSituationIngest`, `newsGdacsIngest`. Live-verified across a full ingest→tracker loop:
- **Corroboration cap WORKS (the headline win):** open-tier distribution went **1 high / 35 elevated / 2 moderate / 15 low → 1 high / 14 elevated / 38 moderate / 2 low** — the single-outlet mass that was inflating "elevated" now correctly sits at "moderate". GDACS untouched.
- **coverage_ratio + tier_changed_at emitted** on active news rows (sampled `news#conflict#IRN#iran`: `coverage_ratio 1.17`, clean evidence, `tier_changed_at` present) — fixes the live "new"-mislabel bug (frontend already reads coverage_ratio).
- **Source-aware cooling:** no new `gdacs_*`/"Event no longer current in GDACS" leak onto news rows (10 residual leak-text rows are pre-deploy cooling/closed rows whose old text is frozen by the cooling early-return; they age out in the 8h keep window — cosmetic, self-healing).
- **Reopen:** tracker sweep after the ingest re-key emitted `reopened: 4`; those rows carry a fresh `opened_at`. Escalation `raised` still fires under hysteresis.
- **⚠️ Identity normalization is PARTIAL — honest finding.** `normalizeEntity` correctly folds title/org/article variants (President Trump / Trump administration / the Trump Administration → `trump`), but the DOMINANT live fragmentation this day was the two vectors explicitly deferred: **given-name splits (`donald-trump` vs `trump`)** and **axis flips (`conflict#USA` vs `political#USA`)** for the same event. So single-outlet share barely moved (50/80) — the **corroboration cap, not the identity fix, is what actually quiets the map today**. A second identity pass (safe given-name/surname aliasing via a high-frequency-actor map — the editorial-fact-layer pattern — and/or cross-axis consolidation) is now the highest-value churn-reduction follow-up. Recorded as a new sub-item below.
Env knobs live for tuning without redeploy: `CORROBORATION_CAP` (on), `RAISE_COOLDOWN_MIN` (180). Rollback = `CORROBORATION_CAP=false` (env) or redeploy the prior zip.

**S5.5·T3b (identity pass 2 — given-name aliasing) — ✅ DONE + DEPLOYED 2026-09-10** (commit after `5fa77c2`; `newsSituationIngest` patch-zip, Sha `hqnV8oEit…`). Added a curated `ACTOR_ALIASES` map applied in `normalizeEntity` (editorial-fact-layer pattern): full-name/given-name variants of ~20 recurring world figures fold onto one unambiguous-surname key ("donald trump" & "President Trump" & "Trump administration" → `trump`); pure name-variant normalization (no office claim → no leader-accuracy risk), unambiguous surnames only (full name kept where a surname is shared/short, e.g. `xi jinping`), missing key = harmless no-op. Live-verified: **Trump-related storyIds collapsed 6 → 3** — the given-name splits (`donald-trump`/`trump`/`trump-advisers`) are gone; the 3 remaining are split purely by AXIS (see T3c). 9/9 unit checks green (incl. no-over-merge: North/South Korea stay distinct). Chose the alias map over surname-extraction precisely to avoid the North/South Korea → "korea" collision.

**S5.5·T3c (identity pass 3 — axis-flip consolidation) — ✅ CLOSED 2026-09-10: MEASURED, DO NOT BUILD (accepted residual).** Measure-first ran instead of building (Sonnet, read-only): all 44 corpus hours (2026-09-08→09-10) replayed through the REAL `clusterStories` per day + 637 production `stories/state` files cross-checked; every candidate pair hand-labeled. Full labeled table: `T3C_MEASUREMENT_PAIRS.md` (in this dir). Numbers:
- **Frequency:** 275 cross-axis candidate pairs / 2.25 days — but only **21 true same-event** (7.6%); 242 different-event, 12 partial (a sub-story bundled inside otherwise-distinct clusters — whole-cluster merge would drag in unrelated content).
- **Payoff:** modest — 8 of the 21 true merges would cross the ≥3-outlet corroboration line over 2.25 days (~3.5/day).
- **Separation — the kill:** title-Jaccard does NOT separate the classes. SAME range 0.056–0.429 vs DIFFERENT 0–0.235, heavy overlap; the zero-false-merge threshold (≥0.236) recovers only **7/21 (33%)** of true merges. The motivating false pair ($5k pledge vs Iran war, jFilt 0.138) scores IDENTICALLY to a genuine same-event pair (S. Korea Hormuz debate, 0.138) — no threshold can tell them apart. Entity-set Jaccard as a secondary signal also fails to separate.
- **Verdict:** with false-merge = worse-than-no-merge as the constraint, an automatic merge is unsafe at any useful recall; a human-review suggestion queue was considered and REJECTED (the operator's existing review queues — breaking, weekly-markets — already sit unrun; adding another is process debt, and the residual is ~3.5 tier-crossing misses/day that the corroboration cap already renders harmless to the tier display). **Axis-split fragmentation is an accepted, documented residual.** Re-open only if the map's churn/duplicate-pin cost visibly grows — the measurement script pattern in `T3C_MEASUREMENT_PAIRS.md`'s header makes a re-run cheap.
- Side-finding: 148 same-axis pre-pass-2 slug-drift pairs exist in `stories/state/` (old `donald-trump`-style keys next to new `trump` keys) — **inert**: the tracker reads only `stories/index.json` (current keys), the old state files are read by nothing user-facing and age out via the 45-day stories lifecycle. No action.

--- (build/verify record from before deploy, retained:) ---
Option A built 2026-09-10; 31/31 logic checks green (scratchpad harness), `node --check` clean on all 5 files, the two `situations-core.js` copies re-verified logic-identical (only the 2-line header differs, as designed).
Built (all 6 items):
1. **Story identity** (`newsSituationIngest/classifier-core.js`): new `normalizeEntity()` folds obvious lexical variants of one actor onto a single cluster key — leading article + honorific/title prefixes + trailing generic org words stripped ("President Trump" / "Trump administration" / "the Trump Administration" → `trump`). Conservative by design: NO fuzzy/similarity merge (merging distinct actors would hide situations). Residuals left for a later pass (documented): given-name variants ("Donald Trump" vs "Trump") and axis-flip fragmentation (same event as conflict vs political).
2. **Corroboration cap** (`situations-core.js buildStorySituation` + new pure `capTierByOutlets`): 1 outlet→moderate max, 2→elevated, ≥3→uncapped; severity still sets the floor within the cap. Proven in-harness that GDACS (buildSituation path) is untouched.
3. **Source-aware cooling** (`coolSituation`): news situations cool with "No longer in active coverage" and keep their evidence clean (no `gdacs_*` splice); pre-cool tier retained through the grey-out (no forced `low`); GDACS wording unchanged.
4. **Escalation hysteresis**: velocity≥1.5 counts only when prev outlets≥2 (kills the 1→2 = 2.0× trigger); spread ignored when prev was null; repeat `raised` rate-limited by `RAISE_COOLDOWN_MIN` (default 180).
5. **Suppression ≠ cooling / reopen**: a reappearing closed storyId now re-opens fresh (`change:'reopened'`, state `emerging`, `opened_at` reset) instead of resuming at `peak` with a stale age. (The one-pin-merge cooling side-effect is mitigated by reopen; a fuller "mark merged, don't cool" is a later refinement if it still bites.)
6. **Slice-4 fields**: `evidence.coverage_ratio` (= velocity — fixes the live "new"-mislabel bug: frontend already reads it) and `tier_changed_at` (stamped when tier≠prev.tier in both builders; added to `summarize()` → world bundle). Frontend CONSUMPTION of `tier_changed_at` ("escalated since your visit") is a separate FE build — the field is now emitted, ready.
Env knobs added (tracker `index.js`): `CORROBORATION_CAP` (default on), `RAISE_COOLDOWN_MIN` (default 180). Also `threadId` now preserved across story rebuilds (was hardcoded null — needed for S3·T2).
Files: `newsSituationTracker/src/{situations-core.js,index.js}`, `newsGdacsIngest/src/situations-core.js` (byte-identical logic), `newsSituationIngest/src/classifier-core.js`.
**Deploy plan (when operator says go):** zip-diff both Lambdas' live bytes first (preserve `node_modules` + any env-overridden defaults); deploy `newsSituationIngest` + `newsSituationTracker` (+ `newsGdacsIngest` shares the core file but its logic path is unaffected — deploy it too so the shared file stays synced on disk-of-record); expect a one-time re-key wave; watch tracker CloudWatch churn counts (opened/cooled/closed per sweep) fall and the closed-row share of `world/latest.json` drop over a few sweeps. Rollback = env `CORROBORATION_CAP=false` + redeploy prior zip.
...
Still todo (selector-consumes-stories = "Option A"): prompt receives top-N clusters from `stories/index.json`; `continues_topic` → `storyId`; `captureIngestion` records the corpus key instead of truncated input.

**⚠️ REFRAMED 2026-09-10 by the pins-to-pages design investigation (`scratchpad/pins-bridge-plan.md`; goal = "map pins open into a real page"):** DO NOT do Option A (the selector rewrite) as the way to give pins somewhere to go. Findings:
- **The map world and the editorial world are two independent clusterings that share no IDs** (situation `news#<storyId>` from `axis#iso3#entity` vs editorial `threadId` from title-Jaccard in `NewsProjectInvokeAgentLambda:980-1012`). A situation's `threadId` is always null.
- **The situation detail panel is ALREADY the page** (~90% populated: metrics, evidence, outlet-attributed headlines, history). "Pins go nowhere" was mostly ONE dead footer element (`SituationHome.jsx:290` `"No dedicated situation page yet"`). **Fixed 2026-09-10 (commit `[footer]`, source only, /map deploy gated):** the footer now renders only when a real destination exists (matched thread or GDACS report). = **Option C, the recommendation, largely DONE.**
- **Option A is the wrong tool:** highest blast radius (rewrites the content-pipeline spine that feeds ~13 Lambdas), and it wouldn't even solve the goal — the selector is quota-capped (~13 topics × 4/day) while ingest clusters far more, and GDACS situations are deliberately threadless. Deprioritized (its only remaining rationale is de-duplicating the two RSS pipelines — a separate efficiency win, not a pins-to-pages need).
- **Option B (opportunistic thread bridge)** — cross-pipeline fuzzy matching is banned (T3c: title-similarity can't separate same-event pairs even within one pipeline; a wrong link is misinformation). **RESOLVED 2026-09-10 by the operator's one-source-of-truth discussion → `EVENT_REGISTRY_PLAN.md` (this dir): the map's `stories/` layer becomes the canonical event registry; the bridge is DETERMINISTIC via exact article-URL overlap** (both pipelines keep URLs from the same feeds — a shared URL proves same event, no fuzziness). Plan phases: 0 measure URL-overlap rate (read-only) → 1 additive bridge (topics gain storyId, tracker stamps threadId, zero frontend change needed) → 2 single-ingest spine change (LATER, shadow-week-gated) → 3 riders. Direction approved; Phase 0 not yet run.
- **Isolated safe piece** (capture→corpus): confirmed genuinely isolated (`captureIngestion` fire-and-forget) but LOW value (audit fidelity only) and touches the spine Lambda (zip-diff ceremony). Park until a spine deploy is happening anyway.
