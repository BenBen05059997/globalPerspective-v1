...
2. **Resolve** — legacy `newsPredictionResolver` (#20) still proposes hint-verdicts, but the pilot showed it punts `unclear` 86% of the time. The v1 loop is **agent-run** (Phase 2, `PREDICTION_METHODOLOGY_V1_PLAN.md` §4, runbook `predictions/V1_RESOLUTION_RUNBOOK.md`): scan due v1 triggers oldest-first, blind web-grounded verification (double-pass on every `fired`), write `finalVerdict` + `confirmedBy:'agent-verified'`; operator blind-spot-checks ~10%. `node predictions/review.js` remains the manual write path. No public auth surface. **First run DONE 2026-07-07** (weekly cadence): 16 of 21 due triggers scored, 5 deferred (window still open on run day). Same-day-deadline learning: verifiers need a "defer" option when the event window hasn't closed (not yet folded into the runbook doc).
