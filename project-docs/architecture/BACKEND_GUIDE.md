...
> ⚠️ **PARTIALLY REFRESHED 2026-09-08 — read first.** Originally verified **2026-05-18**; **`ARCHITECTURE.md` (2026-08-01) is authoritative where they differ.** Corrections applied 2026-09-08: billing is **Polar** (`newsPolarBilling`, LIVE 2026-06-22), not Paddle/`newsStripeWebhook` (removed 2026-06-01, along with the `portal_session` action); the fleet is **~33 live Lambdas** (this doc lists ~13 core — see ARCHITECTURE.md for the rest); the LLM provider is **DeepSeek v4-flash/v4-pro** (the `XAI_API_KEY`/`GROK_MODEL` env names are legacy holdovers pointing at DeepSeek — `grok-*` defaults are dead fallbacks). Billing is LIVE (the "when billing goes live" note below is obsolete).

**Last verified:** 2026-05-18 (billing/provider/inventory corrected 2026-09-08).

Quick-start guide to the backend system. For a complete architecture overview, see `ARCHITECTURE.md` (authoritative).
...
Polar webhook                → newsPolarBilling → Users DDB
...
| ~~Situations~~ | — | — | **Created then DROPPED 2026-09-08.** Situation state now lives in **S3** (`situations/state/*`, sole writer `newsSituationTracker`); openers append observations to `situations/inbox/*`. See `DATA_STRATEGY.md` + `ARCHITECTURE.md` §S3 World Store. |
...
**Billing (LIVE since 2026-06-22, Polar):** content stays free/public — membership buys analysis *compute* (`/analyze` on our key, member-gated via `newsAnalyze`) + living-analysis depth, enforced by depth-capping, not a hard paywall. See [`POLAR_BILLING_PLAN.md`](../billing/_active/POLAR_BILLING_PLAN.md) + `MEMBER_GATING_PLAN.md`. (The old Paddle re-gating plan is obsolete.)

---

## `newsSensitiveData` — Actions (~33 per ARCHITECTURE.md 2026-08-01; the ~16 below are the 2026-05-18 subset)
...
| ~~`user_profile`~~ | — | **REMOVED 2026-06-01** (membership now via `get_membership` on `newsPolarBilling`) |
| ~~`portal_session`~~ | — | **REMOVED 2026-06-01** (was Paddle portal) |
...
4. For each thread: Brave News + Web search → **Gemini 2.5 Flash** (this Lambda uses Gemini, not Grok/DeepSeek)
...
## Lambda 5: ~~`newsStripeWebhook`~~ → `newsPolarBilling` (Polar, LIVE 2026-06-22)

> ⚠️ `newsStripeWebhook` (Paddle) was **REMOVED 2026-06-01**. The live billing Lambda is `newsPolarBilling` (`amplify/backend/function/newsPolarBilling/src/index.js`): a Function URL handling Polar checkout-session creation + webhook — verifies the Polar signature → creates/updates/downgrades `tier` in the Users table, and grants credits on credit-pack orders. The Paddle event/HMAC details below are historical.

**Trigger (historical — Paddle, removed 2026-06-01):** Paddle webhook (separate API Gateway endpoint)
...
Posts a daily AI-written summary article to Dev.to using **DeepSeek v4-flash via OpenRouter** (`deepseek/deepseek-v4-flash:free`) — *not* xAI Grok (the `XAI_API_KEY`/`GROK_MODEL` names are legacy). Also generates the **Daily Intelligence Brief** (this Lambda is the sole producer of `/daily`, via a separate DeepSeek path) (stored as `DAILY_BRIEF#YYYY-MM-DD` in `SUMMARIZE_PREDICT_TABLE`, TTL 90 days). Brief generation is wrapped in try/catch — never blocks Dev.to publish. Deduplicates via `SOCIAL_POSTS_TABLE` (90-day TTL).
...
5. **Polar signature** — `newsPolarBilling` verifies the Polar webhook signature before processing (Paddle/`newsStripeWebhook` removed 2026-06-01)
...
|---------|------------|------------|
| DeepSeek (v4-flash/v4-pro; legacy `xAI`/`GROK_*` env names) | Tokens per call | 600 max tokens for topic AI, higher for thread/country |
...
| Polar | Transactions | Per-subscription/credit fee (Merchant of Record) |
