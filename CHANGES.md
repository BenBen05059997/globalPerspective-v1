...
- **Topic Limit Increase:** Changed frontend to request up to 20 topics from the backend instead of hardcoded 10. This allows the frontend to display all topics generated by the backend (currently configured for 17-18 topics).
- Updated `global-perspectives-starter/frontend/src/hooks/useGeminiTopics.js` to change `getGeminiTopics(10)` to `getGeminiTopics(20)` in both the initial load (line 43) and background polling (line 82).
