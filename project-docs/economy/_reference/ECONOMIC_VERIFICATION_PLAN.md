...
**Status:** Draft 2026-05-23. [Corrected 2026-09-08 doc-staleness sweep: the §13 "scripts do not yet exist" line is stale — `verify_ddb.js` / `verify_proxy.sh` / `calibration_report.js` / `atoms_economic.test.jsx` were added in the SAME commit (`69794de`) as this doc and now exist; the §0a "0% inline citations" and L1 DDB-integrity findings still need a live re-run.] To be executed via repeated `/loop` runs until every check is green, then re-run after every deploy that touches `newsEconomicImpact`, `newsEconomicQuality`, the three economic hooks, or any page that imports `SeverityBadge` / `MechanismCard` / `DisruptionRow` / `DisruptionPreview` / `QualityFlag`.
