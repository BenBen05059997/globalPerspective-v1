...
Surfaced the newsletter opt-in where readers actually are. New `SubscribeCard` component (`components/SubscribeCard.{jsx,css}`) placed on **Home** (under the masthead) and **`/weekly-brief`** (under the KPIs). It reuses the existing opt-in — `usePreferences` → `newsRecommend set_prefs` → `GlobalPerspectiveUserPrefs.digestOptIn` — so **no backend change**: signed-in users subscribe in one click; anonymous users sign in with Google and the subscribe intent is applied immediately after (`signInWithGoogle` → `save({digestOptIn:true})`); already-subscribed users see a "Manage" link to `/account?tab=notifications`. Renders nothing if the prefs endpoint is unconfigured. Theme-aware (uses `--ink`/`--paper`/`--border-color`), responsive (stacks on mobile). Verify green (0 errors, 192 tests); Home card browser-verified; `/weekly-brief` card sits in the same signals branch as the KPIs (local preview can't load the brief cross-origin — verified on prod). Files: `components/{SubscribeCard.jsx,SubscribeCard.css,Home.jsx,WeeklyBriefPage.jsx}`.
