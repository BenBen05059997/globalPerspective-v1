...
> ℹ️ **Mostly current (last content update 2026-07-03; reviewed 2026-09-08).** The route entries and removed-route notes are accurate. **Not yet documented here:** the newer routes `/breaking`, `/breaking/:id`, `/weekly-markets`, `/spider-demo` — see `ARCHITECTURE.md` §Frontend Routes / `App.jsx` for the full current list.
...
- **Key UI elements:** EditorialShell 3-col; **"Today's lede" band** (`LedeBand`, deterministic `composeTopicsLede` — no LLM) directly under the StatusStrip; left rail `TodayArchiveSidebar` + Coffee CTA; right rail `TopicNav`; center masthead + **`SubscribeCard`** (Weekly Brief email opt-in, directly under the masthead — reuses the `digestOptIn` opt-in via `usePreferences`; signed-in = 1-click, anon = Google sign-in then auto opt-in; shows a transient **"Thanks for subscribing!"** confirmation on a real persist, else "You're subscribed" + Manage. `kind='breaking'` variant lives on `/breaking`) + per-region topic cards (with per-topic economic-disruption badge) + AI button trio + collapsible Sources. Topics are the day's Gemini set (not archive threads), so they carry **no risk score** — the lede band, not a tier hierarchy, provides the "what matters most today" ordering.
...
## `/economy` · `features/economy/EconomyPage.jsx`
...
- **Key UI elements:** **four tabs** via `?tab=` (default `saved`) — **Profile** (avatar + identity + "Since" date + sign-out + delete-account), **Saved** (chip filter + grid of cards with heart toggle), **Notifications** (email opt-ins — Breaking / Weekly-digest toggles + cadence select + "Unsubscribe from all"; shows a transient **"✓ Subscribed / Unsubscribed from …"** confirmation on each change; copy now reads **"Email delivery is live"** since delivery went live 2026-07-04), **Analysis** (BYOK key view/change/remove + link to `/analyze`).
...
> ⛔ **REMOVED — route unregistered + `UpgradeSuccess.jsx` deleted; billing is Polar with no success-page redirect. Historical only (see the Removed-routes note at the top).**
