...
**Status:** ✅ DEPLOYED (live-verified 2026-09-08) — `newsSignals` Lambda Active since 2026-06-23; `GlobalPerspectiveApiKeys` ACTIVE but **0 keys minted** (the one real remaining gap). 45 unit-test claim accurate. This doc is the contract to hand a design partner.

> **⚠️ STORAGE MIGRATED 2026-09-09 (S8·T1).** The build no longer writes the `GlobalPerspectiveSignals` DynamoDB table (dropped) — it now writes a single **`signals/latest.json`** document in the world S3 bucket (`globalperspective-world-280362093938`), plus an immutable dated `signals/snapshots/YYYY-MM-DD.json` (Glacier IR after 30d, expire 365d). SERVE reads `signals/latest.json`, ETag-cached in module scope. Retention is now an explicit `event_time ≥ now−120d` filter at build time (the old 120d DDB TTL never actually expired anything, since the daily rebuild restamped it). Rationale: the table was **write-only** — ~23K writes/day, **0 reads over 14 days**; migrating conforms to `DATA_STRATEGY.md` ("S3 for the world, DynamoDB for the user"). `GlobalPerspectiveApiKeys` **stays** in DynamoDB (auth/rate-limit data). Pure store logic + tests: `src/signalStore.js` (59 tests total in `test-adapter.mjs`). The §5/§6/§10 create-table + TTL steps below are **historical** — the Signals table no longer exists. **`signals/` is deliberately excluded from the Cloudflare Worker `/data/*` whitelist** — this is the paid, key-gated product and stays behind the Function URL only.
