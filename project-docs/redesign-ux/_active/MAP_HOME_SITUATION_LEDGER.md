...
- **Option B (opportunistic thread bridge)** — set `situation.threadId` from a matching existing editorial thread — is the ONLY remaining pins-to-pages lever, but it's **cross-pipeline fuzzy matching of two independently-worded LLM outputs**, which the T3c measurement already showed title-Jaccard can't do reliably even WITHIN one pipeline. A wrong link (sends the reader to an unrelated analysis) is worse than no link. **So Option B must be MEASURED before building** (same discipline as T3c): does a high-precision situation→thread join exist (shared entities+iso3+recency at a confidence that yields ~zero false links)? If yes, build it gated at that confidence; if not, skip — the panel already suffices. 🔭 DECISION PENDING (operator: measure B, or call panel-as-page enough and move to S6).
