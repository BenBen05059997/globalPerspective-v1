...
- **Slice 2b** ✅ DONE + DEPLOYED (commits `e641d10` build, `aec70d6` deploy). Spread arcs + affected-country fill as a selection state; new `features/map/lib/countryGeo.js` iso3→geometry join. Live on /map.
...
- **Slice 4** (tracker `tier_changed_at`/`coverage_ratio`), **Slice 5** (feed-quality gate) — backend, paused for a thresholds discussion (operator's call 2026-09-10). **2026-09-10 audit re-based the scope** (`project-docs/redesign-ux/_active/SITUATION_BACKEND_AUDIT.md`): storyId fragmentation is the root cause (one event live-observed split across 6 storyIds → churn, 65/118 closed rows, diluted corroboration, fake escalation); the corroboration cap is item 2 of a 7-item causal work list now recorded in ledger **S5.5·T3**. Fleet-wide follow-ups (retirements, dead env vars, ops decisions) recorded as ledger **Stage S9**. Nothing implemented — awaiting operator scope decision.
