...
5. **Naming traps (live-verified 2026-09-10):** deploy names carry `-dev` suffixes that are actually prod (`newsInvokeGemini-dev`, `newsSensitiveData-dev`, `NewsProjectInvokeAgentLambda-dev`); the LinkedIn poster's repo dir `newsPostLinkedIn` deploys as `newsPostLinkedin` (lowercase "in"); the EventBridge Scheduler entry for country intelligence is named `countryIntelliegence` (typo, real, do not "fix" without updating the live schedule); `newsStripeWebhook`'s dir is an **undeployed** Paddle handler kept for reference only (see #12).
