     ...
     function isScheduledEvent(event) {
      ...
      function augmentTopic(rawTopic, idx) {
        ...
        return {
          ...
          sources: Array.isArray(rawTopic?.sources)
            ? rawTopic.sources.map((s) => ({
                url: String(s?.url || '').trim(),
                title: String(s?.title || '').trim(),
                publish_date: String(s?.publish_date || '').trim(),
                publisher: String(s?.publisher || '').trim(),
                verified: Boolean(s?.verified),
              }))
            .filter(s => s.url && s.publish_date)
            : [],
        };
      }
     ...
     exports.handler = async (event) => {
       ...
       try {
         ...
         const prompt = [
           ...
           '- sources: array of source objects with url, title, and publish_date',
           ...
           '2. Browse AT LEAST 3-4 major news websites: BBC, CNN, Reuters, Associated Press, Al Jazeera, local news sources.',
           '3. Search across multiple sources - DO NOT rely on single source information.',
           '4. For each story, find at least 2-3 different news outlets reporting the same event.',
           '5. Cross-verify all facts, dates, and details across sources.',
           '6. Each topic MUST include actual URLs that REALLY EXIST and are clickable.',
           '7. Each source MUST have recent publish_date (within 24-48 hours maximum).',
           '8. If you cannot find sources from 24-48 hours, expand to max 7 days old.',
           '9. URLs must be real working links from established news organizations.',
           '10. If you find any conflicting information, exclude that story entirely.',
           '11. DO NOT make up URLs - you must provide verifiable working links.',
           '12. Each topic should have multiple sources confirming the same story.',
           '',
           ...
           'Each topic must include multiple verified sources, like:',
           '"sources": [',
           '  {',
           '    "url": "https://www.reuters.com/world/asia-pacific/story-url-here",',
           '    "title": "Actual Headline from Reuters",',
           '    "publish_date": "2025-11-04"',
           '    "publisher": "Reuters"',
           '    "verified": true',
           '  },',
           '  {',
           '    "url": "https://www.bbc.com/news/story-url-here",',
           ...
           '- Does each URL actually work and show the news story?',
           '- Are there at least 2-3 different sources confirming the same story?',
           '- Are all publish dates within the past 7 days (preferably 24-48 hours)?',
           '- Do multiple reputable news outlets report similar facts?',
           '- If ANY answer is NO, exclude that topic completely.',
           '',
           'WARNING: DO NOT generate fake URLs. If you cannot find real, working, recent sources, return ZERO topics.'
         ].join('\n');
