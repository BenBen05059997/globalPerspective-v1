...
> ℹ️ **Reviewed 2026-09-08 — corrections applied inline.** The JWT / CORS / secrets guidance is current. Billing-security items updated: Paddle/`newsStripeWebhook` was removed 2026-06-01 → billing is **Polar** (`newsPolarBilling`, verifies the Polar webhook signature). Note the `GROK_*`/`XAI_API_KEY` env names are legacy and hold DeepSeek/Gemini values.
...
**Polar webhook security:** `newsPolarBilling` verifies the Polar webhook signature (Standard-Webhooks) before processing. (Paddle/`newsStripeWebhook` + `PADDLE_WEBHOOK_SECRET` were removed 2026-06-01.) Never skip signature verification.
...
- Keep all actual secrets (LLM API keys — DeepSeek/Gemini, held under the legacy `XAI_API_KEY`/`GROK_*` names; the Polar access token + webhook secret; Firebase credentials) only in Lambda environment variables — never in `config.js`
...
| newsSensitiveData | `MAPBOX_GEOCODING_KEY`, `FIREBASE_PROJECT_ID`, `USERS_DDB_TABLE` (Paddle `PADDLE_API_KEY` removed 2026-06-01) |
| newsSavedItems | `SAVED_ITEMS_TABLE`, `FIREBASE_PROJECT_ID` |
| newsPolarBilling (replaced newsStripeWebhook 2026-06-01) | `POLAR_ACCESS_TOKEN`, Polar webhook secret, `USERS_DDB_TABLE` |
