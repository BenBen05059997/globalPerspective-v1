...
### S5.5 · T3 — Feed quality + tracker correctness (backend "slice 5" bundle) — 🟢 SCOPE DECIDED, ready to build
Status: 🟢 **Operator decided 2026-09-10: Option A — the full bundle (items 1-6 below in one pass)**, with two softenings: (a) identity change is CONSERVATIVE — normalize entity slugs + pick the most-common entity across the cluster, NO similarity-merge machinery yet; (b) every new threshold behind an env knob so tuning is an env write, not a redeploy. Known one-time cost accepted: storyIds re-key once at cutover (a wave of closes/opens that self-heals within the 8h closed-keep window). **No code changed yet — build is the next step, deploy gated as always.**
