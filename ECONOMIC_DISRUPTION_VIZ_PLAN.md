...
## ✅ Sparkline — DONE 2026-05-26
- **Backend:** `markets_history` extended from FX-only to resolve any `symbol` across `COMMODITIES#/RATES#/EQUITIES#/CRYPTO#/FX#` HISTORY rows, returning `[{date, value}]`. Deployed + verified (SPX → 2 pts, BRENT → 1 pt, `topics` healthy).
- **Frontend:** `useMarketsHistory(symbol)` hook + `Sparkline` atom rendered in the instrument-pivot expand (only ≥2 points → graceful).
- **Data note:** equities/crypto/FX have history now; **commodities/rates** (Brent/gold/US10Y) only began accruing 2026-05-26, so their sparklines fill in over the next several days (render nothing until then — honest, no fake line).
