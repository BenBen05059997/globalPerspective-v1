...
> ✅ **Corrected 2026-09-08 (doc-usage audit):** despite the 'PROPOSED' header below, Phases A–D (incl. the per-axis drift email) are DEPLOYED and live in code — `utils/riskDimensions.js`, `significance.js` `axisForCategory`, `newsDriftCorrector` `currentDimensions`, `renderDriftEmail.js` Level A/B. Moved from `_proposed/` to `_shipped/`.
...
- **Phase D-email — the drift *email* renderer. ✅ SHIPPED + DEPLOYED 2026-07-08 (main `0ec494f`; both Lambdas live, bytes verified).** The last scalar-only surface now renders the per-axis view via a richest-available ladder (Level B current 4-axis standing → Level A per-axis change pills → scalar fallback). `newsDriftCorrector` embeds `currentDimensions`/`currentLead`/`currentRiskScore`/`currentRiskLevel` onto each new `DRIFT#`/`DRIFTLOG#` note (vector was already loaded); `renderDriftEmail.js` reads them off the note (no frontend-util import), computes the breadth flag inline. Not retroactive — Level B lights up for notes written from the next 07:20 UTC cron; pre-deploy notes render at Level A. Corrector live-invoke ran clean (idempotent, today's notes stay Level A as designed). Tests 28→38 green. Spec + build detail → §12.
