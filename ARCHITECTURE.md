...
| `corrections_feed` | None | `{limit?≤100}` | Site-wide corrections ledger (the living-analysis loop's aggregate view): recent grounded `DRIFT#` notes across ALL `COUNTRY#`/`THREAD#` rows, newest-first. DDB Scan filtered on `begins_with(SK,'DRIFT#')` (same pattern as `economic_impact_list`; 60d-TTL rows, client-cached 30min). Each note: `{scope, name, asOf, changeLevel?, changeScore?, triggerEvent?, whyChanged?, noSingleDriver}`. Deferred optimization: a single `CORRECTIONS_FEED` GetItem row (have `newsDriftCorrector` append). Powers the `/track-record` Accountability hub |
...
### 29. `newsEmailSender` — one sender for weekly brief + breaking alerts (LIVE 2026-07-04)
**Path:** `amplify/backend/function/newsEmailSender/src/index.js`
**Trigger:** EventBridge `TriggerWeeklyEmailSend` — `cron(0 14 ? * SUN *)` (Sundays 14:00 UTC, ~8h after the brief is generated) **ENABLED**; `TriggerBreakingEmailSend` — `rate(15 minutes)` **ENABLED**. Both invoke with `{mode}`. **No LLM.** Built 2026-07-03, **went live 2026-07-04** (domain verified); plan → `EMAIL_SENDER_PLAN.md`.
