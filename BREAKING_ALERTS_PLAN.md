...
4. **Email send (Resend) + opt-in UI.** ✅ **BUILT + DEPLOYED (dry-run) 2026-07-03** as `newsEmailSender` (Lambda #29 in `ARCHITECTURE.md`; shared `sendEmail.js`). The `breaking` mode scans `GlobalPerspectiveBreakingAlerts` for `status:'confirmed'` + `emailedAt` unset + `alertedAt`≤48h → emails `breakingOptIn` subscribers → stamps `emailedAt`. Trigger `TriggerBreakingEmailSend` `rate(15m)` created **DISABLED** (enable at go-live). Opt-in UI exists (Account → Notifications; `breakingOptIn`). **Remaining = operator only:** verify `globalperspective.net` in Resend → flip `EMAIL_FROM`+`EMAIL_SEND_DRY_RUN=false` → enable the breaking rule. Double-opt-in confirmation (`breakingVerified`) still deferred (auth verifies the email). See `EMAIL_SENDER_PLAN.md`, [[project-email-sender]].
