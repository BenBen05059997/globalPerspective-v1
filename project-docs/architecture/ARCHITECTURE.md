...
| Google Maps | WeeklyMap.jsx (embedded by CountryPage) | Interactive map rendering. **Note:** WorldMapV2 (`/map`) uses **d3 + topojson**, not Google Maps. (The legacy, unrouted `WorldMap.jsx` was removed 2026-09-24 — see below.) |
...
**Removed/never-wired:** a "Cut: orphans" cleanup deleted several components and routes, and the 2026-05-26 subscription deprecation removed billing UI. The following are **no longer routed and the component files no longer exist**: `/cli` (CLIPage), `/intelligence-map` (IntelligenceMap), `/test/briefing-card` (BriefingCardTest), `/pricing` (Pricing), `/weekly/pairs` + `/weekly/pair/:slug` (PairPage/PairListPage), `/upgrade/success` (UpgradeSuccess — billing). Also deleted in the billing cleanup: `TrialBanner.jsx`, `WeeklyLockedPreview.jsx`, `useUserProfile.js`. `WeeklyMap.jsx` still exists as a file but is **not routed** (`/map` uses WorldMapV2; there is no `/weekly-map` or `/map-v2` route). **`WorldMap.jsx` + `WorldMap.css`** (the legacy pre-V2 Google Maps view, zero importers) **and `MapSidePanel.jsx`, `BriefingCard.jsx`** (zero live importers) **were removed 2026-09-24** as dead code (`CLEANUP_AUDIT_2026-09-24.md` §4 Tier B1; build + test-suite verified, deploy deferred).

### Key Components

58 component files (in `src/components/` + 19 in `src/components/atoms/`), incl. `LedeBand.jsx` and the Analysis Studio pair `AnalysisStudio.jsx`/`ProviderModal.jsx` added 2026-06-10. (Count as of the 2026-09-24 dead-code cleanup — see `CLEANUP_AUDIT_2026-09-24.md` §4 Tier B1.) Key ones:
...
| `WorldMapV2.jsx` | The live map at `/map` — the **"Today's lede" band** (`LedeBand`) between the map title and the search box, then **three** stacked layer lenses: **Today's pulse** (24h fresh-news rings), **Connections** (bilateral arcs from `usePairAnalyses`), **Editorial** (top-5-by-signal numbered pins). Base country fills + two-tier signal markers are always drawn from `useCountrySignal` (a **client-side 30-day z-score** over `useWeeklyArchive`, bucketed H ≥ 1.5σ · E ≥ 0.5σ, forced quiet under 2 articles — coverage-volume anomaly, deliberately distinct from the backend `riskScore`/tier shown in the detail panel). A 4th **Economy** lens (disruption-exposure rings) was **removed 2026-07-29**: its `econByISO` lookup missed `nameToISO`'s lowercased keys, so it had never rendered a ring, and an always-empty exposure layer reads as "no exposure anywhere" ([[feedback_no_misinformation_fallback]]). Disruption data still reaches this page via the lede band and the detail panel's Economic Disruption section. Rendering is **imperative d3** — `drawMap()` wipes and re-appends the whole SVG on a 240ms-debounced effect, so live data reaches it through state-mirrored refs; geometry is `world-atlas@2/countries-110m` fetched from jsDelivr **at runtime**, not bundled |
...
29 hook files (in `src/hooks/`; count as of the 2026-09-24 dead-code cleanup — see `CLEANUP_AUDIT_2026-09-24.md` §4 Tier B1):
