...
## 2026-09-25 (Country page: remove the country-scoped Causal Web tab)

Operator direction: the web is news-based, and the country-related web "should be removed or merged to the analysis studio". The country page's **Causal Web** tab and its `SystemsGraph` component (+ CSS) are removed. The page's views are now Situation / Story Arcs / Coverage. The `systems_analysis` data and `useSystemsAnalysis` hook stay: they are the source of the planned story links and still feed `/spider-demo` until it retires. A country systems view becomes a planned Analysis Studio lens (checklist C12). New `countryPageTabs.test.jsx` renders the page from fixtures and clicks every tab; a live browser check was not possible because the paused news pipeline leaves anonymous visitors with no country coverage. 193/193. Docs: ARCHITECTURE.md (country page + path map), `STORY_WEB_RETHINK_PLAN.md` §5, `COUNTRY_INTEL_VS_STUDIO_2026-09-24.md` §7, ACTION_CHECKLIST C12. **Not deployed.** It ships with the next `./deploy.sh`.

## 2026-09-25 (Fix: story page blanked when opening "Root cause" on 11 stories)

Found during the story × spider design debate and confirmed live on globalperspective.net. `rootCauseChain` is stored in two shapes: a string on most stories, and an object `{proximate, medium_term, deeper_structural}` on 11 of them, including the newest (BRICS summit 09-23, US diesel 09-19, US–Iran Hormuz 09-12). `ThreadPage` rendered the value straight into a `<p>`, so opening "Root cause" on an object-shaped story threw React error #31 and blanked the whole page. `CopyBriefing` printed "[object Object]". The weekly-map `ThreadIntelligence` panel had the same crash path. New `shared/lib/rootCause.js` (`rootCauseSteps`, `rootCauseText`) normalises both shapes. The story page now shows the object as three labelled steps (Trigger / Enabling condition / Structural factor), each clamped with Read more, and the copy and weekly-map paths use the text form. Tests: `rootCause.test.js` (5) + `threadPageRootCause.test.jsx` (2; the object case fails on the old code with the exact production error). 192/192. **Not deployed.** It ships with the next `./deploy.sh`. Backend note (not fixed): `newsEconomicImpact` index.js:429 calls `.slice` on `rootCauseChain` and would throw on the object shape.
...
file in the repo for current-state claims that still described removed things as live: `WorldMapV2.jsx`/`WorldMap.jsx`/`MapSidePanel.jsx`/`BriefingCard.jsx`/`CountryGrouping.jsx`/`ArticleCard.jsx`/`LoadingStates.jsx`/`MiniMap.jsx`/`SectionNav.jsx`/`SideNav.jsx`/`ApiKeyGate.jsx`, hooks
`usePairAnalyses`/`usePrediction`/`useSummary`/`useBookmarks`/`useResearchBriefing`/`useTraceCause`,
`utils/geocoding.js`, `services/appsyncProxy.js`, root `src/` Amplify scaffold, and the `/contact`
"effectively orphaned" mischaracterization (it's footer-linked). Fixed: `CLAUDE.md`, `project-docs/INDEX.md`, `project-docs/architecture/{PAGES_GUIDE,SYSTEM_WIRING,OPTIMIZATION_REPORT,ops/DEPLOYMENT_NOTES}.md`, `project-docs/economy/ECONOMIC_DISRUPTION.md`, `project-docs/pairs/_proposed/PAIR_UI_PLAN.md` (banner), `project-docs/redesign-ux/_shipped/SITE_ORIENTATION_PLAN.md` (banner), `internal-docs/MAP_UPGRADE_FEATURES.md`, `global-perspectives-starter/frontend/FRONTEND_ARCHITECTURE.md` (banner — wholesale stale, predates several redesigns), `quality/dashboard.md` (added the missing pre-commit hook mention alongside pre-push). Dated audit/plan/ledger bodies (2026-09-08/09-11 snapshots, `_legacy/`, shipped-plan narration) were left untouched per the docs-as-code convention — they're historical record, not living reference. `ARCHITECTURE.md` itself was already thoroughly updated same-day and needed no further fixes beyond re-verification. Full file list + before/after in `CLEANUP_AUDIT_2026-09-24.md` §5. No code change, no build, no deploy.
...
## 2026-09-24 (Legacy map REMOVED — final cleanup of CLEANUP_AUDIT §4 D3, build+tests green, deploy deferred)

Phase 2 of `TASK_2026-09-24_pair_arcs_relocation.md`, executed via the cancelled-path (pure removal, no relocation — the Phase 1 port was already reverted from `main` and archived on `archive/pair-arcs-port`, previous entry). Removed the legacy `WorldMapV2.jsx` (1,362 ln) + `WorldMapV2.css` (728 ln), the `/map-legacy` route and its import in `App.jsx`, `usePairAnalyses.js` (44 ln, orphaned once `WorldMapV2` was gone), and the 3 tests that rendered it directly — `signalFilters.test.jsx`, `searchBar.test.jsx`, `layers.test.jsx` (628 ln combined) — all confirmed zero-importer outside the deletion set by grep immediately before deletion. Tidied now-stale `WorldMapV2` comment references in `tokens.js`, `utils/threadPath.js`, `utils/topicMatch.js`, and `test/macroValues.test.js` (kept — only a comment mention, not a render). `/map` (`SituationHome`/`SituationMap3D`) is now the **only** map in the app. `SpiderDemo`/`/spider-demo` untouched per operator instruction. The backend pair pipeline (`newsPairIntelligence`, the weekly `TriggerPairIntelligenceWeekly` cron, `pair_analyses_list` action in `newsSensitiveData`) is **untouched and still running** — it currently has no frontend consumer; disable-vs-leave-warm remains an open operator decision (`CLEANUP_AUDIT_2026-09-24.md` §4 D3). Verified: `npm run build` clean; `npx vitest run` 193/193 passing across 16 files (down from 235/19 — exactly the 3 deleted files' tests, nothing else regressed). Docs updated in this commit: `ARCHITECTURE.md` (map section/route table/component+hook tables and counts now describe `SituationHome`, not `WorldMapV2`), `CLEANUP_AUDIT_2026-09-24.md` §4 D3, `TASK_2026-09-24_pair_arcs_relocation.md` (Phase 2 checklist ticked), `INDEX.md` (new reference-doc row for the idea harvest + D3 status fix). **Deploy deferred** — no `dist/` copy to `docs/`, no `deploy.sh`, no push; this rides the next explicitly-gated deploy.

## 2026-09-24 (Pair-arcs port CANCELLED per operator "no build" — reverted from main, archived on branch; legacy-map ideas harvested)

Operator redirect mid-verification: the new map (/map = SituationHome) is the design; the legacy map's value is its *ideas*, not a code port. The Phase 1 Connections-layer port (previous entry) — which had reached render/toggle-verified in browser (click-picking still rough) — is **cancelled, not shipped**: its four source-file changes (SituationMap3D.jsx, SituationHome.jsx, countryGeo.js, situationLabels.js) reverted to pre-port state on main; the full working port preserved on branch **`archive/pair-arcs-port`** for any future revival. Instead, a Sonnet idea-harvest read all 1,362 lines of WorldMapV2 (+CSS) and wrote `project-docs/redesign-ux/_reference/LEGACY_MAP_IDEA_HARVEST_2026-09-24.md`: 10 legacy-map ideas with data-liveness + new-map-equivalent checks and verdicts (steal-someday: time-window control, situation search, label-density tiering; already-covered: urgency rings [new map's static version is deliberately better], dossier rail [belongs on /weekly/country]; let-die: per-group filters, signal-weighted arcs, the 180-line alias tables). Task + plan docs banner-cancelled in place (supersede-don't-delete). CLEANUP_AUDIT §4 D3 resolved; legacy-map removal (Phase 2) proceeds as pure cleanup. OPEN: whether to disable the weekly pair cron now that no surface displays its data.

## 2026-09-24 (Pair-arcs relocation Phase 1: "Connections" layer built on /map, off by default, single-accent — build+tests pass, deploy deferred)

Phase 1 of `TASK_2026-09-24_pair_arcs_relocation.md` (per `PAIR_ARCS_RELOCATION_PLAN.md` §2/§6, monitor overrides §8): relocated the pair-arcs ("Connections") feature from the unlinked `/map-legacy` (`WorldMapV2`) onto the live `/map` (`SituationHome` → `SituationMap3D`, deck.gl) as a new opt-in `pair-arcs` `ArcLayer`. `src/utils/countryGeo.js` gained `NAME_TO_ISO3` (inverted `situationLabels.ISO3_NAME` + a small alias overlay for backend `canonicalize()` variants — UAE, Czech Republic, Ivory Coast), which required exporting `ISO3_NAME` from `situationLabels.js`. `SituationMap3D.jsx` gained `pairAnalyses`/`showConnections`/`onSelectCountry` props, a `pairArcs` transform (slug-parsed pair records → resolved centroids, `stale` at a fixed 30-day cutoff — no signal-based strong/mod weight, per monitor decision §8.2), and a `pair-arcs` layer using a **single neutral accent color** `[124, 134, 168]` (desaturated slate/indigo, chosen distinct from `AXIS_RGB`'s four crisis-type hues) at alpha 190 (fresh) / 90 (stale) — the `fx`/`tech`/`geo` 3-color group classification from `WorldMapV2` was dropped entirely, per monitor decision §8.3, so no legend entry is needed; the tooltip still shows `pairTitle`. `SituationHome.jsx` wires `usePairAnalyses()`, a `showConnections` state (**default OFF**, per monitor decision §8.1) and an `onSelectCountry` callback (`navigate('/weekly/country/<name>')`, parity with `WorldMapV2`'s click behavior) into a new "Connections" toggle button beside the existing Globe/Flat and Key controls, shown only when `pairAnalyses.length` and `USE_3D`. The non-3D `<SituationMap>` fallback was left untouched (out of scope, per monitor decision §8.4). `npm run build` clean (no new lint warnings); `npx vitest run` 235/235 passing across 19 files, including all 3 `WorldMapV2` tests, which stay — **the legacy map, its route, and those tests are untouched pending Phase 2** (legacy-map removal, gated on this phase's browser verification). Deploy deferred — no `dist/` copy to `docs/`, no push.

## 2026-09-24 (Cleanup audit D2 resolved: plural SNS topic is ALIVE — keep + document)

Live-AWS check of the `GlobalPerspectivesAlerts` (plural) SNS topic the audit flagged as a possible orphan: it is **alive infra-monitoring**, not dead. ~14 CloudWatch alarms (`Lambda-Errors-Spike`, `lambda-errors-*`, `dynamodb-{read,write}throttleevents-*`, `lambda-throttles-*`), created by `scripts/setup-cloudwatch-alarms.sh`, target it → AWS Chatbot (Slack/Chime) + email. The audit's Lambda-source grep missed it because it's wired at the CloudWatch-alarm layer, not in app code (this is why the singular `GlobalPerspectiveAlerts` app-topic and this plural infra-topic coexist). Documented the two-topic split in ARCHITECTURE.md's alerting section; the near-identical names are an accepted wart (renaming a topic wired to 14 alarms = risk for no functional gain). CLEANUP_AUDIT_2026-09-24.md §4 D2 marked RESOLVED/KEEP.

## 2026-09-24 (Docs-as-code mechanization: task-file convention + pre-commit doc-guard hook)

Made the existing docs-as-code convention (`TASK_WORKFLOW.md`) mechanical, per operator request. (1) `project-docs/playbooks/TASK_TEMPLATE.md` — reusable task-file template (Goal / Reads / Changes (code) / Docs to update / Completion checklist), copied into `project-docs/<domain>/_active/TASK_<date>_<slug>.md` before any non-trivial multi-file goal, so on completion the code + all its docs update in one pass. (2) `.githooks/pre-commit` — **blocks** a commit that stages `amplify/backend/function/**`, `frontend/src/**`, `scripts/**`, or `quality/*.js` changes without a `CHANGES.md` entry (bypass: `SKIP_DOC_GUARD=1` or `--no-verify`), and prints a **non-blocking** reminder of any open task file's unstaged "Docs to update" list when staged code matches it; fail-open, skips merge commits. Coexists with the existing `pre-push` verifier (`core.hooksPath=.githooks`, no new install step). Regression self-test parked at `scripts/test_pre_commit_hook.sh` (5/5 cases pass, throwaway repo). First task file written under the new convention: `project-docs/architecture/_active/TASK_2026-09-24_cleanup_execution.md`, retroactively documenting the CLEANUP_AUDIT §4 Tier A/B + newsStripeWebhook execution (commits `4c264f2`, `a7e1c75`). `TASK_WORKFLOW.md` + `INDEX.md` updated.

## 2026-09-24 (Cleanup execution, commit 2 of 2: Tier B dead frontend code + legacy Amplify scaffold — build + test verified, deploy deferred)

Second half of the operator-approved `CLEANUP_AUDIT_2026-09-24.md` §4 execution. Removed 20 dead frontend files from `global-perspectives-starter/frontend/src/` (10 components — `ApiKeyGate`, `BriefingCard`, `CountryGrouping`, `PerspectiveComparison`, `SectionNav`, `SideNav`, `LoadingStates`, `MiniMap`, `ArticleCard`, `MapSidePanel`; the pre-V2 `WorldMap.jsx`+`WorldMap.css`; `SummaryDisplay.css`; 5 hooks — `usePrediction`, `useSummary`, `useBookmarks`, `useResearchBriefing`, `useTraceCause`; `utils/geocoding.js`; `services/appsyncProxy.js`) after re-confirming zero importers outside the deletion set for each (WorldMapV2.jsx/.css and its 3 test files were left untouched — confirmed still passing). Also removed the repo-root legacy Amplify scaffold `src/` (7 files, 5 tracked + 2 git-ignored) and uninstalled the now-unused `aws-amplify` + `@aws-amplify/api-graphql` deps (`npm ls aws-amplify` empty, package.json diff removal-only). Verified: `npm run build` clean, `npx vitest run` 235/235 passing across 19 files (incl. all 3 WorldMapV2 tests). ARCHITECTURE.md updated (component/hook counts, struck rows for the removed components/hooks, `contentService` callers list, map-section prose). **Deploy deferred** — this is dead-code removal only, produces an identical bundle for existing routes; `docs/` was not touched and `deploy.sh` was not run.

## 2026-09-24 (Cleanup execution, commit 1 of 2: Tier A + newsStripeWebhook — operator-approved, backend/repo-root/S3/env only, no frontend touched yet)

Operator approved Tier A + Tier B + the newsStripeWebhook deletion (Tier D1) from `CLEANUP_AUDIT_2026-09-24.md` §4. This commit executes Tier A + D1: removed the untracked/git-ignored `amplify/backend/function/linkedInAutoPost/` shell (23MB node_modules + stale zip — was never actually in git despite ARCHITECTURE.md:481 claiming it had been removed already; corrected that doc line), `git rm -r amplify/backend/function/newsStripeWebhook/` (operator reversed the 2026-06-01 keep-for-reference call), the 5 root one-off files (`test-gemini.js`, `simple-prompt.js`, `index.html`, the 101-byte empty-stub `package-lock.json`, `global-perspectives-starter/get-pip.py`), and 6 stale S3 objects under `shadow/` in the world bucket. Also fixed `newsAnalyze-sandbox`'s `GROK_MODEL` env var (`deepseek-chat` → `deepseek-v4-pro`, merge-not-clobber against the full var set, verified live afterward). Updated ARCHITECTURE.md (#10, #12, naming-traps line 122), README.md:24, and `CLEANUP_AUDIT_2026-09-24.md` §4 with execution-status notes. Tier B (frontend dead code + legacy `src/` scaffold + npm dep removal) follows in a second, build-verified commit; deploy is deferred to the next explicitly-gated deploy.

## 2026-09-24 (Cleanup audit: verified dead-weight inventory + keep-vs-cut debate + tiered plan — docs only, nothing deleted)

While Phase 4 stays blocked on the provider charge: full-system cleanup audit (`project-docs/architecture/CLEANUP_AUDIT_2026-09-24.md`). Three Sonnet verifiers (frontend / backend+live-AWS read-only / repo+docs+DDB+S3) executed a monitor-written checklist; then a Cutter-vs-Keeper Sonnet debate, monitor-adjudicated. Verified dead: ~2,800 lines of frontend code (10 components + 5 hooks + util + service + 2 CSS, incl. the 797-line pre-V2 WorldMap.jsx), 2 unused npm deps (aws-amplify pair), root legacy Amplify `src/`, the 23MB `linkedInAutoPost/` shell (AWS function confirmed deleted; ARCHITECTURE.md:481 falsely claims the dir was already removed — fix staged), broken root one-offs (test-gemini.js has a literal syntax error), 2.1MB tracked get-pip.py, S3 `shadow/` residue. ALIVE surprises that kill prior assumptions: `/contact` (footer-linked), **`significance` (still sorts the live `newsPostLinkedin` queue — NOT dead data)**, S3 `tags/backfill` (R3 matcher's pre-staged launch-day input — Keeper saved it from a "zero readers" cut). Operator decisions queued (Tier D): newsStripeWebhook reference-keep, the plural `GlobalPerspectivesAlerts` SNS topic (possible console-side Chatbot wiring), and the `/map-legacy`↔WorldMapV2↔pair-arcs entanglement (`/map` now renders SituationHome; the pair-arcs surface is reachable only via the unlinked route, yet WorldMapV2 is eagerly imported so it ships in the main bundle on every load). Zero-byte corpus purge gated on post-charge recovery. Nothing executed; Tier A/B await operator go.

## 2026-09-15 (Deploy-day prep: agreement-script rubric-version stamp + four doc patches pre-staged — Haiku-executed, orchestrator-verified)

Two safe prep items so the Phase 4 deploy commit is mechanical: (1) `quality/severity_agreement.js` gains `--rubric-version` (default "v1 (pre-Phase-4 prompt)") stamped into every report footer — v1/v2 grading eras can never blend (the era-cut discipline); hard-rule footer sentence replaced accordingly; syntax + run verified. (2) The four deploy-coupled doc edits (SEVERITY_CODEBOOK header/§1 relabel, SCORING_STANDARDS verdict, GLOSSARY entries, INDEX row) pre-staged as verbatim-anchored patch text inside ONE_TRUTH_EXECUTION_PLAN.md Step 4.5, with <DATE>/<NUMBERS> placeholders — marked APPLY ONLY IN THE DEPLOY COMMIT. Monitor catch: the executor's test run wrote a stray "test-v9" report into quality/calibration/ (gold set is now fully labeled, so the script no longer refuses) — deleted; the frozen severity-2026-09-14.md baseline untouched. Board now clear until the provider charge lands.

## 2026-09-15 (Live-score flag audit + doc-staleness sweep and merge pass — docs only)

Two Sonnet audits, orchestrator-verified. **(1) Live-score flag pass** (`quality/calibration/live-score-flags-2026-09-15.md`): all 64 currently-served scored records audited against SEVERITY_CODEBOOK incl. SS2.5 rulings — **51 flagged, 0 deflated** (bias one-directional across the full live population); political axis carries ~58 flags (the "involves government/diplomacy" catch-all defect, live: Venezuela political=98, Israel=85 from other countries' actions, the operator-ruled UEFA and Canada cases still served unchanged, 4 countries scored where their own why-text admits no signal). Honest calibration: only **3 reader-facing headline tiers actually drop** (France Elevated→Moderate, Clayton thread High→Elevated, UEFA Elevated→Moderate) — elsewhere a genuine conflict/humanitarian axis independently holds the band, so the damage concentrates in the per-axis scorecards (~80% of records), not the headline colors. This report = the pre-registered before-picture for the Phase 4 prompt fix. **(2) Full staleness sweep + doc-merge pass**: every scoring-related doc checked; found a second staleness wave (claims false TODAY, predating the deploy): SCORING_STANDARDS "BENCHMARK=NONE", GLOSSARY x3, IMPORTANCE_SCALE_MAPPING SS5 "no edit made", INDEX rows (WORLD_MODEL "review pending", ONE_TRUTH "labeling pending", SCORING_STANDARDS "zero ground truth") — all patched. Merge pass applied (banners over rewrites, per TASK_WORKFLOW convention): one living owner per question — WORLD_MODEL=semantics, SEVERITY_CODEBOOK=rubric, quality/calibration/severity-*=living benchmark status (SCORING_STANDARDS trimmed to a pointer for that row); supersede banners added to IMPORTANCE_SCALE_MAPPING, WORLD_MODEL_DEBATE, WORLD_MODEL_FRAGMENTS. Confirmed correctly-frozen (never edit): gold set, baseline report, VERIFY/DEBATE/FRAGMENTS/DESIGN_UPGRADES bodies. quality/dashboard + calibration/latest flagged as stale DATA (regenerate via their scripts once the pipeline resumes; econ scale, separate track).

## 2026-09-15 (Phase 4 prompt-fix: plan + eval harness + candidate v2 BUILT; iteration BLOCKED on two provider/billing issues — both also hitting production)

Phase 4 (measured severity-prompt fix) written into `ONE_TRUTH_EXECUTION_PLAN.md` (offline-eval-before-deploy design; acceptance gates: political exact >=45%, null-violations <=10, inflation count <=20, no within-one regression; max 4 iterations then stop). Built (Sonnet executor, orchestrator-verified): `quality/severity_prompt_eval.js` (runs a candidate rubric over the frozen 28 narratives via the SAME live model endpoints/keys/params as production, incl. thinking-disable guard; grades with severity_agreement metrics + inflation counters; keys fetched at runtime, never printed) + candidate rubric v2 (`quality/prompts/dimensions_v2_{country,thread}.txt` — codebook SS2.5 axis-scope rulings condensed, per-band anchors, hard null rule). The --baseline-check STOPPED itself correctly (per plan's stop rule): only 12/72 axis-observations gradable — **DeepSeek "Insufficient Balance" on ALL 16 country calls** and **Gemini free-tier RESOURCE_EXHAUSTED (20 req/day) on 8/12 thread calls**. Production impact verified in CloudWatch: `newsCountryIntelligence`'s 09-14 07:00 run failed EVERY country on Insufficient Balance (its DeepSeek key's account still empty despite the 09-15 top-up — likely a different account/key); `newsThreadAnalysis`'s daily batch itself exceeds the Gemini 20/day free tier (429 mid-run). Unreliable partial baseline-check recorded at `quality/calibration/prompt-eval-baseline-2026-09-15.md` (flagged, not evidence). Doc-impact survey for the eventual prompt change recorded in-plan (5 MUST docs + dimensionsPromptVersion stamp adoption). BLOCKED on operator: (1) top up / identify the DeepSeek account behind newsCountryIntelligence's key; (2) decide Gemini paid tier vs moving thread analysis off the free tier. No Lambda touched, no prompt deployed.

## 2026-09-15 (Codebook §2.5 — operator axis-scope rulings recorded)

Three binding scope rulings from the operator's baseline adjudication written into `SEVERITY_CODEBOOK.md` §2.5 (Sonnet-drafted, verified): (1) political axis = DOMESTIC institutional stability only — external pressure/sanctions/war-party status never scores political (FIFA/Hormuz/Canada worked negative examples recorded); (2) war + war-risk = conflict axis, never double-stamped into political; credible IMMINENT escalation may top the current band (North Korea example), never jump one; (3) severity scores the PRESENT — future war-fear belongs to predictions/watch signals per WORLD_MODEL D2's importance-vs-likelihood split. Baseline labels already comply → baseline stands; the coming prompt fix must implement these rules and re-score against the frozen 28.

## 2026-09-15 (FIRST SEVERITY BASELINE — Claude-labeled gold set scored; verdict: systematic one-band inflation, political axis worst)

Step 3.4-3.5 of `ONE_TRUTH_EXECUTION_PLAN.md` executed with a disclosed substitution: the 28-record gold set was labeled by **Claude (cross-model judge, blind to model_dimensions via extracted-narrative read)** rather than the operator — provenance stamped in the file; operator may override any label (even 5 spot-checks would upgrade it). Agreement (`quality/calibration/severity-2026-09-14.md`, models deepseek-v4-pro + gemini-2.5-flash, records 07-21..09-12): **overall exact-band 40.3%, within-one-band 84.7%, worst-axis match 75.0% (n=72 axis pairs)**. Per-axis: conflict 53%/93%, humanitarian 50%/94%, economic 41%/88%, **political 23%/68% — the broken axis**. Directional analysis (the decisive part): of 72 comparable pairs — 29 equal, **41 model-higher, 2 Claude-higher**; and **27 axes the model scored where Claude judged null, ZERO the reverse**. Diagnosis: (1) systematic ~one-band upward inflation, not noise; (2) the model never uses null — it invents signal (and sometimes emits score-0 instead of null, the exact error SEVERITY_CODEBOOK.md §2 warns about); (3) the political axis conflates "story involves politics" with "institutional instability" (FIFA governance spat scored political=70; tech-fines thread=60; Nepal floods=60; US-Iran conflict=80 political). This **explains the live 100%-elevated/high distribution**: grade inflation + worst-axis-wins, with political the most-inflated axis. Where both judged the same real signal (wars, disasters), agreement is strong (within-one 93-94%). Next (measured, per the §3.5 hard rule now satisfied): operator spot-check of labels, then a prompt fix (anchors + null enforcement + political-axis definition) re-scored against this same frozen gold set.

## 2026-09-15 (ONE-TRUTH first slice EXECUTED — WORLD_MODEL amendment, shared-module drift guard, severity benchmark kit; zero Lambda changes, zero deploys)

Executed `ONE_TRUTH_EXECUTION_PLAN.md` Phases 0/1/3.1-3.3 (Sonnet executors, orchestrator-verified at each gate; plan itself was adversarially pressure-tested + file-impact-verified first — both passes' fixes are baked into the plan). **(0) WORLD_MODEL.md amended in 3 coordinated spots** (D2 prose, §4 table, Notes): `urgency` re-recorded as the **4th KEEP-AS-IS exception** (tempo, not importance — per IMPORTANCE_SCALE_MAPPING); `significance` stays derive-then-deprecate-but-unconfirmed with decision "leave as-is (legacy)" — its replacement (Phase 2) DROPPED because topics carry no `threadId`, making a thread-riskScore sort join new infra (resolution+read+IAM) for cosmetic gain; grep-verified no contradicting occurrence remains. **(1) `scripts/check-shared-sync.mjs`** — drift guard over the 4 hand-duplicated module pairs (riskDimensions byte-identical; situations-core with tolerance ANCHORED to the literal 2-line header; country_facts structural-compare excluding description/sync; entity-normalization compared via require()+toString/extracted literals since it's a block-vs-file). Verified both ways: live run 4× PASS exit 0, `--self-test` perturbs one byte per pair and catches all 4 as DRIFT. No function files touched (deliberate — comment-only edits would create repo≠deployed drift). **(3) Severity benchmark kit:** `SEVERITY_CODEBOOK.md` (anchored 4×4 rubric + null rule + labeling instructions; prompts untouched), `quality/pick_severity_sample.js` → ran live (read-only): **28-record gold set** (16 country + 12 thread) frozen narrative+model_dimensions at read time, outage-window (09-13→09-15T12Z) excluded (sample spans 07-21..09-12), null-rates printed, pool restricted to records actually carrying `dimensions` (63 of 253 — thread rollout is recent); honest finding: **live worst-axis distribution is 100% elevated/high — zero low/moderate examples exist in production**, so the first baseline can only test high-end calibration. `quality/severity_agreement.js` validated against a hand-computed toy example (numbers matched exactly) and proven to REFUSE the real unlabeled set (exit 1, no report). **Pending:** operator labels the 28 records per the codebook (~2h) → run agreement → the first-ever severity baseline; hard rule: no generator-prompt changes before that baseline exists.

## 2026-09-14 (Design-thinking arc: fragment map, scoring standards, 3-way optimization debate, industry-pattern pressure-test — docs only + stale-doc fix)

A full design-review day, no code deployed. **(1) Whole-backend read:** six Sonnet cluster-readers covered all 37 Lambdas' complete source (~20k lines) — synthesized as two pipelines + analysis cascade + economy/predictions/digests/serve/billing/watchdog verticals; notable findings: `newsSituationTracker` defaults `DRY_RUN=true` (shadow), `newsSensitiveData` = 30-route serve layer, `newsEconomicQuality.is_low_quality` inconsistently enforced (filters the economy lede via `composeEconomyBriefing.js:111` but not the Active-disruptions list). **(2) `WORLD_MODEL_FRAGMENTS.md`:** every fragmented concept classified TRUE-MERGE (5 hand-duplicated modules; `sendEmail.js`'s divergence later confirmed a *deliberate documented fork*, not accidental drift) / SERIAL (signalAdapter = the live prototype; PRED#/ECON# already correctly foreign-keyed; significance→tier blocked on topic-level score) / DISTINCT (urgency=tempo, breaking score=gate, econ severity=magnitude, probability=likelihood, quality≠importance, relevance≠importance). **(3) `SCORING_STANDARDS.md`:** rubric+benchmark audit of all ~10 scores — predictions = the only score with real ground truth (Brier + calibration + era-cut); the canonical severity tier = most relied-on and least benchmarked (one-sentence rubric, zero reference set; ≡ the never-built IMPACT_FIRST P0 set). **(4) 3-way adversarial optimization debate** (Consolidator/Subtractor/Reliability): converged on loud-failure subtractions first → cheap verify harness → piloted consolidation; unanimous no-Lambda-layers. **(5) `DESIGN_UPGRADES_2026-09-14.md`:** six industry patterns pressure-tested — ADOPT: fleet reconciler script (SDK-based, modeled on newsModelGuard), generic artifact-freshness checker, shared-sync diff gate, model code-default scan, completion-marker guard on the 4 true daily-chain dependencies, severity codebook + 25-event gold set (baseline-before-prompt-change rule); SKIP: event-chain rewiring, esbuild bundling (breaks the zip-diff discipline), S3 models.json (needs a ~20-30-Lambda redeploy). **Stale-doc fix applied:** ARCHITECTURE.md §10 + #6 warning + scheduler table still described `linkedInAutoPost` as live three days after its 09-11 retirement (found *by* the reconciler pressure-test; live scheduler listing re-verified 2026-09-14) — corrected to RETIRED. INDEX.md rows added for the three new docs. **(6) `project-docs/GLOSSARY.md`:** plain-language reference card for every decision-board term (scores, identity roots, dormant features, plan terms, the six adopted tools) with what-it-is / where-it-comes-from (file:line + origin) / current-status; linked from INDEX header. **(7) 🔴 LIVE INCIDENT FOUND during verification: DeepSeek account balance exhausted** — every call to `api.deepseek.com` returns 402 "Insufficient Balance" since ~2026-09-13 00:00 UTC; 17 of 19 LLM Lambdas point at that account (topics, agent, situation classifier, country/econ/drift/briefs/resolver...), so the whole AI pipeline has been down ~38h (newest topics 38.5h old; map news-layer corpus files zero-byte hourly; only the 2 Gemini Lambdas — threadAnalysis, econQuality — unaffected). `newsFreshnessMonitor` HAS been SNS-alerting correctly since it crossed the 9h limit. Fix = operator tops up DeepSeek balance (no redeploy needed; pipeline self-heals on next crons). R3 real-tag week restarts after top-up (~09-20/21). Also live-verified: tracker `DRY_RUN=false` (map live, not shadow), `SYSTEMS_TEST_COUNTRIES` removed + `SYSTEMS_TOP_N=10`, breaking alerts 0-of-34 ever confirmed, recommendation ranking has zero frontend callers.

## 2026-09-12 (Importance-scale mapping study — the D2 "easy part" read-only check; docs only)

Ran the WORLD_MODEL D2 consumer-grep as four read-only Sonnet passes (backend consumers, frontend consumers, tier-computability at each read site, verbatim prompt semantics) and wrote the grounded result to `project-docs/architecture/IMPORTANCE_SCALE_MAPPING.md` (+ INDEX row). **The check changed the plan — two evidence-based refinements to D2:** (1) **`urgency` is not mergeable** — its prompt defines it as *tempo* ("breaking in last 24h" vs "developing" vs "slow-moving"), a different axis from the tier's *standing severity*; it should be recorded as a **4th kept exception**, not deprecated. (2) **The canonical tier isn't computable on the surfaces that read the enums** — the tier needs a 0-100 `riskScore`/`dimensions` vector, which is a country/thread-level construct only; the editorial **topic** pipeline (`newsInvokeGemini`) never attaches a score, so Home/Map-halo/lede carry only the enums and can't show a tier without new backend work. `significance` has **zero frontend data reads** (only the LinkedIn auto-poster's post-order sort at `newsPostLinkedin/src/index.js:171-174` reads it, `InvokeLinkedIn` schedule ENABLED); `impactScores` (Trace-the-Cause, 1-10, ungoverned) = keep-as-exception; econ `severity` = keep (already decided). Net: very little should actually be merged now; the only genuinely safe immediate action is documentation (record the 4th exception). No code touched; D2 block not edited (amendment is an operator call).

## 2026-09-11 (Both usage-pass findings resolved: /daily was a false positive; This-week unstalled + auto-publish)

Diagnosed the two live issues the usage-tracing pass surfaced. **(1) /daily — NOT broken, false positive:** the tracing curl sent a flat body instead of the proxy's required `{action, payload:{dateKey}}` envelope, so the server silently defaulted to today's not-yet-generated brief and returned null; with the correct shape the page works end-to-end (generator healthy — 23:00 JST cron, clean runs; data present; the page shows yesterday's brief with an honest banner until today's publishes). No fix needed; smoke-test lesson recorded in `SYSTEM_USAGE_2026-09-11.md`. Optional cosmetic hardening (reject payload-less requests) noted, not applied. **(2) /economy "This week" — genuinely ~10wk stale, now fixed twice over:** root cause was the third unrun human-review gate (cron fired every Sunday without a gap; 9 drafts piled up since the last review on 06-29). A Sonnet reviewer quality-checked the newest draft (2026-09-06: judge-passed grounded notes; the judge demonstrably rejected the one ungrounded note itself; honest "no clear driver" where the dead Perplexity tier left no coverage) → published it (two-field DDB flip; live-verified `weekOf: 2026-09-06`). Standing fix deployed: `newsWeeklyMarkets` now writes `status:'published'` directly (auto-publish; Gemini judge = the quality bar — the `newsWeeklyBrief` precedent), zip-diffed first (bonus: replaced the deployed retired `deepseek-chat` fallback with repo's corrected `deepseek-v4-flash`); `weekly-markets/review.js` stays for spot-checks/retractions. The 8 older unreviewed drafts stay drafts (never served — page shows latest published only).

## 2026-09-11 (Frontend audited to the backend's standard — two audit docs; management proposal in draft)

Two Sonnet auditors covered the frontend the way the 2026-09-10 fleet audit covered the backend. **Structure/contracts** (`project-docs/architecture/FRONTEND_STRUCTURE_AUDIT_2026-09-11.md`): 27-route inventory; the full site-wide data-contract map (page → hook → proxy action/endpoint → Lambda → table/S3 key); ARCHITECTURE.md frontend drift found — the map-as-home components (`SituationHome`/`SituationMap3D`/`useWorld`/`worldData`) were never added to its frontend tables even though their backend is documented; `restProxy.js` is a five-backend god-file with copy-pasted auth blocks; `appsyncProxy.js` confirmed dead (zero importers); client recomputes backend-shipped `lede`/`ranked` (2 confirmed instances); caching-key inventory (7 localStorage keys; doc table stale). Three live contract spot-checks all passed. **Quality/debt** (`FRONTEND_QUALITY_AUDIT_2026-09-11.md`): no route-level code splitting (1.3MB main chunk, ~60-75% cuttable via `React.lazy()`); verified dead-code kill list (`WorldMap.jsx`+css, `MapSidePanel.jsx`, `MiniMap.jsx`); 20 of 32 hooks duplicate the same localStorage-TTL cache block; test coverage inverted (legacy `/map-legacy` well-tested, live `/map` zero dedicated tests, 3/32 hooks tested); unused `aws-amplify` deps. A synthesis pass is drafting the system-management proposal from the combined backend+frontend evidence — options for the operator, nothing built. Docs only.

## 2026-09-11 (Footer fix DEPLOYED to /map + linkedInAutoPost retired — Phase 1 Tier 1 close-out)

Closed out the Phase 1 Tier 1 task with the two parked items (R3 skipped — waiting on ~1wk of real tags). **Footer fix live at /map:** built + deployed the SituationHome change that drops the dead-end "No dedicated situation page yet" footer (renders only when a real destination exists — a matched thread or GDACS report); pairs with the now-live matcher so covered pins show "Full analysis →" and uncovered ones show nothing rather than a broken promise. Build clean (verified the dead-end string is gone from the bundle; `docs/config.js` preserved, `docs/404.html` byte-identical, source maps stripped). **`linkedInAutoPost` retired:** Scheduler `LinkedinThreadsDaily` deleted, Lambda `linkedInAutoPost` deleted (204), repo dir removed (git-recoverable) — the survivor `newsPostLinkedin` (Active, `InvokeLinkedIn` ENABLED, verified) carries LinkedIn at higher volume. Ledger S9·T1 done. R3 fingerprint tier remains flag-gated off pending the ~09-13/14 real-tag re-measurement.

## 2026-09-11 (Event Registry Phase 1 Tier 1 DEPLOYED + verified — map pins now link to their analysis)

Built and shipped the linking matcher (URL-exact Tier 1; R3 fingerprint present but flag-gated `ENABLE_R3_TIER=false` until the real-tag re-measurement). **Writer** `NewsProjectInvokeAgentLambda-dev`: new `matcher.js`/`url-normalize.js`/`entity-normalize.js` (last byte-identical to the classifier's actor-folder), runs in a fully isolated try/catch after threadId assignment, reads `stories/state/*`, writes `threads/story-map.json`. **Reader** `newsSituationTracker`: reads the map each sweep, stamps `situation.threadId` (latching — a proven link survives topic rotation; can't outlive the story it's on). IAM: agent role gained its first S3 grants (`matcher-s3-access`), tracker role gained read on `threads/story-map.json` (both least-privilege, existing statements untouched). Deploys followed patch-zip discipline (writer's grok model-default preserved; additive-only vs live bytes; both shared `situations-core.js` copies verified identical). **Live-verified end to end:** map wrote 25 Tier-1 pairs / 0 ambiguous; tracker stamped threadId on 6 situations; non-fatal isolation proven (the pre-IAM run failed gracefully, generation cycle unaffected); and the links land on REAL pages — verified `/weekly/thread` is archive-entry-driven (`readNarrativeThread`), so freshly-stamped threads (no `THREAD_ANALYSIS` yet) still render the story timeline, with the deeper analysis overlay filling in on the daily run. Matcher harness 10/10, slice-5 31/31, node --check clean. R3 stays gated pending the ~09-13/14 real-tag re-measurement. Source + IAM + docs; commit precedes this entry.

## 2026-09-11 (Backfill sidecar uploaded + matcher build spec reviewed — the bridge is fully specified)

Two parallel Sonnet passes completed the pre-build package for the linking matcher. **(1) Bounded backfill (Phase 1d):** the ~3-day window tagged (204 topics + 890 stories, provenance-marked production vs backfill-agent; conservative empty-actors over guesses; orchestrator review removed 6 bad tags pre-upload) and uploaded to `s3://…/tags/backfill/2026-09-11.json` — a sidecar (originals untouched, not Worker-exposed) giving the matcher full launch-day coverage. **(2) Matcher build spec** (`project-docs/redesign-ux/_active/EVENT_REGISTRY_MATCHER_SPEC.md`): matcher runs inside `NewsProjectInvokeAgentLambda`'s existing non-fatal threadId block → writes `threads/story-map.json` (full-replace) → tracker stamps `situation.threadId`; IAM deltas, deploy methods, verification and rollback specified. Two orchestrator corrections applied: tracker stamping must LATCH (pure mirroring would flicker links on every 4h topic rotation; latched links can't outlive their 90-day thread pages), and the "missing editorial trigger" blocker resolved (fleet uses EventBridge Scheduler — `InvokeGoogleGemini`/`InvokeNewsAgent`). Ship state: **Tier 1 (URL-exact, 0 FP proven on real data) ready on operator yes; Tier 2 (R3 fingerprint) behind `ENABLE_R3_TIER` (default off) until the real-tag re-measurement ~09-13/14.** Docs + one S3 sidecar upload — no code deployed.

## 2026-09-11 (Event Registry Phase 1c — backfill validates the two-subject fingerprint: rule R3 = 0 false links)

Backfilled the past week's tags (Sonnet retroactively fingerprinted the 164 Phase-0 topics + 792 stories with the deployed rules) to answer the fingerprint question now instead of in a week. Result: rule **R3** (≥2 shared countries + ≥1 shared genuine named actor) hits **0% false positives** (vs Phase 0c's 17% floor on thin tags) and adds +3.7 pts, giving **combined URL ∪ R3 = 35.4% coverage, 0 confirmed false links**. Only R3 clears the zero-false-link bar; it rejects every known adversarial collision. Calibration against today's 9 real production-tagged topics: iso3 overlap 100%, event_type 78%, actor overlap 67% — actor tagging is the weak link but weak conservatively (headline-only under-detects vs production full-body), so real coverage likely ≥ measured. This is a PROXY PASS (Sonnet tags, not production): the plan now targets shipping URL ∪ R3, gated on re-confirming 0 FP against ~1 week of real Phase-1b data. Matcher design de-risked: two tiers → `threads/story-map.json` (S3) → tracker stamps threadId. Evidence: `project-docs/redesign-ux/_active/EVENT_REGISTRY_PHASE1C_RESULTS.md`. Read-only — docs only.

## 2026-09-11 (Phase 1b tag emission DEPLOYED + verified end-to-end)

Deployed the event-fingerprint tag emission to all three Lambdas (patch-zip, each function's known model-default drift preserved; newsInvokeGemini node_modules kept). Live-verified the full editorial path: the selector now emits `iso3`/`actors`/`event_type` per topic with genuine named actors and zero country names (Houthis, Israel Defense Forces, Hezbollah, Revolutionary Guard…) — validation drops any actor equal to a country; the fields survive through `staging` → `latest`/active → `archive#today` (carry-through in buildTopic + buildArchiveEntry confirmed, 16/16 fresh entries); and the map classifier's entities went from **21.7% country names to 0.0%**, with 87.6% of articles now carrying a real named actor (was ~30%). Both sides of the fingerprint finally have actor signal — the Phase 0c precondition. The classifier change shifted story keys (entities-based); one supervised tracker sweep folded the re-key ripple (map healthy after: 68 open, tiers 1H/18E/47M/2L, lede ok). `max_tokens` 8192→12000. Selection/summaries otherwise unchanged (additive fields). **Next:** ~1 week of tagged data, then re-run the fingerprint measurement on real tags; the linking tier ships only at zero false links. Source commit precedes this; deploy verified via live invokes. Ledger EVENT_REGISTRY_PLAN Phase 1b.

## 2026-09-11 (Phase 1b tag-emission SPEC written after recon — plan only, nothing built)

Operator chose the tag route ("tag route first"). Pre-build recon (read-only, `project-docs/redesign-ux/_active/EVENT_REGISTRY_TAGRECON.md`) returned GO with load-bearing caveats: three field-whitelists would silently drop the new tags (selector staging write + the summarizer's buildTopic/buildArchiveEntry), `max_tokens` 8192 already truncates ~5% of selector runs (raise to 12000), the map classifier's entities are 21.7% bare country names (needs the "named people/orgs only, never countries" rule too — accepted side effect: a one-time story re-key ripple), and deployed-vs-repo state confirmed (newsInvokeGemini = known single-line model-default drift to preserve; newsSituationIngest clean). Full spec — exact fields, controlled vocabulary, validation, file:line edit list across the 3 Lambdas, deploy method per Lambda, verification steps, and the pre-committed zero-false-link gate for the fingerprint tier — written into `EVENT_REGISTRY_PLAN.md` Phase 1b. URL bridge + footer-fix deploy stay parked behind it per operator ordering. Docs only.

## 2026-09-11 (Event Registry Phase 0c — two-subject fingerprint tested: concept validated, today's tags too thin; tag EMISSION is the precondition)

Measured the operator's tag-fingerprint idea (subject A + subject B + event type, conjunction-matched, controlled vocabulary) on existing data, read-only. The concept holds — strict conjunction rules correctly rejected every known adversarial false pair (mismatched second subject was always the failure mode) — but **no rule achieves zero false positives on today's data** (best 17% FP) because editorial topics carry no genuine actor signal: every observed "shared actor" was a country name in disguise, and requiring a real named entity collapses coverage to zero. Exactly the operator's prediction ("the tags from the article are too few and not correct"). Conclusion: **structured tag emission at generation time** (both LLM passes output subject_a/subject_b/event_type from a fixed menu — real ISO3 codes + genuine actor list) is a precondition for a safe fingerprint tier — recorded as optional Phase 1b (additive prompt fields, forward-only). Until then the URL bridge (~32%, 0 FP) remains the only safe linkage. Bonus: found+documented 7 real omissions in the name→iso3 map. Evidence: `project-docs/redesign-ux/_active/EVENT_REGISTRY_PHASE0C_RESULTS.md`. Read-only — docs only.

## 2026-09-11 (Event Registry Phase 0b — widened-join test FAILED safety; baseline 32% bridge is the only safe one)

Follow-up measurement (read-only, 24,515 corpus articles replayed through the production cluster key): widening the story-side URL join from 5 retained headlines to full corpus sets confirmed the truncation hypothesis (raw rate can roughly double) **but every widening mechanism produced confirmed false links** — the storyId key is only collision-free within one hourly run; accumulating across hours reuses keys for different real events. Per the T3c zero-false-positive rule, the widened join is rejected. Feed alignment also rejected: 81.3% of the unmatched residual is true selection divergence (not feed-pool), ceiling ~45%, and the ingest has zero cap headroom (438-449 fetched vs 420 classified per run). Conclusion: the Phase 0 baseline bridge (~32%, 0 false positives) is the only safe bridge today; the deep residual belongs to Phase 2 (shared ingest). Evidence: `project-docs/redesign-ux/_active/EVENT_REGISTRY_PHASE0B_RESULTS.md`. Phase-1 ship/defer decision remains with the operator. Read-only — docs only.

## 2026-09-11 (Event Registry Phase 0 — URL-overlap measured; safe but below the coverage gate; Phase 1 decision pending)

Ran Phase 0 of EVENT_REGISTRY_PLAN (read-only): measured the exact article-URL overlap between editorial topics (DDB `NewsCache` id=`latest`+`archive#`, `sources[].url`) and map stories (`stories/state/*.json` headline URLs) over 4 days / 164 topics. **Unambiguous match rate 31.7%** (windowed) — below the ~50% rough gate — but **zero false positives** (26/26 sampled same-event; the deterministic no-wrong-link property holds perfectly), ambiguity 26.2% (all axis-flip-residual same-event → resolve to null), and matches skew to high-significance topics (75% vs 33%). Shortfall is structural — the two pipelines select different events (only 13 shared RSS feeds) — not a normalization/Google-News-wrapping artifact (wrapping negligible: topics 0%, stories 1.7%), and fuzzy matching would only rescue 4.5% more, so it stays banned. This is exactly what Phase 2 (shared ingest) fixes. **Decision pending (operator, per missed-gate discipline — no auto-proceed):** ship Phase 1 now at ~32% coverage (safe, additive, richer home-swap front door) vs defer all linkage to the Phase 2 rework. Evidence: `project-docs/redesign-ux/_active/EVENT_REGISTRY_PHASE0_RESULTS.md`. Read-only — no code, no build, no writes.

## 2026-09-10 (Event Registry plan written — one source of truth for event identity; direction approved, nothing built)

Operator discussion resolved the pins-to-pages question at the architectural level: instead of bridging the map and editorial worlds post-hoc (fuzzy matching banned per the T3c measurement), the map's `stories/` layer becomes the **canonical event registry** and editorial becomes a selection over it. New plan `project-docs/redesign-ux/_active/EVENT_REGISTRY_PLAN.md`: Phase 0 measure the **deterministic article-URL-overlap bridge** (both pipelines keep URLs from the same feeds — a shared URL proves same-event, zero fuzziness) → Phase 1 additive bridge (topics gain `storyId`, tracker stamps `situation.threadId` via a small `threads/story-map.json`, zero frontend change — the footer fix already renders "Full analysis →" when threadId exists) → Phase 2 single-ingest spine change (later, gated on a full shadow week — never flip editorial selection on vibes) → Phase 3 riders. Honest limits recorded: editorial coverage stays scarce by design (most pins keep the panel as their page), registry identity residuals (axis flips) leak upward, GDACS stays threadless. Supersedes the S7·T1 "selector consumes stories/index.json" remainder; ledger + INDEX updated. Docs only — no code, no build.

## 2026-09-10 (S9 operational decisions — all four executed on evidence; no code, no frontend build)

Two read-only Sonnet measurements gathered decision evidence, operator approved all four, executed: **(1)** `newsSystemsAnalysis`: removed the binding Phase-1 `SYSTEMS_TEST_COUNTRIES` allow-list (fixed 9 countries daily, 25-30 eligible excluded per run) and set `SYSTEMS_TOP_N=10` — coverage now rotates with actual news volume (merge-don't-clobber env write, 8 keys preserved). **(2)** Disabled the breaking-email sender cron `TriggerBreakingEmailSend` (15-min poll): exactly 1 breaking subscriber exists, 33 proposed / 3 ever sent; alert generation stays ON (feeds the in-app bell). **(3)** Disabled `linkedInAutoPost`'s schedule `LinkedinThreadsDaily`: 13 posts ever vs 5,227 from `newsPostLinkedin`, same-day duplication, 3 days silently dead on an expired token this month — delete after a week's observation. **(4)** Adopted the prediction-resolution policy **N=2 weeks + disclosed "lapsed unscored" tier**: measurement showed the "37k backlog" is half legacy-excluded-by-design (real actionable: 4,287 due v1 triggers) and that no triage window converges (~1,120 newly-due/week vs ≤280/week ceiling) — so only fresh triggers get resolved, older lapse with disclosure; the /track-record disclosure text rides along with the next frontend deploy (no build now); structural fix (reduce trigger generation volume) flagged for the next generator touch. Frontend impact of all four: none (systems 404s are designed; bell unaffected; LinkedIn external; policy is docs-only today). Ledger S9·T1/T4.

## 2026-09-10 (T3c axis-flip merge — MEASURED, decision: do not build; closed as accepted residual)

Ran the measure-before-build for cross-axis story consolidation (S5.5·T3c) instead of building it: a read-only Sonnet pass replayed all 44 corpus hours (09-08→09-10) through the real production `clusterStories`, cross-checked 637 `stories/state` files, and hand-labeled all 275 cross-axis candidate pairs (table: `project-docs/redesign-ux/_active/T3C_MEASUREMENT_PAIRS.md`). Findings: only 21/275 pairs are true same-event (7.6%); payoff is ~3.5 tier-crossing merges/day; and **title-Jaccard cannot separate same-event from different-event pairs** — the zero-false-merge threshold recovers just 33% of true merges, and the motivating false pair ($5k pledge vs Iran war) scores identically (0.138) to a genuine same-event pair. Since a false merge (hiding a real situation) is worse than fragmentation the corroboration cap already defuses, the automatic merge is unsafe at any useful recall and a human-review queue would be process debt (existing review queues already sit unrun). **Decision: axis-split fragmentation is an accepted, documented residual**; re-measure cheaply if the cost visibly grows. Side-finding: 148 pre-pass-2 slug-drift pairs in `stories/state/` are inert (nothing reads old keys; 45-day lifecycle ages them out). No code or infra changed — docs + evidence artifact only. Ledger S5.5·T3c.

## 2026-09-10 (Identity pass 2 — actor-alias fold DEPLOYED + live-verified)

Follow-up to slice 5: fixed the dominant story-fragmentation vector the conservative pass-1 normalization left. Added a curated `ACTOR_ALIASES` map (editorial-fact-layer pattern) applied in `normalizeEntity` (`newsSituationIngest/classifier-core.js`) so full-name/given-name variants of ~20 recurring world figures fold onto one unambiguous-surname key ("Donald Trump"/"President Trump"/"Trump administration" → `trump`). Pure name-variant normalization — no claim about who holds office (a fold stays correct regardless), unambiguous surnames only (full name kept where a surname is shared, e.g. `xi jinping`), a missing key is a harmless no-op. Deployed `newsSituationIngest` (patch-zip, Sha `hqnV8oEit…`) and live-verified: **Trump-related storyIds collapsed 6 → 3** — the given-name splits (`donald-trump`/`trump`/`trump-advisers`) folded into `trump`; the 3 that remain are split purely by axis. 9/9 unit checks green (incl. North/South Korea staying distinct — why the alias map beats surname-extraction). **Remaining residual = axis flips** (same event as political/conflict/economic), recorded as S5.5·T3c (deferred — needs content-similarity merge to avoid fusing distinct same-actor stories). Commit follows `5fa77c2`. Ledger S5.5·T3b.

## 2026-09-10 (Slice 5 — situation feed-quality bundle DEPLOYED to prod + live-verified)

Deployed the slice-5 bundle to the three situation Lambdas (`newsSituationTracker`, `newsSituationIngest`, `newsGdacsIngest`) via patch-zip — all confirmed clean baselines first (live bytes == repo before my edits; no `node_modules`; no deployed-only defaults). Live-verified across a full ingest→tracker loop: **the corroboration cap is the headline win** — open-tier distribution moved from **1 high / 35 elevated / 2 moderate / 15 low** to **1 high / 14 elevated / 38 moderate / 2 low** (the single-outlet stories that were inflating "elevated" now correctly sit at "moderate"; GDACS tiers untouched). `coverage_ratio` (1.17 on the sampled Iran situation) and `tier_changed_at` now emitted — the live "new"-mislabel bug is fixed. Source-aware cooling confirmed (no new GDACS-text/`gdacs_*` leak onto news rows; 10 residual leak rows are pre-deploy cooling/closed and self-heal). Reopen path fired live (`reopened: 4`, fresh `opened_at`). **Honest caveat:** the identity `normalizeEntity` fix only folds title/org/article variants — the dominant live fragmentation (given-name `donald-trump` vs `trump`, and axis flips) is the deferred residual, so single-outlet share barely moved; the CAP, not the identity fix, is what quiets the map today. A second identity pass (S5.5·T3b) is the top churn-reduction follow-up. Env knobs `CORROBORATION_CAP`/`RAISE_COOLDOWN_MIN` live for tuning; rollback = `CORROBORATION_CAP=false` or prior zip. Commit `ffd80cc` (source). Ledger S5.5·T3.

## 2026-09-10 (Slice 5 — situation feed-quality bundle BUILT + verified, source only, NOT deployed)

Built the full slice-5 bundle (Option A) into the situation-pipeline Lambda source — **not deployed** (the prod deploy triggers a one-time storyId re-key wave and is gated on an explicit yes). Six coordinated fixes from `SITUATION_BACKEND_AUDIT.md`: **(1) story identity** — `newsSituationIngest/classifier-core.js` gains `normalizeEntity()`, folding lexical variants of one actor onto a single cluster key (leading article + honorific/title prefixes + trailing generic org words stripped; "President Trump"/"Trump administration"/"the Trump Administration" → `trump`), conservatively (no fuzzy merge — merging distinct actors would hide situations); **(2) corroboration cap** on news tier — new pure `capTierByOutlets` (1 outlet→moderate, 2→elevated, ≥3→high), severity still the floor, GDACS provably untouched; **(3) source-aware cooling** — news situations cool as "No longer in active coverage" with clean evidence (the "Event no longer current in GDACS" leak + `gdacs_*` splice onto news rows is gone) and keep pre-cool tier through the grey-out; **(4) escalation hysteresis** — velocity≥1.5 counts only with prev outlets≥2, spread ignored on brand-new stories, repeat `raised` rate-limited (`RAISE_COOLDOWN_MIN`, default 180); **(5) reopen handling** — a reappearing closed storyId re-opens fresh (`reopened`/`emerging`/reset `opened_at`) instead of resuming at `peak` with a stale age; **(6) slice-4 fields** — `evidence.coverage_ratio` (fixes the live "new"-mislabel bug — frontend already reads it) and `tier_changed_at` (emitted in `summarize()`, ready for the FE "escalated since your visit" consumer). New env knobs `CORROBORATION_CAP`/`RAISE_COOLDOWN_MIN`; `threadId` now preserved across rebuilds. **Verification:** 31/31 logic checks (throwaway harness), `node --check` clean on all 5 files, the two `situations-core.js` copies re-confirmed logic-identical (only the 2-line shared-module header differs). Files: `newsSituationTracker/src/{situations-core.js,index.js}`, `newsGdacsIngest/src/situations-core.js`, `newsSituationIngest/src/classifier-core.js`. Plan/status: ledger S5.5·T3. **Source only — not deployed; deploy plan + rollback recorded in the ledger.**

## 2026-09-10 (Planning sync: slice-5 re-based on the audit; new ledger Stage S9 for fleet hygiene — docs only, nothing implemented)

Recorded the backend-audit conclusions as planned work so every pending modification lives in the tracking docs. **Ledger `MAP_HOME_SITUATION_LEDGER.md`:** S5.5·T3 rewritten — the feed-quality gate is now a 7-item causal bundle (storyId identity stabilization → corroboration cap → source-aware cooling → escalation hysteresis → suppression/reopen fixes → slice-4 structured fields rider → env/lifecycle ops riders) with constraints (byte-identical `situations-core.js` in both dirs, zip-diff before edit, env-overridable thresholds, churn-metrics exit criteria); new **Stage S9 — Fleet hygiene & retirements** (retire `linkedInAutoPost`, delete `newsStripeWebhook` dir + dev.to dead code, dead env-var cleanup incl. the unresolved `newsPostLinkedin` ghost-platform question, the three operational decisions — breaking-confirm workflow, 37k prediction-backlog triage, `SYSTEMS_TEST_COUNTRIES` — and hardening riders: `list_alerts` Scan, shared vendored modules, audit-feedback loop, morning-chain completion guard). **`MAP_UI_FIX_QUEUE.md`** execution-order + slice-status rows updated to point at the re-based scope. All items are planned only — no code, no infra, no env changed; each S9 item needs its own operator yes.

## 2026-09-10 (Whole-backend audit: every Lambda read + live-verified; ARCHITECTURE.md reconciled as backend source of truth)

Ten Sonnet auditors (4 on the situation pipeline, then 6 covering every remaining Lambda) read every source line of all 37 function dirs and verified live AWS state read-only — real schedules (both classic EventBridge rules AND EventBridge Scheduler), enabled/disabled states, Function URL + CORS configs, env var names, S3 layout/lifecycle, alarms — with load-bearing claims re-verified by the orchestrator against live data. Results: **`project-docs/architecture/BACKEND_AUDIT_2026-09-10.md`** (fleet inventory + 14 ranked findings) and **`project-docs/redesign-ux/_active/SITUATION_BACKEND_AUDIT.md`** (situation pipeline deep-dive: storyId fragmentation as the churn root cause, GDACS-text leakage into cooling news rows, `coverage_ratio` consumed-but-never-emitted, one-pin merge killing open situations, etc.). Headline fleet findings: prediction backlog is **37,021 pending triggers** vs a binding 40/day resolver cap (memory said 585); breaking-alert subscriber broadcast has **never flowed** (0 `confirmed` rows ever — the operator confirm step `breaking/review.js` is never run while the 15-min sender cron polls); two LinkedIn posters share one account with no cross-dedup; `newsWeeklyMarkets`' Perplexity tier and the summarizer's Brave grounding are silently OFF (no keys); `SYSTEMS_TEST_COUNTRIES` testing allow-list still live on prod; `newsRecommend`'s public `list_alerts` is an unauthenticated full-table Scan; CORS is clean fleet-wide; `newsSignals` deployed-drift RESOLVED (redeployed 2026-09-09 with S8); GDELT ingest healthy. **`ARCHITECTURE.md` reconciled in place** as the backend source of truth: new "Deploy model" section (repo = intent, deployed zip × env = truth; env as second deploy surface; review scripts as pipeline stages; hand-synced module pairs; naming traps; no-CI compensations), 7 new numbered Lambda sections (#31–37: gdacs/situation trio, gdelt, impact audit, predictions snapshot, signals), corrections across ~14 existing sections, verified cron chain, S3 lifecycle gap, INDEX.md rows for both audit docs. Memory corrected (signals drift resolved; prediction backlog re-measured). Docs only — no code or infra changed.

## 2026-09-10 (Map patch slice 2b + 3 DEPLOYED to /map: spread arcs, affected fill, flat/globe toggle)

Built the two remaining frontend slices into `docs/` and shipped them to the unlisted `/map` (commit `aec70d6`; `/` still the old Home). **Slice 2b** — selecting a situation now draws spread **arcs** from its origin to each affected-country centroid (axis hue, fading toward the target, no arrowheads) and tints those **country polygons**, rings the destinations hollow, and dims every other pin ~40%; arcs/fill are a selection state, absent in the base view (the anti-spaghetti rule). New `utils/countryGeo.js` (ISO-3 → topojson numeric-id join, 133 entries + 4 manual centroids for countries absent from the 110m mesh); centroids/polygons resolved at runtime via `d3-geo`. **Slice 3** — a **flat ↔ globe toggle**: the overview defaults to the flat map (every pin legible), a "Globe" button by the Key chip swaps to deck.gl `_GlobeView` (choice persisted in localStorage per visitor). deck.gl can't morph between MapView and GlobeView, so this is a deliberate user switch, not an auto-transition; on the globe the callout re-projects via `_GlobeViewport` with far-side culling, fly-to zoom is view-aware, and spread arcs render as true great circles. Verify green (eslint 0 errors, 235 tests); browser-verified against live data (click 4/4, arcs on a 5-country situation, globe toggle + persistence), 0 console errors. `docs/config.js` preserved, `docs/404.html` resynced; map chunk `SituationMap3D-Ck5ZYIzE.js`. **The frontend map programme is now fully shipped** (slices 1, 2a, 2b, 3 all live). Files: `SituationMap3D.jsx`, `SituationHome.jsx`, `SituationHome.css`, new `utils/countryGeo.js`.

## 2026-09-10 (S7: Brave removed from newsInvokeGemini ingest — LIVE, deployed)

Executed the plan's Brave-in-ingest removal (§2.6 / old P1·T4). `newsInvokeGemini` (the 4-hourly editorial content pipeline) no longer calls Brave News for gap-fill: deleted `fetchBraveNews()`, the `BRAVE_API_KEY`/`BRAVE_NEWS_ENDPOINT` consts, and the RSS+Brave merge — `fetchAllNews()` is now RSS-only. Grounded in the P0·T1 measurement: Brave was ~5.8% of the article pool, ~18% 429-rate-limited, and only **0.4% of chosen topics were sourced solely from Brave-only domains** (the wire targets it chased — reuters/apnews — yielded ~0.38 articles/run), so removal is no meaningful loss and needs no fallback. The **five analysis-grounding Lambdas keep Brave** (separate provider follow-up, not touched). Deployed via the patch-the-deployed-zip method (downloaded the live zip, confirmed it differed from repo ONLY in the env-overridden `MODEL_NAME` default + one comment, applied the Brave removal, preserved `node_modules` + the deployed grok default, `update-function-code`). **Verified live:** a scheduled-style invoke returned 200 / no error / 13 topics from a **168-article RSS-only pool** (matches the measured ~167.5 RSS/run — Brave's ~10 gone with no loss); CloudWatch shows `COMBINED: 164 unique articles (168 RSS + 0 Brave; Brave removed S7)` and zero Brave call/failure lines. Cuts ~1,800 Brave ingest calls/mo and the 429 noise. The now-unused `BRAVE_SEARCH_API_KEY`/`BRAVE_CONCURRENCY` env vars on this function are left in place (harmless; avoids a clobber-risk env write). Files: `amplify/backend/function/newsInvokeGemini/src/index.js` (repo = source of truth; deployed keeps its grok `MODEL_NAME` default per patch-zip). Docs: plan §2.6, ledger, memory.

## 2026-09-10 (Map patch port — slice 1 + 2a: unclickable-pin fix, manual tour, mobile, since-you-last-looked — source only, NOT deployed)

Second `/map` pass, from the operator's post-deploy review + the audit in `MAP_UI_FIX_QUEUE.md`. **The reported "dots are unclickable" bug is fixed** — two causes: (1) the pickable core dot was 3–5px with deck.gl's default 0px click tolerance while the visible target was the glow → added `pickingRadius={16}` + a pointer cursor over pins; (2) the 60fps breathing-halo `setState` loop re-rendered the map and a frame landing mid-click ate the tap → **the escalating halo is now a distinct STATIC double-ring** (bigger + a hollow outer ring), removing the animation entirely (also kills idle CPU). Headless click testing (SwiftShader) now opens the detail on 4/4 fresh-page clicks incl. two 11px off-centre — the previous build was 0/N. **Trade flagged:** motion-exclusive-to-escalation became a static distinct treatment; breathing can return later as a shader-driven pulse (globe slice) that never touches React render.

Also from the patch-design boards (`MAP_HOME_DESIGN_TARGET_PATCH.html`): **manual tour** replacing the autoplay timer (design 3b) — "Walk me through today" opens a tour bar in the anchored card (n of N, ←/→/Stop, progress ticks, Esc to exit, rail highlights the stop); advancing flies the camera and **never writes the URL** (drives a local `tourId`). **Callout edge rule** (design 3c) — 4 candidate quadrants (NE/NW/SE/SW), first that fits with 8px margin, else clamp + a ≤180px leader line; fixes the edge-detachment. **Mobile pass** (design 3d) — map height responsive (~60vh via measured wrapper), legend collapses to a "Key" chip → bottom legend sheet, a fixed **peek row** (top situation, tap → detail) as the mobile lede+hero, 44px tap targets. **"Since you last looked"** v1 (design 3f) — lede clause ("· N new since <day>"), rail ◇ new markers, and a hollow-ring on new pins; new-only using `opened_at` vs a localStorage `gp_map_last_seen` stamp (the "escalated since" variant needs a `tier_changed_at` field the bundle lacks — deferred). **Bug fixes** from the audit: error branch on the lede, expired/closed `?focus=` deep-link state, state word rendered in the detail head, `??`-vs-`||` 0-outlet copy, `--sh-dim` raised to #7c879a (WCAG AA), scrollbar styling, sticky back button. Cadence copy now derived from `next_expected_at` instead of a hardcoded "every 30 minutes". Verify green (eslint 0 errors, 235 tests); all deterministic paths (rail/callout/tour/mobile/legend) browser-verified against live prod data, 0 console errors. **Deferred: slice 2b** (spread arcs + affected-country fill — needs an iso3→centroid/polygon join, pairs with the globe), **slice 3** (globe fly-to, variant C), **slice 4** (tracker `tier_changed_at`/`coverage_ratio`), **slice 5** (feed-quality gate). Files: `SituationMap3D.jsx`, `SituationHome.jsx`, `SituationHome.css`. Frontend **source only — not built into `docs/`, not deployed.**

## 2026-09-09 (Map-as-home design port: /map rebuilt from the approved design target — source only, NOT deployed)

Reworked the `/map` situation home from the ground up to a designer-produced target (`project-docs/redesign-ux/_active/MAP_HOME_DESIGN_TARGET.html`, in-repo), after an in-person review found the shipped version read as a working renderer, not a finished product. **Data contracts unchanged** — `useWorld`/`useSituationDetail`, `worldData.js`, the Worker `/data/*` route, and `world/latest.json` all untouched; this is a presentation-layer rewrite. Changes: **(1) Encoding** — dropped extruded columns (they occluded each other exactly where severity mattered) for glow **dots**: hue = axis, luminance + halo radius = tier, a **white keyline exclusive to high tier**, and a slow 2s **breathing halo exclusive to escalating** (the only motion on the page, off under reduced-motion). Palette **normalised to `oklch(0.70 0.155 h)`** across map + list + D3 fallback so the old lightness imbalance (amber shouted, violet sank) is gone. **(2) Composition** — templated count-only **lede** ("18 high-tier situations active · Europe escalating") whose region clause is anchored to the ranked hero; an anchored **hero callout** projected onto the top situation; a 70/30 **map + rail** split where the detail panel **replaces** the rail (one-tab-system, back arrow); the tour is now a **"Walk me through today"** button that **defaults OFF, never fires on load, and never writes the URL** (drives a local `tourId`, not `?focus`). Rail is height-capped with internal scroll. **(3) Detail panel** — structured **metric row** (outlets / spread / vs-prior, parsed from the tracker's existing `what_changed` so it works with no backend change), affected-country **name chips** (new `utils/situationLabels.js` ISO-3→name/region + lede builder), outlet-attributed evidence rows, a **thin-evidence** state ("Coverage is still consolidating…"), a **GDACS disaster variant** (hazard facts + official-report link + pass-through note), and a **"No dedicated situation page yet"** footer when `threadId` is null. **(4) Below the fold** — truthful **methodology** copy rewritten to what the tracker actually does (the mock's invented scoring text was NOT shipped), corrected section teasers, and a crawlable axis-grouped **situation index** (SEO). Browser-verified against **live prod data** (SwiftShader, 1440 + 390): lede, hero callout, rail, click→fly-to→detail with real metrics/evidence/affected-names, below-fold, mobile — 0 console errors. Verify green (eslint 0 errors, 235 tests). **Deferred:** on-map side-flag leader labels (P5, the custom collision piece — smaller dots + rail already carry labels); and the backend quality items this surfaced (the feed currently over-tiers single-outlet stories to "high" and keeps stale "Closed — inactive" rows — a tracker/classifier gate, its own pass). Files: `SituationHome.jsx`, `SituationMap3D.jsx`, `SituationHome.css`, `SituationMap.jsx` (palette), new `utils/situationLabels.js`. Frontend **source only — not built into `docs/`, not deployed.**

## 2026-09-09 (Deploy: ship the 2.5D situation map to prod as an unlisted route `/map`)

Frontend deploy — the map-as-home work reaches prod for the first time, but **only as an unlisted preview route**: `/` still renders the existing Home (the S6 home-swap is deliberately NOT done yet), and `/map` now serves the live 2.5D situation map (deck.gl `SituationMap3D`, code-split into its own 909 KB chunk loaded only on that route), with `/map-legacy` keeping the old `WorldMapV2`. Hidden link for cross-device review: `https://globalperspective.net/map`. Pre-deploy gate green (eslint 0 errors, 235 vitest tests pass); `docs/config.js` preserved; `docs/404.html` resynced byte-identical. No backend change. Home, nav, and all other routes unchanged.

## 2026-09-09 (S8·T3: /track-record no longer Scans PredictionLog on every load; LIVE)

Killed the `PredictionLog` full Scan that ran on **every** `/track-record` page load — the table's 2nd-hottest read path (~12.5k reads/day, ~5.5k items each). **Reframed on inspection:** `PredictionLog` should NOT be dropped — it is a legitimately **mutable per-record store** (the resolver updates trigger verdicts in place; `prediction_snapshot` does cheap point Queries). Forcing it into one JSON would be wrong. So T3 is a **read-optimization, not a table migration**: a new `newsPredictionsSnapshot` Lambda (`rate(30 min)`) Scans the table once per cadence and writes the finished aggregate to `predictions/track_record.json`; the `newsSensitiveData` proxy `prediction_track_record` serves that cached object and only falls through to a live Scan if the cache is missing — so correctness holds even before the builder first runs. The aggregation is a pure module (`trackRecord.js`, 11 tests) that was **byte-verified equal** to the proxy's inline computation against live data (v1Logged 3073, resolved 122, brier 0.154, calibration + all 30 recent[] identical). After deploy, the proxy returns the cached aggregate with zero Scan — verified. `prediction_snapshot`, the writers (`NewsProjectInvokeAgentLambda`, `newsPredictionResolver`), and the table all stay on DynamoDB. New IAM: `newsPredictionsSnapshot-role` (Scan PredictionLog + PutObject `predictions/*`); proxy role +GetObject `predictions/*`. The proxy was deployed via patch-the-deployed-zip (its `index.js` was byte-identical to repo; `lib.js`/`dossier.js` preserved). (`newsSignals` `/v1/track-record` still Scans but has 0 consumers — optional later.) Files: new `amplify/backend/function/newsPredictionsSnapshot/src/{index.js,trackRecord.js,trackRecord.test.mjs,package.json}`, `newsSensitiveData/src/index.js`. Docs: `DATA_STRATEGY.md` §4/§6, `ARCHITECTURE.md`, ledger S8·T3.

## 2026-09-09 (S8·T2b: IngestCapture → S3; newsImpactAudit is now fully DynamoDB-free; LIVE)

Finished the S8·T2 audit-subsystem migration by moving the last table, `GlobalPerspectiveIngestCapture` (the selector's saw-vs-chose capture, 344 writes/day, read only by the audit). Its writer is `newsInvokeGemini` — the live editorial pipeline — so it was held out of T2a; done now because the change is genuinely low-risk: `captureIngestion` is **already fully fenced** (swallows every error, can never break ingestion), so swapping its one DDB `PutItem` for a best-effort S3 `PutObject` to `audit/ingest-capture/latest.json` has zero content blast radius. `newsImpactAudit` now reads the capture from S3 too, making it **fully DynamoDB-free** (all four T2 tables gone). Deploy used the **patch-the-deployed-zip** method (download → swap only `index.js` → preserve `node_modules` and the deployed `MODEL_NAME` default, which differs from repo but is overridden by the `GROK_MODEL` env var anyway). Seeded `audit/ingest-capture/latest.json` from the last DDB row so the audit had data immediately; the pipeline self-refreshes it every 4h. **Verified live:** the audit ran fully off S3 — `missed:5`, `gdeltGaps:2`, reading the seeded capture (input 180 / chosen 13). IAM: `newsInvokeGemini`'s `ingestCapturePut` repurposed DDB→`s3:PutObject audit/ingest-capture/*`, `WORLD_BUCKET` env added; the audit's DDB Scan grant dropped, S3 read widened to `audit/*`. `IngestCapture` dropped (201 items, 15MB). Files: `newsInvokeGemini/src/index.js` (repo — deployed via patch-zip), `newsImpactAudit/src/index.js`. Content pipeline behavior otherwise untouched. Docs: `DATA_STRATEGY.md`, `ARCHITECTURE.md`, ledger S8·T2.

## 2026-09-09 (S8·T2a: GDACS/GDELT/ImpactAudit DynamoDB tables → S3 corpus + audit; LIVE)

Second S8 migration, scoped to avoid the live content pipeline. `newsImpactAudit` (the daily coverage dead-man's-switch) was the only reader of three tables: `GdacsEvents` (1,100 writes/day), `GdeltConflict`, and `ImpactAudit` (its own write-only output). Now: `newsGdacsIngest` writes the full current disaster set to S3 **`corpus/gdacs/latest.json`** (best-effort, alongside the existing situation inbox) and its **DDB mirror is stopped**; `newsGdeltConflict` writes per-day country aggregates to **`corpus/gdelt/<day>.json`** and its DDB mirror is stopped; `newsImpactAudit` reads GDACS + GDELT from S3 (today+yesterday for GDELT, preserving its 2-day window) and writes its verdict to **`audit/impact/<date>.json`+`latest.json`** instead of the `ImpactAudit` table. All three tables were **dropped** (`GdacsEvents` 2,120 items, `GdeltConflict` 274, `ImpactAudit` 53). **Verified live:** invoked both producers (corpus written), then the audit ran fully off S3 — `missed:7`, `gdacsGaps:0` (all-Green cycle), `gdeltGaps:1` (Panama, mentions 36 / Goldstein −10, straight from the S3 corpus), verdict written to `audit/impact/`. One intentional, documented refinement: the GDACS gap now reflects the current-active feed (what the tracker already uses) rather than a TTL-accumulating scan — it no longer flags disasters that already closed and dropped off the feed. IAM: producers +PutObject on their `corpus/*` prefix, dead `PutItem` grants removed; audit +GetObject `corpus/*` / +PutObject `audit/impact/*` / +scoped ListBucket (missing key → 404), Scan narrowed to `IngestCapture` only. Lifecycle: `audit/impact/` → expire 400d (`corpus/*` already covered). Files: `newsGdacsIngest/src/index.js`, `newsGdeltConflict/src/index.js`, `newsImpactAudit/src/index.js`. Backend-only, zero user-facing surface; **the content pipeline (`newsInvokeGemini`) was not touched.** **Deferred (T2b):** `IngestCapture` (344 writes/day, read only by the audit) → `audit/ingest-capture/` — held out because its writer is the live editorial pipeline; low urgency. Docs: `DATA_STRATEGY.md` §4/§6, `ARCHITECTURE.md`, ledger S8·T2.

## 2026-09-09 (S8·T1: Signals DynamoDB table → S3; the first table migration, LIVE)

First step of the S8 DDB→S3 migration. Two independent verifications (a code-reader grep/audit + a 7-day CloudWatch pull) found `GlobalPerspectiveSignals` was **write-only** — ~23,390 writes/day (the daily Signal-API build) with **0 reads over 14 days** and `newsSignals` invoked once/day (cron only, zero HTTP; 0 API keys ever minted). So it became the safe opener: `newsSignals` BUILD now writes a single **`signals/latest.json`** (+ immutable dated `signals/snapshots/YYYY-MM-DD.json`) in the world S3 bucket instead of ~5.8k per-item PutItems; SERVE (`/v1/signals`, `/v1/signals/{id}`) reads `latest.json` ETag-cached in module scope — behaviour identical to the old per-request full table Scan. Retention is now an explicit `event_time ≥ now−120d` filter (the old 120d DDB TTL never expired anything — the daily rebuild restamped it); `first_emitted_at` is carried across rebuilds from the previous `latest.json`. Pure store logic factored into `src/signalStore.js` (+15 tests → 59 total, all green). **Verified live:** build wrote 5,468 current signals (the table's 5,815 included ~347 accumulated stragglers no longer in source); a temp paid key confirmed list/get/filter return the same v1 envelopes from S3, no-key → 401; then the table was dropped. IAM: `newsSignals-ddb` gained `s3:GetObject`/`PutObject` on `signals/*` + scoped `s3:ListBucket` (so a missing key returns 404 not 403), and the dead Signals table ARN was removed; env gained `WORLD_BUCKET`, lost `SIGNALS_TABLE`. S3 lifecycle: `signals/snapshots/` → Glacier IR at 30d, expire 365d. **`signals/` is never exposed via the Cloudflare Worker `/data/*` route** — paid, key-gated product, Function URL only. `GlobalPerspectiveApiKeys` stays in DynamoDB (auth data). Files: `amplify/backend/function/newsSignals/src/{index.js,signalStore.js,test-adapter.mjs,package.json}`. Backend-only, zero user-facing surface. Docs: `DATA_STRATEGY.md` §4/§6, `ARCHITECTURE.md`, `SIGNAL_API_PLAN.md`, plan §11.1 + ledger S8·T1.

## 2026-09-09 (S3 source-broadening: regional RSS + English-title translation; GDELT unreliable from Lambda)

Fixing the Anglophone blind spot the plan targets (my news ingest was ~14 English feeds — the same Western skew the plan indicts). Tried GDELT DOC 2.0 first (the plan's WS1.1 choice) but it's **unreliable from Lambda** — shared egress IPs get IP-throttled / connection-refused / 429'd (0 articles across 4 queries); left opt-in (`GDELT_ENABLED=true`) for a future Worker-routed path. Achieved the same goal reliably instead: **11 regional/non-Western RSS feeds** (Times of India, Al Arabiya, Moscow Times, Kyiv Independent, MercoPress, Rappler, Daily Sabah, Africanews, Le Monde, Jerusalem Post, Buenos Aires Times) plus a classifier **`en_title`** field that translates non-English headlines into a concise English map label. Result: **415 articles / 21 domains** (was 283/14), higher cross-outlet confirmation (e.g. UK–Israel story 5→7 outlets), map now spans India/Philippines/Turkey/Africa/France/Russia sources. Backend deployed; frontend source-only.

## 2026-09-09 (S5·T2: fly-to + idle tour + source-aware card — browser-verified; frontend source only)

Map interactivity. Clicking a situation (or the idle tour auto-advancing) flies the deck.gl camera to it (FlyToInterpolator, zoom-in + tilt); clearing returns to the overview. Idle tour cycles through the top 6 situations every 6.5s with a ⏸/▶ toggle (localStorage `gp_map_tour`, pauses on any manual click, 3D-only, off under reduced-motion). Detail card is now source-aware: a news situation shows outlet count, coverage/velocity/spread, up to 4 linked headlines, and an "aggregated from live news coverage" note; a GDACS situation shows the official report link + the deterministic note. Ingest now includes top-3 headlines in `stories/index.json` so the card can list them. Stale grey-out on the 3D map is covered by the existing page-level `.sh-stale` filter. Browser-verified (SwiftShader): tour advances + fires fly-to, pause toggles, click flies + shows the source-aware card, 0 console errors. **Scrubber deferred** to a follow-up. Backend (ingest headlines) deployed; frontend source-only.

## 2026-09-08 (S3·T3: tracker consumes news stories → the map fills with the world; LIVE)

The payoff. `newsSituationTracker` now reads `stories/index.json` each sweep and folds significant news stories (≥3 outlets OR severity ≥4, cap 40) into situations alongside GDACS. New `buildStorySituation` in `situations-core` (severity→tier, axis from the story, escalating on velocity≥1.5 or spread; situationId `news#<storyId>`); `foldSweep` generalized to merge pre-built situations from any source and cool by the union of present ids; **one-event-one-pin merge** — a humanitarian story sharing a country with a live GDACS situation is suppressed so a disaster isn't double-pinned; `world/latest.json` gains a `news` freshness stamp. Live-verified: **40 situations across all four axes** (conflict 17, political 13, humanitarian 7, economic 3 — Houthi–Saudi, Russian strikes on Kyiv, Canada trade war, UK–Israel, Nepal, Indonesia volcano); the 2.5D map renders a full world of columns, 0 console errors. Also decoded HTML entities in RSS titles (`&#8217;` etc.). Backend deployed + live; frontend still source-only. Tests: +buildStorySituation, 11 gdacs + 7 tracker pass.

## 2026-09-08 (S3·T1: newsSituationIngest — RSS → flash classify → stories; LIVE hourly)

The news producer. New Lambda `newsSituationIngest` (`TriggerSituationIngest` `rate(1 hour)`, role `newsSituationIngest-role`, provider env copied from newsInvokeGemini): fetches ~14 RSS feeds → classifies each headline on `deepseek-v4-flash` (batched 35, **parallel concurrency 4**, strict JSON via `response_format` + **`thinking:{type:'disabled'}`**, raw fetch — no `openai` dep) → deterministically clusters into **stories** (axis, model-provided centroid, max_severity, distinct-outlet count, velocity, spread, headlines) → writes `corpus/YYYY/MM/DD/HH.jsonl` (all classified) + `stories/state/<id>.json` + `stories/index.json` (only stories with ≥2 outlets or severity ≥3, top 80). `classifier-core.js` pure (7 tests). Live-verified: 295 articles → 158 raw clusters, top stories correctly typed across all four axes (UK–Israel political sev4/5o, Houthi–Saudi conflict sev5, Nepal humanitarian sev4, Canada trade economic sev4). `ClassifierLLMCallsToday` metric; `stories/` lifecycle IA@15d expire@45d. **Debugging fixes:** parallelized classification (sequential blew the 300s timeout) and disabled async retries (they re-ran the whole LLM batch 3×); root-caused the initial empty output to V4 thinking-mode burning max_tokens (finish_reason=length). Next: S3·T3 wires these stories into situations → the map's conflict/political/economic hues.

## 2026-09-08 (S5·T1: deck.gl 2.5D map — columns by severity, browser-verified; frontend source only)

The designed 2.5D experience. New `components/SituationMap3D.jsx` (deck.gl v9 `ColumnLayer` over a `GeoJsonLayer` base, tilted `MapView` pitch 54°): each situation is an extruded **column whose height = tier** (severity reads instantly), hue = crisis axis, escalating gets a halo; hover tooltip + click-select via deck.gl picking. Installed `@deck.gl/core|layers|react@9`; **code-split** via `React.lazy` so the 907KB deck.gl chunk loads only on `/map` (main bundle unchanged). `SituationHome` picks 3D by default, falling back to the D3 `SituationMap` under `prefers-reduced-motion` or no-WebGL. Browser-verified headless (SwiftShader): canvas renders with a WebGL context, the China-flood column is clearly visible, 0 console errors. **Deferred to S5·T2:** ripple animation, spread arcs (need affected-country centroids), idle tour, history scrubber, globe fly-to. **Deferred to S6:** retire the 4 z-score tests (WorldMapV2 still lives at `/map-legacy`). Frontend source only — not deployed.

## 2026-09-08 (S4.5: legibility & honesty pass — browser-verified; backend deployed, frontend source only)

Rebuilt the map's legibility per the §12 review — a stranger can now tell what the map watches, why it's sparse, and what each dot is. **T1:** legend as a coverage statement (Conflict/Political/Economic/Humanitarian with live counts, inactive dimmed "none") + a coverage note while the news layer is absent; the tracker's `deriveLede` now writes a real briefing sentence ("China — flood — an elevated humanitarian alert. It is the only situation currently being tracked."). **T2:** `fmtIn` fixes the future-time bug ("next in 26 min", was "next ~just now") and the app's static "Updated hourly" strip is hidden on `/map` so there's one truthful freshness claim. **T3:** rich plain-language detail card — GDACS description + severity + affected country **names** (not `CHN`) + timeline + report link first + plain state labels (New/Getting worse/Ongoing/Easing/Ended) + honest "Deterministic alert from UN/EU GDACS — no AI analysis is generated at this level"; the GDACS observation now carries `description` and `affected_names`. **T4:** map cropped to ~±60° (Antarctica gone), hover tooltip, zoom/pan, stronger severity radius + glow on elevated. Browser-verified headless (legend, coverage, freshness, tooltip, rich card all correct; **0 console errors**). Backend (gdacs + tracker) deployed + live; frontend is source-only (not deployed — `/map` prod unchanged). 7 tracker + 11 gdacs tests pass.

## 2026-09-08 (plan: S4.5 legibility & honesty pass added after first browser review of /map)

Reviewed the S4 screenshot critically: it proves the pipeline but not the designed experience. Recorded in `MAP_HOME_SITUATION_PLAN.md` §12 (8 findings — no legend/coverage statement, fragment lede, emptiness reads as failure, weak severity encoding, `next ~just now` bug + contradiction with the app header's "Updated hourly", jargon states, thin negatively-framed detail card, no interactivity) and added **Stage S4.5** to §11 + 4 ledger rows (legend-as-coverage + briefing lede + coverage note; single truthful freshness claim; rich plain-language detail card with richer GDACS observation; crop ±60°/tooltip/zoom/stronger severity scale). **Rule recorded: no prod deploy of `/map` before S4.5.** Docs only.
...
## 2026-09-08 (S2·T1: newsSituationTracker built — folds inbox → world/latest.json; DRY_RUN shadow running)

New Lambda `newsSituationTracker` (Node 22, 512MB, `TriggerSituationTracker` `rate(10 minutes)`, role `newsSituationTracker-role` least-privilege S3 on situations/*, world/*, shadow/*, stories/* + ListBucket-scoped + PutMetricData). The **folder** (DATA_STRATEGY): each sweep reads the latest `situations/inbox` observation snapshot, folds via the shared `situations-core.js` (`buildSituation`/`coolSituation`) into `situations/state/<id>.json` + `situations/index.json` + `situations/history/…`, and assembles `world/latest.json` (+ member + timestamped snapshot) with situations, freshness (`stale`/`sources`, computed here), deterministic `lede`+`ranked` from situations, and empty `systemic` (markets wiring later). Situations cool when absent from the snapshot, close after 3 cool sweeps (kept 48h for grey-out). **NO LLM.** 7 unit tests (fold open/raise/spread/cool/close/drop, assemble, stale). Running `DRY_RUN=true` → writes a parallel `world/shadow/` universe and does not consume the inbox; live invoke verified the shadow bundle (China flood, tier elevated, lede/ranked/sources correct). `situations-core.js` is shared into the tracker dir (documented byte-identical copy until a Lambda layer). Flip to live is S2·T2 after ~1 week of shadow. Docs: ARCHITECTURE (Lambda + schedule + S3 writers).

## 2026-09-08 (S1·T1: GDACS → S3 inbox observations; DDB Situations table dropped)

Re-pointed `newsGdacsIngest` from writing DDB situation rows (P1·T1, superseded) to **appending observations to S3**, per DATA_STRATEGY (openers append, the tracker folds/owns state). New `amplify/backend/function/newsGdacsIngest/src/situations-core.js` — pure shared module: `buildObservation(feature)` + the fold `buildSituation`/`coolSituation` (now take observations, not raw features) ready for the S2 tracker; 11 unit tests (`situations-core.test.mjs`). GDACS now, each 20-min run: mirrors events to `GlobalPerspectiveGdacsEvents` (unchanged) + writes ONE snapshot `situations/inbox/<ts>-gdacs.json` listing all current Orange/Red observations (level, score, iso3, centroid, verb_label) — NO LLM, holds no state. Deployed + live-verified: snapshot written (1 Orange observation, China flood, correct centroid). Removed the DDB Situations grant from the gdacs role; confirmed zero code/role readers; **dropped `GlobalPerspectiveSituations`**. Docs: ARCHITECTURE (table → deleted note, Lambda + IAM rows), BACKEND_GUIDE.

## 2026-09-08 (S0·T3 complete: /data/* Worker deployed live — browser→S3 read path is up)

Deployed the `/data/*` route to the production Cloudflare Worker `globalperspective-rss` (via authenticated wrangler, after the user allowed `Bash(wrangler:*)` and authorized). Proven in a throwaway workers.dev worker first, then live on globalperspective.net: `/data/world/latest.json`→200 (`x-rendered-by: cf-worker-data`, correct fixture), missing→404, member→401, disallowed prefix→404 — and regression-clean (`/rss`→200 XML, bot pre-render→`cf-worker-bot`, passthrough→200). Deployed matching the existing compat date (2026-04-01); zone routes preserved. Probe testing caught one bug, fixed before prod: a missing key returned 502 because the reader IAM user has GetObject but not ListBucket (S3 answers 403, not 404, for a missing object) — the Worker now maps 403→404. Secrets set via stdin; reader user holds exactly one key (the prod secret), all test/probe keys deleted, temp files scrubbed. The browser→S3 read path is now live; nothing consumes it in the UI until S4. Minor: preview URLs got enabled on the prod worker (serves the same public bundle — optional cleanup later).

## 2026-09-08 (S0·T3 partial: Worker /data/* route + fixture + reader IAM — read path proven; Cloudflare deploy pending operator)

Built the browser→S3 read path for the map-as-home data. Created IAM user `gp-worker-s3-reader` (read-only GetObject on `world/*`, `situations/state/*`, `stories/state/*`). Wrote a self-contained SigV4 GET (Web Crypto) and the `/data/*` route into `project-docs/distribution/WORKER_FULL_CODE.md` — **proven against real S3 from Node** (200, correct fixture) before commit; the test access key was deleted after verifying so no live secret entered the repo or transcript. Route whitelists allowed keys and is fail-closed for `*.member.json` (Firebase JWKS check is a documented stub). Authored `frontend/fixtures/world.json` (the `world/latest.json` contract sample) and uploaded it to `s3://…/world/latest.json` so the path serves real data until the tracker (S2) takes over; added `fixtures/README.md` documenting the `VITE_WORLD_URL` dev convention. Docs: `ARCHITECTURE.md` (S3 read path + reader user), memory `project_cloudflare_worker`. **Pending operator (Cloudflare access needed):** mint a fresh `gp-worker-s3-reader` key, set the two Worker secrets, deploy the Worker; then `curl https://globalperspective.net/data/world/latest.json` verifies live. No frontend build.

## 2026-09-08 (S0·T2: S3 world bucket created — private, lifecycle, GDACS inbox IAM grant)

First S3 in the stack. Created `globalperspective-world-280362093938` (ap-northeast-1) as the world source-of-truth per `DATA_STRATEGY.md`: all four public-access-block flags on, no bucket policy (private), default SSE-S3. 5 lifecycle rules (corpus/world → IA 30d + expire 400d; situations/history → IA 30d; processed inbox expire 30d; abort incomplete MPUs 7d — `world/latest*.json` is overwritten each sweep so it never ages to expiry). Added `s3:PutObject` on `situations/inbox/*`+`gdacs/*` to `newsGdacsIngest-role` (its temporary DDB Situations grant stays until S1). Verified: public-access-block all true, not public, lifecycle present, write/read/delete round-trip OK. Documented in `ARCHITECTURE.md` (new S3 World Store section). Next: S0·T3 (Worker `/data/*` route) needs a Cloudflare-side credential — see ledger. No frontend build.

## 2026-09-08 (architecture: DATA_STRATEGY adopted — S3 for the world, DynamoDB for the user; plan v2 stages)

New `project-docs/architecture/DATA_STRATEGY.md`: everything the pipeline computes about the world becomes versioned JSON in a private S3 bucket (read by the browser through the Cloudflare Worker, JWT-gated member bundle); DynamoDB is kept only for per-user state (`Users`, `SavedItems`, `UserPrefs`, `ApiKeys`). Rules: one writer per prefix; openers append `situations/inbox/` events and the tracker is the sole writer of state and of the frontend bundle `world/latest.json` (freshness is data, not inference); never list on a hot path; lifecycle replaces TTL; local dev on fixtures. Existing tables migrate incrementally (prediction log first), never big-bang. `MAP_HOME_SITUATION_PLAN.md` revised to v2 ADR-style (dated banner, new §3.2, REVISED notes on WS1–WS3, §5 superseded by new §11 stages S0–S8, §6/§9 amended); `MAP_HOME_SITUATION_LEDGER.md` restructured around the stages. **P1·T1's `GlobalPerspectiveSituations` DDB table is marked superseded — to be dropped in Stage S1** before anything reads it (its geometry capture, 20-min cadence, state-machine helpers and tests survive). Docs only — no build, no AWS change.

## 2026-09-08 (P1·T1: GDACS un-shadowed → situation opener; Situations table created)

First build step of the map-as-home programme. `amplify/backend/function/newsGdacsIngest/src/index.js` rewritten (was shadow-only, byte-identical to main): now (1) keeps feature **geometry** → `lat`/`lon` on every event row (100/100 live events now geo-located, was 0), and (2) is the **deterministic situation opener** — upserts Orange/Red GDACS events into the new `GlobalPerspectiveSituations` table with tier (Red→high, Orange→elevated; no "critical"), state machine (emerging/escalating/peak/cooling), affected countries, centroid, spread arcs, templated `what_changed`, and history — **with NO LLM** (§3.1 LLM boundary). Green cools an open row; the tracker (P2) will close it. Idempotent (re-run = `unchanged`, no duplicate). AWS DONE: created `GlobalPerspectiveSituations` (PK situationId, GSIs `state-next_check_at-index` + `all-updated_at-index`, TTL 60d); granted the gdacs role PutItem/Query on it (merged into existing inline policy); deployed the code; changed `TriggerGdacsIngest` `cron(0 */6…)` → `rate(20 minutes)`. Live-verified: 1 Orange event opened 1 situation, clean label "China — flood". 9 unit tests (`index.test.mjs`) pass. Docs: `ARCHITECTURE.md` (Situations Table schema, Lambda + schedule rows), `BACKEND_GUIDE.md` (DDB table), `IMPACT_FIRST_REDESIGN_PLAN.md` (un-shadow note).

## 2026-09-08 (plan: LLM boundary + blast radius written into MAP_HOME_SITUATION_PLAN)

Second design pass before P1. New §3.1 "The LLM boundary": disasters flow GDACS → situation → map with **zero model calls**; tracker `what_changed` is template-first, LLM only on material news change; classifier runs **hourly** (decoupled from the 4h editorial run); thread-vs-situation resolved as Option A (deterministic card, default) / Option B (auto-thread for Red alerts only); one-event-one-pin merge rule; no situation-level predictions in v1; conservative escalation thresholds tuned during a ~1-week DRY_RUN shadow; two daily LLM caps (tracker 300, classifier 600) as metrics; `GlobalPerspectiveSituations` created first and GDACS writes it directly (no tracker dependency). New §10 "Blast radius" verified against the link graph: what is replaced (`WorldMapV2`, Home re-hosted, 4 z-score tests), touched (`Layout.jsx:67` nav, `BreakingDetailPage.jsx:106`, `smoke-test.mjs:106/423`, restProxy, countryMapping, errorSink, tokens.js), deleted (legacy WorldMap/MapSidePanel/MiniMap), and unchanged (all other routes, `docs/config.js`, every topic consumer — topics shape is unchanged). Ledger rows P1·T1/T2/T3, P2·T1/T2, P4·T1, P5·T1/T2 updated to match. Docs only.

## 2026-09-08 (P0·T1: Brave audit — remove Brave from ingest, keep for grounding)

Read-only CloudWatch/DDB investigation (`scripts/brave-audit.md`). Over 30 days, Brave in ingest is ~5.8% of the article pool, ~18% HTTP-429 rate-limited (all failures were 429; `reuters.com` returns ~0.38 articles/run), and — the decisive number from the capture table (197 runs, 2,740 chosen topics) — only **0.4% of chosen topics were sourced solely from Brave** (`brave_unique_chosen`). Decision: **remove Brave from ingest** (GDELT DOC 2.0 replaces it with no meaningful loss; no transitional fallback), unblocking P1·T4. Grounding (5 Lambdas) stays on Brave — true volume isn't log-derivable (they log Brave only on failure) and needs the operator's Brave dashboard, so parts C/D/E are deferred to a separate provider follow-up. Filled plan §2.6 + §5.1 decision table. Also confirmed `TOPICS_LIMIT=13`, `BRAVE_CONCURRENCY=1` (source default 3). Docs/investigation only — no code, no deploy.

## 2026-09-08 (process: plan-execution playbook + ledger; begin executing map-as-home plan)

Started executing `MAP_HOME_SITUATION_PLAN.md`. Added `project-docs/playbooks/PLAN_EXECUTION_PLAYBOOK.md` — a declare→do→update loop for a multi-phase plan worked task-by-task across sessions/agents (extends `TASK_WORKFLOW.md` with a durable per-plan execution ledger). Moved the plan `_proposed/`→`_active/` and created `MAP_HOME_SITUATION_LEDGER.md` declaring every P0–P6 task with its reads/changes/docs-to-update/exit-criterion up front (per the playbook: no task starts with a blank row; docs update in the same commit as code; done means docs updated too). Indexed both in `project-docs/INDEX.md`. Docs/process only — no code, no deploy. P0 (read-only measurement) in progress.

## 2026-09-08 (plan: map-as-home + situation tracker — hand-off spec, no code)

Added `project-docs/redesign-ux/_proposed/MAP_HOME_SITUATION_PLAN.md` after a deep read of `/map` (`WorldMapV2.jsx`) and the ingest path (`newsInvokeGemini`). Verified findings recorded in the plan: the map colours countries by a z-score of our own mention counts; Palestine/Gaza/Kosovo silently match nothing (map keeps its own name→ISO tables, diverged from `utils/countryMapping.js`); topology is fetched from an unpinned CDN with no fallback; GDACS/GDELT ingests are shadow-only and GDACS drops coordinates; the topic selector's category quotas + "prioritize NEW" rule suppress ongoing crises. Plan: per-article structured classification + deterministic clustering (RSS + GDELT DOC 2.0, Brave removed from ingest only), GDACS promoted to a 20-min deterministic opener, new `newsSituationTracker` with adaptive cadence, WebGL 2.5D map (hue = crisis type, height = severity, ripple = escalating, luminance = age), `/` → map. P0 includes a read-only Brave investigation protocol (Logs Insights queries, capture-table yield script, derived grounding volume — the 5 grounding Lambdas log Brave only on failure — Brave-dashboard reconciliation, blind Exa trial, decision table). Indexed in `project-docs/INDEX.md`. Docs only — no build, no deploy.
...
- **Also** added `thinking:{type:'disabled'}` to the repo source. ⚠️ **Correction (found via a deployed-vs-repo diff):** the **deployed** function already had this guard (added 2026-07-26, deployed-only — it never made it back into the repo source), so there was **no live truncation bug**. The fix **reconciles the repo to deployed reality**, preventing a future deploy-from-repo from silently *removing* the guard.
...
- **No deploy warranted.** A deployed-vs-repo diff on `newsImpactAudit` showed the deployed code already carries the runtime-important guard, and every function's live `GROK_MODEL` env is already correct — so the source-default sweep is **inert at runtime** (pure landmine defusal + repo/deployed reconciliation). Deploying from repo would give **zero behavior change and risk regressing deployed-only drift** (the very drift this diff exposed). These land as source hygiene; each function updates on its own next real deploy (diff-reconciled then). Noted-not-changed: `newsAnalyze` + `newsModelGuard` lack the literal `thinking:disabled` string (`newsModelGuard` probes models by design; `newsAnalyze` applies it via a helper/patched zip).
...
- **`signal-api` merged (`59054ec`).** Closes the "Prod infra NOT on `main`" gap: `newsSignals`, `newsImpactAudit`, `newsGdacsIngest`, `newsGdeltConflict` have been deployed and ENABLED in prod since June with source on this branch only. **Merged ≠ reconciled** — the deployed bytes drifted independently for ~5 weeks and remain the authority; diff against live AWS before changing any of the four. Two conflicts, both in `newsBreakingAlert`, resolved as a **union** because the auto-merged `significance.js` genuinely contains both reworks: the branch's `RISK_CAP=50` + risk-weight 2.0→1.0, and `main`'s `axisForCategory`/`regionRiskScore` category-axis dominance fix. `main`'s test assertions had been written against *uncapped* risk and were factually wrong post-merge, so they were replaced with the branch's behavioural assertions while keeping `main`'s pure axis tests (unaffected by the cap) — 50/50 pass.
- **V4 guard verified through the merge.** `newsInvokeGemini` was the highest-risk file: `main` carries the `thinking:{type:'disabled'}` patch that prevents a repeat of the 2026-07-24 retirement outage, and `signal-api` (which predates it) adds a `captureIngestion` harness to the same file. Both are present post-merge, in different regions — checked rather than assumed.
- **P6 member-perk surfacing** cherry-picked from `feat/perk-surfacing` and deployed (`80945bd`). Cherry-picked rather than merged because the branch carried a 134-line `SCORING_MODEL_V2_PLAN.md` that `main` has since superseded (200 lines, Phases A–D shipped) — a straight merge would have regressed it. **P6a is a real bug fix:** Home's trust strip counted corrections with `notes.length`, but `corrections_feed` caps that list at 5 for non-members, so anonymous visitors saw "5 revised conclusions logged" instead of the true total. Now reads the server's honest `total` — the count is public even though ledger depth is member-gated. P6b adds a country-change-alert CTA to the weekly email footer; P6c reframes the Studio upsell as allowance + history + alerts rather than compute-only.
- **Salvage checked, not assumed:** the untracked `SIGNAL_API_PLAN.md` in the `product-improvements` worktree turned out to be an *older* draft ("No code yet") than the version tracked on `signal-api` ("Step 1 BUILT", plus §10 Infra & deploy and §11 What's NOT done). Discarded the draft; the merge delivered the better one.
- **Cleanup:** 20 worktrees removed, 22 branches deleted (17 fully-merged via `-d`; `weekly-ui-redesign`, `email-sender`, both `feat/*` and `feature/multi-language` via `-D` after verifying content is on `main` or superseded). `feature/multi-language` predates the 2026-05-26 `graphqlService.js`→`contentService.js` rename and would conflict heavily — deleted locally, with `origin/feature/multi-language` retained as the backup. Only `main` + the `signal-api` worktree remain. Six `origin/dependabot/*` branches are untouched (open dependency PRs).
- Files: `ARCHITECTURE.md`, `CHANGES.md`, plus everything landed by the merge and cherry-picks.
...
- **Root cause / fix:** `deepseek-chat` is gone; the current IDs are `deepseek-v4-flash`/`-pro`, both of which **default to thinking-mode** — which burns `max_tokens` on invisible `reasoning_content` and truncates/empties output (proven live: v4-flash without the fix clustered only 1 topic at `finish_reason=length`, even with `response_format: json_object`). So the fix is a **code change** (`thinking:{type:'disabled'}` in the request body), not just an env flip — exactly as `BACKEND_DEEPSEEK_V4_MIGRATION_PLAN.md` warned after the 06-12 config-only attempt regressed.
- **Migration executed 2026-07-26 (patched-deployed-zip, code-before-env so no broken window).** Added `thinking:{type:'disabled'}` to each function's DeepSeek call, then flipped `GROK_MODEL` on an **intent-based model split** (why: `pro` buys deeper reasoning only where the LLM does open analytical judgment users read directly; `flash` for mechanical/constrained/judge-protected/high-volume work):
  - **`deepseek-v4-pro`:** `newsCountryIntelligence`, `newsSystemsAnalysis`, `newsEconomicImpact` (each also code-patched + redeployed).
  - **`deepseek-v4-flash`:** `newsInvokeGemini-dev`, `NewsProjectInvokeAgentLambda-dev`, `newsDriftCorrector`, `newsPredictionResolver`, `newsWeeklyBrief`, `newsWeeklyMarkets` (DeepSeek producer call only; Gemini judge / Perplexity untouched), `newsPairIntelligence`, `newsImpactAudit`.
  - Pipeline verified live end-to-end: `newsInvokeGemini` → 13 topics, `NewsProjectInvokeAgentLambda` → 39 AI items, swap to `latest`; `newsWeeklyBrief` republished (`weekOf 2026-07-25`). Site `updatedAt` fresh; stale alerts stopped.
- **PPA project (2026-07-27, separate authorization):** `PPAcomputeWardProfiles-dev` (3 DeepSeek calls) + `PPAfetchMarketNews-dev` (1) were on the same dead alias — same patch + `LLM_MODEL→deepseek-v4-flash`. `PPAfetchMarketNews` smoke-tested green (`ok:true`, 25 candidates, 5 written, `tagged:"llm"`).
- **Repo port (this commit + `8f6c82f`):** the `thinking:{type:'disabled'}` one-liner is now in the **repo `src/index.js`** of all 10 on-main functions, so a future zip-from-repo deploy can't regress it (the live fix had been deployed-zip-only). `newsImpactAudit` source is signal-api-branch-only — deployed-zip patch stands there.
- **Doc staleness audit (earlier, `d27c84e`):** a 5-auditor parallel sweep of `ARCHITECTURE.md` vs live AWS (`AGENT_REVIEW_METHOD.md`) corrected: `newsBreakingAlert` is deployed+scheduled (`DRY_RUN=false`), not "NOT DEPLOYED"; 4/7 EventBridge Scheduler entries run `Asia/Tokyo` (not "All UTC"); `newsPairIntelligence` is on the ENABLED weekly rule `TriggerPairIntelligenceWeekly` (not manual-only); the Common Mistakes #7 CORS list (only `newsPolarBilling`+`newsAnalyze` own CORS in code — `newsSavedItems`/`newsRecommend` rely on populated Function-URL CORS; also fixed in `CLAUDE.md` + memory); documented the live Signal-API/ingest subsystem (`newsSignals`/`newsImpactAudit`/`newsGdacsIngest`/`newsGdeltConflict` + 6 DDB tables) from the unmerged `signal-api` branch; stale counts.
- Files: `amplify/backend/function/{newsInvokeGemini,NewsProjectInvokeAgentLambda,newsCountryIntelligence,newsSystemsAnalysis,newsEconomicImpact,newsDriftCorrector,newsPredictionResolver,newsWeeklyBrief,newsWeeklyMarkets,newsPairIntelligence}/src/index.js`, `ARCHITECTURE.md`, `BACKEND_DEEPSEEK_V4_MIGRATION_PLAN.md`, `CHANGES.md`. Deployed-zip vs repo drift beyond the model patch (e.g. `newsInvokeGemini`'s signal-api capture harness) remains unreconciled — flagged, out of scope.
...
- **Visual-block reliability (P0, `utils/analysisPrompt.js`):** the member-path model (DeepSeek V4 Pro) emitted the `gp-struct` block — the machine-readable index that drives ScenarioBars/IndicatorMatrix/RippleTable — in only **1 of 4** live runs, so the 07-10 visuals shipped mostly invisible on the paid path. Fixed by embedding a **fully-filled JSON template** in the SYSTEM_PROMPT gp-struct instruction (copy-the-shape instead of abstract-shape), **removing the whole-block-omit exit** for the scenario/economic lenses (individual empty arrays may still be dropped), and adding a **mandatory tail reminder** at the point each lens task finishes writing. Honesty guard unchanged — `validateStruct` still strips any number/name not present in the prose, so pushing emission cannot induce fabrication. Live-verified (`scratchpad/emit-test.mjs`): valid-block emission went **1/4 → 8/8** (6 scenario + 2 economic stories), zero fabrication.
- **Alibaba Qwen provider (`services/llm.js`):** added `qwen` to `PROVIDERS` (flagship `qwen3.7-max`, plus `qwen3-max`/`qwen3.7-plus`), OpenAI-compatible via Alibaba Model Studio/DashScope's international endpoint, `enable_thinking: false` forced (same truncation failure mode as DeepSeek V4's `thinking`). Alibaba's **workspace-scoped keys** (`sk-ws-…`) 401 against the generic host and need a workspace-specific host (`https://<WorkspaceId>.ap-northeast-1.maas.aliyuncs.com/compatible-mode/v1`, verified live) — added an optional **per-provider base-URL override** (`allowBaseUrlOverride`, `runChat`/`runOpenAICompat` accept `baseUrl`, `byok.js` persists it back-compat, `ProviderModal.jsx` renders an optional "Endpoint URL" field, `AnalysisStudio.jsx` passes it through) so BYOK users with workspace keys aren't stuck with a silent 401. Blank/default → unchanged behavior for every other provider.
- Files: `utils/analysisPrompt.js`, `services/llm.js`, `utils/byok.js`, `components/ProviderModal.jsx`, `components/AnalysisStudio.jsx`. Built + verified by Sonnet; deploy (frontend build + `newsAnalyze` prompt resync) pending operator review.
...
- **`atoms/RiskScorecard.jsx` (+ `.css`) — the flagship.** A Lighthouse-style card: worst-axis headline (e.g. "ELEVATED · Economic 72"), one bar per dimension colored by its severity band, the per-axis "why" grounding, a "lead" tag on the driving axis, and a breadth chip when ≥3 axes are elevated. A `null` axis renders "no signal" (italic, no bar) — sparsity is visible, never a phantom 0. Renders **nothing** for scalar-only (pre-v2) records, so old countries are unaffected. Reads `deriveHeadline(record)` — the same adapter that back-fills from the legacy scalar.
- **Placed on:** CountryPage (between the stats strip and "What changed") and ThreadPage (Arc Intelligence rail). `/weekly` LEAD + DEVELOPING rows now show the lead axis beside the tier ("HIGH · Conflict"). SpiderWorld tooltips gained the lead axis + a compact Conf/Pol/Econ/Hum breakdown.
- **Backend:** `newsSensitiveData-dev` `world_overview` enrichment now carries `dimensions` + `lead` onto each situation (from `COUNTRY_INTELLIGENCE`) so the map can show the axis. Redeployed; single ACAO header verified (no dual-CORS regression). The `country_intelligence` serve is already a pass-through (`{PK,SK,ttl,...rest}`), so CountryPage got the vector with no serve change.
- **Browser-verified before *and* after deploy (Playwright vs live API):** Ukraine → "HIGH · Conflict 82", 4/4 breadth chip, LEAD on Conflict; Japan → "ELEVATED · Economic 72", LEAD on Economic, **Humanitarian = "no signal"** (the sparsity case), 0 JS console errors. Confirmed again on the live `globalperspective.net` bundle (`index-PghJx9sT.js`); the only console entries are the by-design stripped-source-map 404s.
- **Deploy gotcha (recorded):** touching CountryPage/ThreadPage/WeeklyPage trips the `.githooks/pre-push` economic verifier (L2 Producer / L3 Judge), which `Cannot find module '@aws-sdk/client-dynamodb'` in a fresh worktree. Fix = clone each economic function's `src/node_modules` into the worktree (APFS `cp -Rc`) so the tests load → all 4 layers green. `git push --no-verify` is denied by the auto-mode classifier, so fixing the env (not bypassing) is the path.
- **Partial:** thread-side surfaces (ThreadPage card, /weekly lead labels) fill in as `newsThreadAnalysis` regenerates — it keeps hitting **Gemini rate limits** (~1 thread/run), so thread vectors accrue slowly over crons. Country data is complete now.
- **Not built:** Phase D (per-axis drift in `newsDriftCorrector`; category-aware breaking scorer). verify green (207 tests) + build clean throughout.

## 2026-07-07 (feat: scoring model v2 — multi-dimensional risk, Phase A+B DEPLOYED to prod)

Began `SCORING_MODEL_V2_PLAN.md` — decompose the single `riskScore` (0-100) into a **4-axis `dimensions` vector** (conflict / political / economic / humanitarian, each `{score, why}` or `null`). The headline is the **worst axis + its lead label**, never a weighted average, so a country reads "HIGH — Economic 75" instead of a blended number that hides *which* dimension is driving it. Branch `scoring-model-v2` (off `origin/main`, **not yet merged** — main's generator source is now stale vs prod). Additive + back-compat: the derived legacy scalar is written alongside the vector, so every existing consumer keeps working untouched. Gated by a **Phase 0 prompt spike** (8 real countries) that confirmed the axes differentiate, sparsity holds, and grounding is specific *before* any code shipped.

- **Phase A — frontend seam (commit `23c372c`, invisible).** `utils/riskTiers.js` gains `AXES`/`AXIS_LABELS`, `headlineFromDimensions()` (worst axis + tier + lead + breadth flag + per-axis breakdown; accepts a bare number OR `{score,why}`), and `deriveHeadline(record)` — **one call for any record shape**: prefers the vector, falls back to the legacy `riskScore` then `riskLevel`, so pre-v2 rows still render. 10 new tests (Japan headline-flip, breadth, sparsity, back-compat). Escape-hatch routing deferred to Phase C (they keep working via the derived `riskLevel`). Nothing renders differently yet.
- **Phase B — generators (commit `c7a18f4`, DEPLOYED prod 2026-07-07).** `newsCountryIntelligence` + `newsThreadAnalysis` now ask the LLM for the `dimensions` vector (per-axis calibration + one-sentence grounding + explicit sparsity — "use `null`, not a filler mid-number") and **derive `riskScore`/`riskLevel` from the worst axis** via a shared `riskDimensions.js` helper (byte-identical in both functions; mirrors the frontend adapter). Both core + snapshot (`HISTORY#`/`THREAD_HISTORY#`) writes carry `dimensions` + `lead` beside the derived scalar. Transitional fallback: if a response omits `dimensions`, fall back to a legacy `riskScore` so a rollout never writes null.
  - **Bug caught pre-deploy:** `clampScore(null)` returned `0` (`Number(null)===0`), which would have silently filled every empty axis and killed sparsity. Fixed — `null`/`''` → `null`, explicit `0` preserved; frontend `axisScore` hardened to match.
- **Deploy (prod, 2026-07-07):** `update-function-code` on both Lambdas (no env change; source-only zips incl. `riskDimensions.js`, runtime `@aws-sdk`). Triggered a real `newsCountryIntelligence` run: **18 generated / 0 failed** — real prod vectors confirm the design: **Palestine → HIGH 90 lead humanitarian** (economic = `null`), **Japan → ELEVATED 72 lead economic** (humanitarian = `null`), Ukraine → HIGH 82 lead conflict; sparsity + specific grounding present. `newsThreadAnalysis` run hit **Gemini rate limits** (8 failed on 429 quota, not code) — thread dimensions populate on the next scheduled cron.
- **NOT built:** Phase C (visible scorecard UI — CountryPage/ThreadPage headline, `/weekly` LEAD/DEVELOPING, SpiderWorld bubbles, per-axis drift) is gated on inspecting prod vectors at scale first. Phase D (per-axis drift in `newsDriftCorrector`; category-aware breaking scorer; weekly-brief single derivation). Verify green throughout (203 tests, build ✓); all 3 backend files `node --check` clean.

## 2026-07-07 (feat: prediction capture gate G6 — scope out pure sporting results; first resolution run)

Two things landed after the first live prediction-resolution run (16 of 21 due triggers scored → `/track-record` now shows Brier 0.084, calibration buckets populated; verdicts written by the agent-run loop with a 2nd blind pass on every `fired` + operator spot-check).
- **New capture gate G6 (`NewsProjectInvokeAgentLambda/src/lib.js`, DEPLOYED to `NewsProjectInvokeAgentLambda-dev`):** drops pure sporting-result triggers at capture. A match/tournament outcome or on-field participation carries no geopolitical/economic/institutional signal (the product's remit). Requires BOTH a sporting-competition **context** (world cup · match · round of N · tournament · …) AND a **result/participation** verb (wins · starts · scores · advances · …), so sport-**adjacent** governance triggers survive — "Belgium files a protest with FIFA", "CAS grants an interim measure", "FIFA lifts the ban" are institutional events (no result verb) and pass. Triggered by the Trump–FIFA/Balogun story: its box-score triggers ("U.S. wins its round of 16 match", "Balogun starts") drifted into the log; the story stayed, only the sporting results are now gated. **Corpus-validated: across all 1,038 live v1 triggers G6 drops exactly those 2, zero false positives** (no political "wins/advances", no economic "scores a gain"). `node --test` 21/21 (5 new G6 cases incl. the real run triggers + a political-false-positive guard). Not retroactive — already-published sport triggers remain scored.
- **Methodology disclosure (`TrackRecordPage.jsx`):** the published methodology now states capture requires a real geopolitical/economic/institutional signal, with a dated note "from 2026-07-07 we also gate out pure sporting results; earlier ones already published remain scored."

## 2026-07-07 (feat: member auto-fix gating LIVE — depth-cap + permanent archive + follow-country change-alerts + membership copy)

Shipped `MEMBER_GATING_PLAN.md` end-to-end (merged to `main` `f3e0f4d`; branch `feat/autofix-gating`). The differentiator is the **self-correcting analysis** (living-analysis loop), which was 100% free AND invisible. Site-orientation made it visible; this makes membership buy its **depth + alerts** — while every analysis page + the *latest* correction + the whole track record stay free (data-cap, never a hard block, so anon always gets a 200 and the 2026-04-22 anon-guard regression can't recur). Verify green throughout (0 errors, 192/192, build ✓).

- **P1 — depth-cap (`newsSensitiveData` → deployed `newsSensitiveData-dev`).** New `resolveTier(event)` (best-effort Firebase-JWT → `GlobalPerspectiveUserTable.tier`; never throws/blocks) + pure `capForTier` in new `src/lib.js` (`node --test`). `country_history` caps `driftNotes` to the newest 1 for non-members + returns `driftNotesTotal`/`driftNotesGated`; `corrections_feed` caps to newest 5 + `total`/`gated`. Members get the full chain. Snapshots (the analysis itself) stay fully public. **Prod-verified:** anon Ukraine → 1 note shown / 6 total / `gated:true` / 51 snapshots uncapped; site-wide ledger → 5 / 26 / `gated:true`.
- **P2 — lock affordances (frontend).** `CountryWhatChanged` → "🔒 N earlier corrections — Join to see the full history →/membership"; `/track-record` ledger → "Showing n of N — Members see the full history →". UI reads the server's `gated`/`total` (no client membership check); real counts, no fake blur.
- **P3 — permanent archive (`newsDriftCorrector` + serve).** The corrector now writes each note **twice** (idempotent, country + thread) via `putDriftNote`: `DRIFT#<date>` (60d TTL, live band + ledger) + **`DRIFTLOG#<date>` (NO TTL)** — so "full history" outlives the 60-day window. `country_history` unions archive+live via pure `dedupeByAsOf` (lib.js, tests 9/9); empty archive ⇒ no-op (band unaffected). `begins_with('DRIFT#')` excludes `DRIFTLOG#` → no ledger double-count. Archive begins accruing at the corrector's next 07:20 UTC run.
- **P5 — follow-country change-alerts (the member perk).** `newsRecommend`: `followedCountries` (DynamoDB String Set, atomic ADD/DELETE) + `driftOptIn` on `GlobalPerspectiveUserPrefs`; new `follow_country`/`unfollow_country` actions — signed-in **AND member-tier** (fail-closed `getTier` → `403 members_only`); `handleUnsubscribe` gains `kind='drift'`. `newsEmailSender`: new `mode:'drift_alert'` → `runDrift` scans fresh not-yet-emailed country `DRIFT#` notes (mirrors `runBreaking` `emailedAt` idempotency + freshness window), groups by country, emails each country's followers, stamps once; new `renderDriftEmail.js`. Frontend: `FollowButton` (member 🔔 toggle / non-member+anon → subtle locked `/membership` CTA; renders nothing if prefs/billing unconfigured) in the CountryPage header; `/account?tab=notifications` gains a "Country change-alerts" list (Unfollow + Pause-all).
- **P4 — membership-promise copy (6 spots).** The old "membership buys compute, **not access**" became false, so it was repositioned to a precise, softer truth ("reading stays free — every analysis + the latest correction; membership adds the **depth**: full correction history, change-alerts, Studio compute") across `MembershipPage` (hero + `BENEFITS`), `Disclosures`, `PrivacyTerms`, `TrackRecordPage`, `WhitepaperPage`, `Account`. 0 grep survivors of the old line.
- **Deploy (prod, 2026-07-07):** IAM — added `ReadTier` (`GetItem` on `GlobalPerspectiveUserTable`) to the `newsRecommend-ddb` policy (**required** or `getTier` fails closed → nobody can follow). Code → `newsRecommend` (+`USERS_DDB_TABLE` env), `newsSensitiveData-dev`, `newsEmailSender`, `newsDriftCorrector` (no function needs bundled `node_modules` — only runtime `@aws-sdk` + node `crypto`). Frontend built + `docs/` deployed (bundle `index-DRIay5Lv.js`, prod 200). **Verified:** single ACAO header (no dual-CORS regression), `follow_country`→`sign_in_required`, drift-unsub branch reached.
- **⚠️ Drift email is OFF by design.** `TriggerDriftEmailSend` EventBridge rule created **DISABLED** (`cron(40 7 * * ? *)`, target `newsEmailSender {mode:drift_alert}`). `newsEmailSender` is `DRY_RUN=false` (live), so enabling it sends real mail — do a send-to-self smoke test (set `TEST_RECIPIENT`, invoke, confirm, clear) before enabling. 0 drift subscribers today, so enabling now would send nothing anyway.
...
- **P4b — Cloudflare Worker `/` bot branch DEPLOYED (2026-07-06, via `wrangler deploy`, version `8b73c9c7`):** `WORKER_FULL_CODE.md` gained `renderRootPage()` — a **static** `/` bot branch (no Lambda call; can't go stale-wrong; 24h edge cache; `og:type website`) serving the value prop + the grouped page directory with per-page descriptions. Previously bots/AI crawlers saw the empty SPA shell at the most-linked URL. Drift-checked before deploy (live Worker untouched since 2026-04-11 = the doc's sync date, so nothing was clobbered); explicit operator yes. Live-verified: Twitterbot/GPTBot on `/` → the pre-render (`x-rendered-by: cf-worker-bot`, all directory links); normal UA → SPA; `/rss` + thread/country/daily pre-renders intact. Deploy recipe (minimal `wrangler.toml`, `workers_dev=false`, no routes → dashboard routes preserved) recorded in `SITE_ORIENTATION_PLAN.md` §P4 — the manual CF-editor paste is no longer the only path.
...
- **P1 — Home value-prop + trust strip (`Home.jsx`, `Home.css`):** masthead sub-line now states the accountability model ("AI-driven global intelligence that shows its work — every forecast **publicly scored**, every revised conclusion **logged**", both linking `/track-record`). New honest-data trust strip above `SubscribeCard`: 3 stat cards from data already fetched — dated forecast triggers (`useTrackRecord.totalDatedTriggers`), revised conclusions logged (`useCorrectionsFeed`), sources across today's stories. Each card omitted when its number is absent (honest-empty — verified: renders nothing when the proxy is unreachable). Live sample: 815 / 26 / 34.
...
- **Regression guard (`scripts/contract-check.mjs`):** added a `prediction` contract check that parses `data.content` (a JSON string) and asserts every `scenarios[].triggers[]` is a renderable shape (string or `{text,...}`). This is the drift that shipped invisibly on 2026-07-04 — a backend-only Lambda deploy reshaped an LLM-JSON payload the frontend mounts, and a `curl` `success:true` check couldn't see it. Negative-tested: accepts v1 objects + legacy strings, rejects a bare object-without-`text` / number. `BUG_PLAYBOOK.md` §4 updated.
- Verify green (lint 0 errors, 192/192 tests). Playwright-verified in the browser against prod data: Predict on Home renders the full Scenario Forecast (3 scenarios, trigger rows with deadline chips), no console/React errors. `node scripts/contract-check.mjs` passes 8/8 against live prod. Files: `components/{PredictionDisplay.jsx,SpiderDemo.jsx,SpiderDemo.css}`, `scripts/contract-check.mjs`, `BUG_PLAYBOOK.md`.
## 2026-07-06 (feat: Phase 4 — "Living forecast" board on ThreadPage)

Ships the user's original living-prediction vision: a thread now shows its dated, falsifiable forecast triggers, which render **pending today and strike through (`not_fired`) / check (`fired`) automatically as verdicts land** (Phase 2 resolution, ~2026-07-11+). DEPLOYED + verified (`d53e40a` source + `6192fcb` deploy; live bundle in ~30s).
- **Backend (`newsSensitiveData-dev`):** new `prediction_snapshot` action — given the thread's topicIds, returns the NEWEST `methodologyVersion>=1` `PRED#` snapshot among them (scenarios + dated triggers + verdicts + agent citations). v1-only (legacy retrodiction-prone snapshots never surface); parallel Query-latest per topic. Verified: real thread → scenarios/triggers; bogus topic → `null`.
- **Frontend (ThreadPage Arc Intelligence rail):** `useThreadForecast(topicIds)` hook + presentational `ThreadForecast` board at the rail top (rail now also opens when a forecast exists without full analysis). Scenarios (label + probability pill) → trigger checklist with states `☐ open · ✓ happened · ~~✗ didn't~~ · ⌛ awaiting check`. Honest-empty: renders nothing without a v1 forecast.
- **Wiring proven:** 55/145 active threads carry a v1 forecast whose `topicId` matches its `PRED#` key.
- Scope: latest-forecast board (evolution-down-thread is a later follow-on). Plan `PREDICTION_METHODOLOGY_V1_PLAN.md` Phase 4. Files: `newsSensitiveData/src/index.js`, `hooks/useThreadForecast.js`, `components/ThreadForecast.jsx`, `components/ThreadPage.{jsx,css}`, `services/restProxy.js`.
...
- **New `src/lib.js` (pure, `node --test` 16/16):** capture gates **G1** date-valid · **G2** forward-looking (kills retrodictions) · **G3** ≤180d horizon · **G4** falsifiability lint (no relative windows / precedent refs) · **G5** `FACTS#` premise-check. `buildGatedScenarios` → structured `{text,deadline}` triggers + a `capture.dropped` report + `methodologyVersion:1` stamp. Owns deadline parsing + probability math (moved out of index.js — single source, no drift). Tests use the real pilot defect triggers.
- **`index.js`:** `logPredictionSnapshot` gates every trigger through lib; the two-pass research→forecast prompts are **grounded** — verified `FACTS#` leader premises (1b, prevents false-leader triggers) + the story's own dated **arc digest** from our archive (1c) + optional Brave web context (1a, no-op without `BRAVE_SEARCH_API_KEY`), loaded once per invocation. Forecast prompt emits structured triggers with the forward-date rule stated explicitly.
- **Verified live in prod:** deployed to `NewsProjectInvokeAgentLambda-dev`, full-run verification wrote 27 `methodologyVersion:1` snapshots with gates firing (G4 caught relative-window slips, 0 retrodictions), structured forward-dated triggers. Pre-declare, integration verification caught a real bug unit tests missed — `generatedDate` is a human string ("July 4, 2026") not ISO; slicing it broke `addDays` and would have failed every prediction. Fixed to UTC-today.
- Plan `PREDICTION_METHODOLOGY_V1_PLAN.md`, worked example `PREDICTION_V1_EXAMPLE.md`. Merged to main `6a90b14`. Next: Phase 2 agent-run resolution loop (first v1 deadlines ~2026-07-11), Phase 3 accountability page. Files: `amplify/backend/function/NewsProjectInvokeAgentLambda/src/{lib.js,index.js,package.json}`, `test/lib.test.js`.
...
## 2026-07-04 (feat: /breaking shows only the past 7 days)

`BreakingFeedPage` now filters alerts to `at >= now − 7 days` before grouping — anything older than a week is dropped (a story that broke >7 days ago is no longer "breaking"). Fail-open on an unparseable timestamp (never hide real news on a date bug). Empty-state + header copy updated to "this week" / "past 7 days". Live-verified on prod: day labels run **Today → June 29** (5 days), nothing older. Files: `components/BreakingFeedPage.jsx`.

## 2026-07-04 (fix: /spider-demo World tier — remove the misleading duration bar)

Follow-up to the span-bar readability fix: **removed the World-tier duration bar entirely.** Each bar drew one country's earliest→latest active window at the bubble's y, but multiple bubbles share a region lane and every situation runs most of the month, so the bars overlapped into a single horizontal rule that read as a connector joining bubbles it connects nothing between (the real connectors are the shared-actor links). The active window already lives in the hover tooltip (`active X–Y · peak Z`), so nothing is lost. Dropped the `<line className="spider-world-span">` render + its now-dead `_x0`/`_x1` layout math + the unused `.spider-world-span` CSS, and corrected the intro copy ("the bar shows how long it ran" → "size = thread count (hover for how long it ran)"). ESLint clean; live `/spider-demo` Playwright check confirms 0 spans / 11 shared-actor links / 12 bubbles, 0 console errors. Prototype-only route. Files: `components/{SpiderWorld.jsx,SpiderDemo.jsx,SpiderDemo.css}`.

## 2026-07-04 (feat: subscribe/unsubscribe confirmations + fix stale "delivery being set up" copy)

Now that delivery is live, gave subscribing/unsubscribing clear feedback. `usePreferences.save()` now returns a success boolean so callers can confirm a real persist (no fake success). **Subscribe:** `SubscribeCard` shows a transient **"Thanks for subscribing!"** thank-you state (green accent, "check your inbox" + Manage link) after a click actually persists — distinct from the calmer persistent "You're subscribed" that returning subscribers see. **Unsubscribe:** the Account → Notifications panel shows a **"✓ Unsubscribed from …"** confirmation when a toggle is turned off or "Unsubscribe from all" is used (and "Subscribed to …" when turned on). Also fixed the now-stale panel copy "Email delivery is being set up" → **"Email delivery is live…"**. (The email-link unsubscribe already returns its own HTML confirmation page via `newsRecommend`.) Verify green (0 errors, 192 tests). Files: `hooks/usePreferences.js`, `components/{SubscribeCard.jsx,SubscribeCard.css,Account.jsx}`.

## 2026-07-04 (EMAIL DELIVERY LIVE — `newsEmailSender` out of dry-run, real send confirmed)

Turned real email delivery on. Verified `globalperspective.net` in Resend (Cloudflare **Auto configure** wrote DKIM/SPF/MX/DMARC — confirmed live via `dig`), then flipped `newsEmailSender` to production: `EMAIL_SEND_DRY_RUN=false`, `EMAIL_FROM=brief@globalperspective.net`, `TEST_RECIPIENT` cleared (env merged, not clobbered — key preserved), and **enabled `TriggerBreakingEmailSend`** (both rules now ENABLED). End-to-end confirmed: a live `{mode:weekly,force:true}` invoke sent a **real** Weekly Signals Brief from `brief@globalperspective.net` to the first real subscriber (`sent:1, errors:[]`); breaking sanity clean (0 subs / 0 fresh). **Kill-switch:** set `EMAIL_SEND_DRY_RUN=true` to revert to log-only. Weekly now auto-sends every Sunday; breaking emails confirmed alerts (≤48h, unsent) to `breakingOptIn` subscribers on the 15-min poll. Backend-only change (Lambda env + EventBridge); no code/frontend change.

## 2026-07-04 (fix: /spider-demo World tier — span bars read as broken connectors)

On the `/spider-demo` World tier, the thick opaque horizontal **span bars** (each country's earliest→latest active window, drawn at the bubble's y) dominated the canvas and, because every situation runs ~a month while its bubble marks a single peak day, they extended far past the dots and merged per-region-lane into long horizontal rules — so they *looked* like connectors that fail to join the dots. The actual connectors (shared-actor links, curved diagonals) were visually lighter than these duration bars. Fix, `components/SpiderWorld.jsx` + `SpiderDemo.css`:
- **`.spider-world-span`** is now lighter than the connectors and **dotted** (opacity `0.28→0.22`, width `3→2`, `stroke-dasharray: 1 5`), so it reads as a background duration axis; the solid `.spider-world-link` lines are now the dominant "these dots connect" signal.
- **Removed three silent fallbacks** (per "we should never need a fallback"): `_peakMs` no longer falls back to `latest` (a situation with no real peak day is filtered out rather than placed at a fake x); `xForMs(null)` no longer snaps a missing date to the far-right edge (returns `null`, callers guard) — that was the exact branch that would draw a floating, disconnected span line; the span bar now renders only when both `earliest` and `latest` are real dates.
- Verified: ESLint clean, `npm run build` green, live `/spider-demo` Playwright screenshot (12 bubbles / 11 links / 12 spans, 0 console errors) — duration bars recede, connectors read clearly. `/spider-demo` is an unlisted throwaway prototype; no prod deploy. Files: `components/{SpiderWorld.jsx,SpiderDemo.css}`.

## 2026-07-04 (feat: breaking-alert subscribe card on /breaking)

Made the breaking opt-in discoverable (it was previously only reachable via Account → Notifications). Generalized `SubscribeCard` with a `kind` prop (`'digest'` | `'breaking'`) — same component, per-kind copy + opt-in field (`digestOptIn` / `breakingOptIn`). Added `<SubscribeCard kind="breaking" variant="breaking" />` on the **`/breaking`** feed page, directly under the header so it shows even when there are no current alerts (red "Breaking alerts · Free" kicker to match the BREAKING accent). One-click for signed-in users; anonymous → Google sign-in → auto opt-in; subscribed → Manage link. Verify green (0 errors, 192 tests); browser-verified on `/breaking`. Note: opt-in is stored, but breaking email delivery stays off until the `TriggerBreakingEmailSend` rule is enabled + the Resend domain is verified (still DRY-RUN). Files: `components/{SubscribeCard.jsx,SubscribeCard.css,BreakingFeedPage.jsx}`.

## 2026-07-03 (feat: Weekly Brief email subscribe button on Home + /weekly-brief)

Surfaced the newsletter opt-in where readers actually are. New `SubscribeCard` component (`components/SubscribeCard.{jsx,css}`) placed on **Home** (under the masthead) and **`/weekly-brief`** (under the KPIs). It reuses the existing opt-in — `usePreferences` → `newsRecommend set_prefs` → `GlobalPerspectiveUserPrefs.digestOptIn` — so **no backend change**: signed-in users subscribe in one click; anonymous users sign in with Google and the subscribe intent is applied immediately after (`signInWithGoogle` → `save({digestOptIn:true})`); already-subscribed users see a "Manage" link to `/account?tab=notifications`. Renders nothing if the prefs endpoint is unconfigured. Theme-aware (uses `--ink`/`--paper`/`--border-color`), responsive (stacks on mobile). **Tested + DEPLOYED (pushed, live):** verify green (0 errors, 192 tests); Home card browser-verified; `/weekly-brief` card browser-verified by intercepting the `weekly_brief` API in Playwright and returning the real brief (renders in-order `wb-kpis → subcard → wb-sec`); pushed to `main` (`eb78dc7`) and **live-confirmed on globalperspective.net** — the card renders on both Home and `/weekly-brief` against the real API. Files: `components/{SubscribeCard.jsx,SubscribeCard.css,Home.jsx,WeeklyBriefPage.jsx}`. (Backend `newsEmailSender` + `newsRecommend` unsubscribe source also landed on `main` in this commit to match deployed prod.)

## 2026-07-03 (feat: email sender for weekly brief + breaking alerts — DEPLOYED to prod in DRY-RUN mode, real send-to-self verified)

Both email products had content flowing but nothing sending: `newsWeeklyBrief` auto-publishes a brief every Sunday and `newsBreakingAlert` confirms alerts into the in-app feed, but the Resend send seam (`sendEmail.js`) was orphaned — only a manual CLI test used it, reaching only the operator via the `onboarding@resend.dev` test domain. Subscriber capture already worked (signed-in opt-in → `set_prefs` → `GlobalPerspectiveUserPrefs` with `email` + `unsubToken`); the gap was the sender + a public unsubscribe. Built **one Lambda for both** (per "single build instead of two"):
- **New `newsEmailSender`** (`amplify/backend/function/newsEmailSender/src/`): `event.mode` dispatch. `weekly` → paginated-scan the latest **published** `WEEKLY_BRIEF`, scan `GlobalPerspectiveUserPrefs` for `digestOptIn`, render + send one personalized email each, write an idempotency marker `EMAILLOG#weekly#<weekOf>` (re-invoke = no-op unless `{force:true}`). `breaking` → scan for `status:'confirmed'` **AND `emailedAt` unset AND ≤48h** (freshness gate prevents back-blasting the ~50 historical `sent` alerts), reuse each alert's pre-rendered `draft`, send, stamp `emailedAt`. Files: `index.js`, `renderWeeklyEmail.js`, `sendEmail.js` (copied from newsBreakingAlert + extended with a `headers` param for `List-Unsubscribe`), `package.json` (no bundled deps).
- **Public one-click unsubscribe** added to `newsRecommend` (`handleUnsubscribe` + `unsubscribe` action, GET link + RFC 8058 POST): the token IS the auth, finds the user by `unsubToken` (paginated scan), flips `digestOptIn`/`breakingOptIn`, returns a small HTML confirmation. Emails carry `List-Unsubscribe` / `List-Unsubscribe-Post` headers.
- **Safe by default:** `EMAIL_SEND_DRY_RUN` defaults ON (logs recipients + would-be email, sends nothing); `TEST_RECIPIENT` overrides the audience for send-to-self; real delivery to subscribers additionally needs a verified `EMAIL_FROM` domain.
- **No new IAM** (classifier gates `aws iam`): `newsEmailSender` reuses `newsprojectLambdaRolefcb19312-dev` (`AmazonDynamoDBFullAccess`), and unsubscribe was refactored to GetItem-by-uid (uid+token in the link) so `newsRecommend-role`'s existing GetItem on UserPrefs suffices.
- **Tested:** hermetic harness `test-sender.js` (28 assertions, mocks SDK + fetch, no live AWS) — all pass.
- **DEPLOYED to prod in DRY-RUN mode:** created `newsEmailSender` (nodejs20, DRY_RUN=true, TEST_RECIPIENT=owner); redeployed `newsRecommend` (unsubscribe — verified 1 ACAO header, `list_alerts` still serves, unsubscribe GET → HTML 200); EventBridge `TriggerWeeklyEmailSend` cron(0 14 ? * SUN *) ENABLED + `TriggerBreakingEmailSend` rate(15m) DISABLED. Live-invoked both modes; **sent one real weekly email to the owner** via the test domain (`sent:1, errors:[]`), then restored DRY_RUN=true. **Go-live remaining (operator):** verify `globalperspective.net` in Resend → flip EMAIL_FROM + EMAIL_SEND_DRY_RUN=false + clear TEST_RECIPIENT → enable the breaking rule. See `EMAIL_SENDER_PLAN.md`.

## 2026-07-03 (fix: /weekly-brief was empty — unpaginated scan + stuck approval gate, DEPLOYED to prod Lambdas)

The `/weekly-brief` nav page rendered "No brief published yet" despite the generator producing a good brief every Sunday. Two independent failures:
- **Serving couldn't find published rows.** `newsSensitiveData` `weekly_brief` action used a single-page (unpaginated) DynamoDB `Scan`; `SummarizeAndPredict` has grown to 2.6 MB (>1 MB scan page), so the published row sat past page 1 and the scan returned 0 (reproduced: scanned 498/1395 items, found 0, `LastEvaluatedKey` present). Fixed by paginating the scan (`do…while ExclusiveStartKey`, 25-page cap — the same pattern the SYSTEMS/PAIR scans already use). **Applied the same fix to two sibling actions with the identical latent bug: `weekly_markets`** (backing `/economy?view=week` — was surviving only by luck of row placement) **and `pair_analyses_list`.**
- **The approval gate went stale.** `newsWeeklyBrief` wrote every brief as `status:'draft'` awaiting a manual `weekly/review.js` approve — training wheels from launch that were never operated past 2026-06-10, so nothing reached `published`. The synthesis prompt is already strictly grounded (uses only facts in each signal's source material, VERB-MARKs unconfirmed claims), so the gate added stale-risk without quality upside for a solo operator. Changed the generator to write `status:'published'` directly (auto-publish); "set a brief to draft" remains as a manual kill-switch. One-time flipped the existing 2026-06-28 draft → published so last week's brief shows immediately.
- Deployed `newsSensitiveData-dev` + `newsWeeklyBrief` via `update-function-code`. Prod-verified: API serves WEEK OF JUNE 28 (6 signals / 5 watch); the live page renders the full brief + KPIs, no empty state, 0 page errors; `weekly_markets` still serves. Generator stays on its Sunday 06:00 UTC cron (`TriggerWeeklyBrief`), so the page now refreshes weekly on its own. Files: `amplify/backend/function/{newsSensitiveData,newsWeeklyBrief}/src/index.js`.

## 2026-07-03 (fix: cross-page wiring audit — 4 broken navigation contracts, DEPLOYED)

Full audit of how every page hands a reader to another (parallel agents over all 24 routes), checking not just "does the link resolve" but "does the receiver honor the sender's *intent*" (query params, route params, hash). Every internal link resolves to a real route; the encode-once/decode-once and `?returnTo` / `?view=week` / `#instrument`-scroll / `?tab=membership` / `/breaking/:id` contracts all verified working. **Four broken contracts found + fixed** (commits `f6034f5`, `01fb0f3`):
- **`?tab=economy` deep-links were ignored** — ~10 senders (Economy, Daily, Home, Map, CountryPage disruption rows, DisruptionRow, WeeklyMarketsView) pass `{tab:'economy'}` via `threadPath`, but ThreadPage never read the param → every economy deep-link landed on Timeline. Now inits `contentTab` from `?tab=` (known keys only) and **derives** the rendered tab from availability (`activeTab`) rather than resetting via effects — no race with the async economy/analysis fetches; shows Timeline until the tab's data arrives, switches when it does, stays on Timeline if it never materializes.
- **CountryPage breadcrumb context was asymmetric** — its arc cards passed `{from:'country',country}` but its coverage "View full story" + Economic Disruption rows didn't, so those threads' back-breadcrumb read generic "Threads" instead of the country. Now all three pass the context.
- **`/weekly?category=<cat>` was a dead link** — ThreadPage's breadcrumb sends it, but WeeklyPage never read the query string. Now seeds `activeCategory` from `?category` (validated against `CATEGORY_ORDER`, re-syncs on back/forward), so the category crumb lands on the filtered feed.
- **CountryPage double-decoded `:countryName`** — React Router v6 `useParams()` already URL-decodes; the extra `decodeURIComponent` decoded twice → latent `URIError`/page crash for any name containing a literal `%`. Removed it (+ the now-unused `useSearchParams` import).
- Prod-verified all four live (Economy tab preselects, Iran breadcrumb, `?category=conflict` filters + hides hierarchy, `United%20States` renders). 192 tests green. Files: `components/{ThreadPage,CountryPage,WeeklyPage}.jsx`.
- **Left open (operator call):** `/contact` is an orphan route (registered, zero inbound links — all "Contact" UI is `mailto:`). Minor: `BreakingDetailPage.jsx:158` hand-builds a thread link instead of using `threadPath` (encodes correctly + guarded — convention drift only, not a bug).
...
- **Deployed + LIVE (published 2026-06-29):** `newsWeeklyMarkets` created + `newsSensitiveData-dev` updated (both ap-northeast-1); frontend shipped via `./deploy.sh`; first report (week of 2026-06-27, 10 movers / 5 grounded in our coverage) reviewed + **published** + **live browser-tested** (Playwright — all render/tier/anchor/cross-link checks pass; only console noise is the expected SPA-fallback 404 + Cloudflare RUM beacon). `npm run verify` green (0 ESLint errors, 178 tests).
- **Markets-history backfill:** a 16-day gap (06-06→06-21, commodities + equities) from the Stooq→Yahoo outage left **null** `HISTORY#` rows that `seed_history` wouldn't overwrite. Deleted the 32 null gap rows → re-ran `seed_history` → Yahoo refilled them. Made this week's report rich AND fixed the `/economy` sparklines. (Gotcha recorded for future ingest outages.)
- **Also fixed (outdated):** `EconomyPage` masthead + footer credited "Stooq" — corrected to "Yahoo" (markets migrated 2026-06-23). `ARCHITECTURE.md` updated — new Lambda #25, `weekly_markets` action, `WEEKLY_MARKETS#{week}` record, `/weekly-markets` route, `useWeeklyMarkets` hook, Perplexity external-API row, header refresh.
- **Scheduled + nav (2026-06-29):** EventBridge Rule `TriggerWeeklyMarkets` — `cron(30 8 ? * SUN *)` now generates a weekly **draft** (human still publishes via `weekly-markets/review.js`). Added **"Markets"** to the top nav (`Layout.jsx`, after Economy). Report regenerated + published for the current week (2026-06-29, full universe restored post-backfill).
