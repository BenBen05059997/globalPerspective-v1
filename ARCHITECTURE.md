...
### Prod infra NOT on `main` — Signal-API + impact-first ingest (documented 2026-07-25)

Four Lambdas are **deployed and ENABLED in prod** but their **source lives only on the unmerged `signal-api` branch/worktree**, so they had no entry here. They pair with 4 undocumented DDB tables (below). Flagged by the 2026-07-25 staleness audit; source not on `main` means the deployed-bytes are the only reference — verify against live AWS before changing them. See [[project_signal_api_deployed]], [[project_impact_first_redesign]], `signal-api` branch.
...
### Signal-API + ingest tables (`signal-api` branch — live in prod, documented 2026-07-25)
These exist in prod DynamoDB but their producers' source is on the unmerged `signal-api` branch (see "Prod infra NOT on `main`" under the Lambda inventory). Key schemas confirmed via `describe-table` 2026-07-25:
...
| `TriggerSignalsBuild` | `cron(0 10 * * ? *)` | newsSignals (Signal-API — `signal-api` branch, deployed to prod; see "Prod infra not on `main`") |
| `TriggerImpactAudit` | `cron(0 9 * * ? *)` | newsImpactAudit (impact-first — `signal-api` branch) |
| `TriggerGdacsIngest` | `cron(0 */6 * * ? *)` | newsGdacsIngest (GDACS ingest — `signal-api` branch) |
| `TriggerGdeltConflict` | `cron(0 */6 * * ? *)` | newsGdeltConflict (GDELT ingest — `signal-api` branch) |
