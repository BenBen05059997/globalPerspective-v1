...
| `world_overview` | None | `{}` | `{ situations, links }` — per-country situation stats (paginated SYSTEMS# scan) + cross-country shared-actor `links` (ambient actors excluded) for the World Overview tier |
