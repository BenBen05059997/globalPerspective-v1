...
- [x] `no-restricted-imports` eslint rule is active and `npm run verify` is clean under it.
- [x] `components/`, `hooks/`, `utils/`, `services/`, `contexts/`, `onboarding/`, `data/`,
      `assets/`, `styles/` no longer exist under `src/`.
- [x] `project-docs/architecture/ARCHITECTURE.md` has a complete old→new path table (all 193
      files, minus the 5 P0 deletions, covered across the phase-by-phase appends) and the
      Feature → Lambda index table.
- [x] `project-docs/INDEX.md` has a row pointing at this plan + the design doc (P12).
- [x] `CHANGES.md` has one entry per phase commit (12 phases + P0 = up to 13 entries, or fewer
      if §0(a) chose P0+P1-only).
- [x] `README.md` "where things live" table added (P12).
- [x] This task file (`TASK_2026-09-24_frontend_feature_folders.md`) header flipped from
...
- [x] Operator auto-memory lazy pass done (P12) — only path-qualified mentions, not every
      basename citation.
- [x] No deploy has occurred as part of this programme unless the operator explicitly asked
      for one with a fresh "yes" at a specific phase (§0(f)).

**Closing note (P12, 2026-09-24):** The programme is complete — P0 through P12, 13 commits, no
deploy, no reverts needed. Every phase's stop conditions (§1 ground rule 5) held throughout: test
count never drifted beyond the one expected P0 deletion, every build was byte-size-identical to
its pre-phase build, and every browser click-through in the phase ledger above came back clean.
`ARCHITECTURE.md` is now the durable record of the final tree; this plan and the task file stay
in `_active/` (per the operator's stated convention — status header flips to `done`, the file
isn't relocated) as the historical how-we-did-it record for anyone who lands on a stale path
citation later.
...
| P2 | done | see CHANGES | verify: 15 files / 184 tests (unchanged); build: main bundle byte-identical, 1,046,069 bytes; `verify_pages.sh` 32/0 (first run surfaced 2 FAIL on the un-updated `App.jsx` guard rows — confirms the P0 hardening fires loudly, fixed same commit); `auth-guard-check.mjs` PASS (7/7); browser sweep 17 routes, 0 console errors (fresh `npm run dev`, stale-HMR errors from the pre-move dev process discarded) | 46 files moved via `move-module.mjs`. 2 relative-import breakages found and fixed by hand (not covered by the `@/`-only codemod, since these are co-located CSS/JS pairs that did NOT move together): `main.jsx` (unmoved) → `./index.css`/`./bootstrapProxy.js` (both moved to `app/`) fixed to `@/app/...`; `App.jsx` (moved to `app/`) → `./components/atoms/atoms.css` (moved to `shared/ui/`, different dir than `App.css`) fixed to `@/shared/ui/atoms.css`. SourceRobustness.jsx/sourceRobustness.js case-collision pair confirmed correctly split (shared/ui/ vs utils/, unchanged until P7). ARCHITECTURE.md got its first Frontend Path map rows + the full Feature→Lambda index table. |
| P3 | done | see CHANGES | verify: 15 files / 184 tests (unchanged); build: main bundle byte-identical, 1,046,069 bytes, CSS 295,600 bytes; `verify_pages.sh` 32/0 (after updating the 2 `Disclosures.jsx` guard rows); `auth-guard-check.mjs` PASS (7/7); relative-import sweep clean (only the 2 documented N-file edges) | 8 files moved via `move-module.mjs`. Only live-doc hit needing a fix: `quality/verify_pages.sh`'s 2 Disclosures.jsx rows. `.githooks/pre-push`'s basename regex (`Disclosures`) already matched, no edit needed. SpiderDemo/SpiderWorld's cross-feature imports of threads/countries left pointed at pre-move paths per the plan's note, to be retargeted automatically by move-module.mjs when P10/P11 move those files. |
| P4 | done | see CHANGES | verify: 15 files / 184 tests (unchanged); build: main bundle 1,046,070 bytes (1,046.07 kB) and CSS 295,600 bytes (295.60 kB), byte-identical to baseline; `verify_pages.sh` 32/0 (after updating the 3 `DailyPage.jsx` guard rows); `auth-guard-check.mjs` PASS (7/7, `useDailyBrief.js` resolves via basename search after leaving `hooks/`); relative-import sweep clean (only the 2 documented N-file edges) | 10 files moved via `move-module.mjs`: daily (2 + 1 hook), weekly-brief (2 + 1 hook), track-record (2 + 2 hooks). Live-doc fixes: `quality/verify_pages.sh` 3 `DailyPage.jsx` rows, `PAGES_GUIDE.md` 3 path-qualified headings (`/daily`, `/weekly-brief`, `/track-record`). No N files in this phase. |
| P5 | done | see CHANGES | verify: 15 files / 184 tests (unchanged); build: main bundle 1,046,069 bytes (1,046.07 kB) and CSS 295,600 bytes (295.60 kB), byte-identical to baseline; `SituationMap3D` lazy chunk 942,987 bytes (942.99 kB, gzip 262.63 kB), byte-identical, still a separate chunk; `verify_pages.sh` 32/0 (no path edits needed — its guards reference economy/threads files only); `auth-guard-check.mjs` PASS (7/7); relative-import sweep clean (only the 2 documented N-file edges) | 9 files moved via `move-module.mjs`. Confirmed `SituationHome.jsx`'s `lazy(() => import('@/components/SituationMap3D.jsx'))` dynamic-import string was rewritten to `@/features/map/components/SituationMap3D.jsx` by the helper. `assets/countries-110m.json` confirmed importer-only-within-map (SituationMap.jsx, SituationMap3D.jsx) so kept in the feature, not `shared/`. Live-doc fixes: `PAGES_GUIDE.md`, `MAP_HOME_SITUATION_PLAN.md`, `MAP_HOME_SITUATION_LEDGER.md`, `MAP_UI_FIX_QUEUE.md`, `PAIR_ARCS_RELOCATION_PLAN.md`, `TASK_2026-09-24_pair_arcs_relocation.md`, plus the 2026-09-11 frontend audit snapshots (FRONTEND_QUALITY_AUDIT, FRONTEND_STRUCTURE_AUDIT, SYSTEM_USAGE) picked up by the helper's live-surface grep. |
| P6 | done | see CHANGES | verify: 15 files / 184 tests (unchanged); build: main bundle 1,046,070 bytes (1,046.07 kB) and CSS 295,600 bytes (295.60 kB), byte-identical to baseline; no lazy chunks touched by this phase; `verify_pages.sh` 32/0 (no path edits needed — guards reference economy/threads files only, confirmed by grep); `auth-guard-check.mjs` PASS (7/7); relative-import sweep clean (only the 2 documented N-file edges) | 22 files moved via `move-module.mjs`: breaking (7 files: 2 pages + css, NotificationBell jsx/css, BreakingStrip jsx/css, 2 hooks), account (13 files: Account jsx/css, SignIn, AuthCallback, MembershipPage jsx/css, SaveButton, FollowButton, SubscribeCard jsx/css, 3 hooks). No N files in this phase. Billing UI (MembershipPage.jsx, useMembership.js) moved as pure file relocation only — no logic touched. Live-doc fixes: `PAGES_GUIDE.md` (4 route headings: `/membership`, `/signin`, `/auth/callback`, `/account`), `POLAR_BILLING_PLAN.md` (frontend file list), `MAP_HOME_SITUATION_PLAN.md` (`BreakingDetailPage.jsx:106` citation). Post-edit doubled-segment grep (`features/account/features/account` etc.) clean. |
| P7 | done | see CHANGES | verify: 15 files / 184 tests (unchanged); build: main bundle 1,046,070 bytes (1,046.07 kB) and CSS 295,600 bytes (295.60 kB), byte-identical to baseline; `verify_pages.sh` 32/0 (no path edits needed — its guards reference economy/threads files only, confirmed by grep); `auth-guard-check.mjs` PASS (7/7); N-file Node-tooling consumers: `node quality/analysis/run.mjs` 18/18 (Layer A + A2 golden fixtures, no key needed), `check.mjs`/`compare.mjs`/`judge.mjs`/`source_check.mjs` each reached their "set ANALYSIS_EVAL_KEY" message (import-resolution proven); relative-import sweep clean (only the 2 documented N-file edges, unchanged by this phase) | 15 files moved via `move-module.mjs`: AnalysisStudio jsx/css, ProviderModal jsx/css, AnalysisVisuals jsx/css → `features/analysis-studio/` (+`components/`); `llm.js` (N), `analysis.js`, `analysisPrompt.js` (N), `analysisValidator.js` (N), `analysisStruct.js` (N), `byok.js`, `sourceRobustness.js` → `features/analysis-studio/lib/`; 2 test files → `__tests__/`. The 4 N files have zero outgoing imports of their own (confirmed by grep before moving), so no internal relative-import rewrite was needed on them — the "keep relative" header comments from P1 remain accurate as documentation of the constraint even though there was nothing to rewrite. Updated the 13 relative import lines (by hand, `move-module.mjs` doesn't touch Node tooling outside `src/`) across `quality/analysis/{check,compare,judge,run,source_check}.mjs` plus 1 path reference in `quality/analysis/README.md`. Completed the P2-flagged case-collision split: `utils/sourceRobustness.js` now at `features/analysis-studio/lib/sourceRobustness.js`, never sharing a directory with `shared/ui/SourceRobustness.jsx`. No prompt text edited (newsAnalyze Lambda's server-pinned `SYSTEM_PROMPT` byte-copy unaffected). Live-doc fixes: `PAGES_GUIDE.md` (`/analyze` heading), `QWEN_AND_VISUAL_BLOCK_PLAN.md` (5 `src/...` citations). Post-edit doubled-segment grep (`features/analysis-studio/features/analysis-studio`) clean. |
| P8 | done | see CHANGES | verify: 15 files / 184 tests (unchanged); build: main bundle 1,046.07 kB, CSS 295.60 kB, `SituationMap3D` lazy chunk 942.99 kB — all byte-identical to baseline; `verify_pages.sh` 32/0 (after updating the 4 `EconomyPage.jsx` rows + 6 atom `QualityFlag`-wiring rows); `auth-guard-check.mjs` PASS (7/7); N-file Node-tooling consumers: `verify_compose.mjs` 5/5, `verify_instrument_why.mjs` 10/10, `verify_lede.mjs` 4/4 (target unaffected, moves in P9), `test-disruption-gate.mjs` import path re-pointed and confirmed its `ERR_IMPORT_ATTRIBUTE_MISSING` JSON-import-attribute failure is pre-existing (identical on pre-P8 tree via `git stash`, not a regression); `verify_all.sh --fast` all 4 layers green (L2 61/61, L3 26/26, L5/L6 34/34, L7 32/32); relative-import sweep clean (only the 2 documented N-file edges) | 26 files moved via `move-module.mjs`: EconomyPage/WeeklyMarketsPage/WeeklyMarketsView (5), 6 atoms (incl. DisruptionRow/DisruptionPreview kept per operator §0(c)), 7 hooks, 3 N files (composeEconomyBriefing.js, disruptionGate.js, economicAnalogs.js/.json), 3 tests. `disruptionGate.js → ../data/economicAnalogs.js` relative edge preserved unchanged (identical relative offset at new location). Live-doc/script fixes: `quality/verify_pages.sh` (10 rows), `quality/dashboard.js:155-156`, `quality/verify_all.sh` (L5/L6 test paths), `quality/briefing/{verify_compose.mjs,verify_instrument_why.mjs,assertions.js}`, `scripts/test-disruption-gate.mjs`, `PAGES_GUIDE.md`, `project-docs/economy/ECONOMIC_DISRUPTION.md`. `e2e/economic.spec.js` confirmed route-only (no path edit needed). `ECONOMY_BRIEFING_PLAN.md` (`_proposed`) grepped, no path-qualified hits. |
| P9 | done | see CHANGES | verify: 15 files / 184 tests (unchanged); build: main bundle 1,046,069 bytes (1,046.07 kB) and CSS 295,600 bytes (295.60 kB), byte-identical to baseline; `verify_pages.sh` 32/0 (after updating the 4 `Home.jsx` rows); `auth-guard-check.mjs` PASS (7/7); N-file Node-tooling consumer `quality/briefing/verify_lede.mjs` 4/4 cases passing after re-pointing to `features/home/lib/composeTopicsLede.js`; `quality/verify_all.sh --fast` all 4 layers green (L2/L3 26/26, L5/L6 34/34, L7 32/32); relative-import sweep clean (only the 2 documented economy N-file edges, unchanged by this phase) | 15 files moved via `move-module.mjs`: Home jsx/css, AIComponents.css → `features/home/`; TopicNav, TodayArchiveSidebar (jsx/css pairs), ArchiveTopicModal, PredictionDisplay, SummaryDisplay, TraceCauseDisplay, LedeBand jsx/css → `features/home/components/`; useTodayArchive.js → `features/home/hooks/`; composeTopicsLede.js (N) → `features/home/lib/`. `composeTopicsLede.js` had zero outgoing imports to rewrite (confirmed by grep before moving) — the P1 "keep relative" header comment remains accurate documentation even with nothing to rewrite. `AIComponents.css` is imported cross-feature by `ThreadPage.jsx`/`WeeklyPage.jsx` (still at pre-move `components/` path, moving in P11) — `move-module.mjs`'s repo-wide specifier rewrite retargeted both import lines to `@/features/home/AIComponents.css` automatically; documented cross-feature edge (threads → home), not a stranded import. Live-doc fixes: `quality/verify_pages.sh` (4 `Home.jsx` rows), `PAGES_GUIDE.md` (`/` heading), `ARCHITECTURE.md` (new P9 Frontend Path map section; also corrected the "Frontend imports" N-file list and the `LedeBand.jsx` Key Components row, which still cited stale pre-P7/P9 paths from earlier phases). `IMPORTANCE_SCALE_MAPPING.md`'s dated "Grounded in ... this date" snapshot line left untouched (point-in-time grounding statement, not a live path reference, consistent with how prior phases treated it). |
| P10 | done | see CHANGES | verify: 15 files / 184 tests (unchanged); build: main bundle 1,046,069 bytes (1,046.07 kB), CSS 295,600 bytes (295.60 kB), `SituationMap3D` lazy chunk 942,987 bytes — all byte-identical to baseline (same hashes: `index-DyBJtaXZ.js`, `index-DLLacZou.css`); `verify_pages.sh` 32/0 (after updating the 3 `CountryPage.jsx` rows + 3 `CountryListPage.jsx` rows); `auth-guard-check.mjs` PASS (7/7, `useCountryIntelligence.js` resolves via basename search, no `!user` guard introduced); `verify_all.sh --fast` all 4 layers green (L2/L3 26/26, L5/L6 34/34, L7 32/32); relative-import sweep clean (only the 2 documented economy N-file edges, unchanged by this phase) | 18 files moved via `move-module.mjs`: CountryListPage/CountryPage jsx/css → `features/countries/`; CountryOverviewMap, BackgroundTimeline, SystemsGraph jsx/css, CountryWhatChanged jsx/css → `features/countries/components/`; useCountryIntelligence/useCountryHistory/useSystemsAnalysis → `features/countries/hooks/`; countryDrift.js → `features/countries/lib/`; 5 test files → `features/countries/__tests__/`. No N files. `features/spider-demo/SpiderDemo.jsx`'s cross-feature `useSystemsAnalysis` import (pointed at pre-move path since P3) retargeted automatically by the helper's repo-wide specifier rewrite. `features/countries/CountryListPage.jsx`'s `WeeklyPage.css` import left pointed at its pre-move path, to be retargeted automatically when threads moves in P11 (same pattern as P3/P6/P9). Live-doc fixes: `PAGES_GUIDE.md` (2 route headings), `ARCHITECTURE.md` (new P10 Frontend Path map section). Doubled-segment grep clean. |
| P11 | done | see CHANGES | verify: 15 files / 184 tests (unchanged); build: main bundle 1,046,069 bytes (1,046.07 kB), CSS 295,600 bytes (295.60 kB), `SituationMap3D` lazy chunk 942,987 bytes — all byte-identical to baseline; `verify_pages.sh` 32/0 (after updating the 3 `ThreadPage.jsx` rows + the `WeeklyPage.jsx` negative-guard path; re-verified the negative guard still fires loudly by temporarily reintroducing a forbidden pattern and reverting); `auth-guard-check.mjs` PASS (7/7); `verify_all.sh --fast` all 4 layers green; relative-import sweep clean (only the 2 documented economy N-file edges); `npx eslint src` 0 errors under the new dependency-direction rule (3 pre-existing unrelated warnings) | 15 files moved via `move-module.mjs`: WeeklyPage/ThreadPage jsx/css → `features/threads/`; WeeklyMap jsx/css, CompactTimeline, StoryEntryCard, ThreadIntelligence, ThreadForecast, TrendBadge → `features/threads/components/`; useWeeklyArchive, useThreadAnalyses, useNarrativeThread, useThreadForecast → `features/threads/hooks/`; mapConstants.js → `features/threads/lib/`. No N files. Confirmed `components/`, `hooks/`, `utils/`, `services/`, `contexts/`, `onboarding/`, `data/`, `assets/`, `styles/` (incl. the leftover empty `components/atoms/`) were empty and deleted; `src/` top level is now `app/`, `features/`, `shared/`, `test/`, `main.jsx` only. Added `no-restricted-imports` to `eslint.config.js`: repo-wide bans `@/components/*`/`@/hooks/*`/`@/utils/*`; `shared/**` banned from `@/features/**`+`@/app/**`; `features/**` banned from `@/app/**`; feature→feature imports left unenforced (the design's §2.5 documented edges — countries→threads, spider-demo→threads/countries, account→threads all retargeted automatically by the helper). Live-doc fixes: `quality/verify_pages.sh` (4 rows), `PAGES_GUIDE.md` (3 route headings + `services/llm.js`/`utils/*` citations in Analysis Studio/Account sections), `ARCHITECTURE.md` (new P11 Frontend Path map section, dependency-direction rule note, Path column added to the 57-row Key Components + 28-row Key Hooks tables, stale `src/onboarding/` reference corrected to `src/app/onboarding/`). `PAIR_ARCS_RELOCATION_PLAN.md` grepped, no hits on this phase's files. |
| P12 | done | see CHANGES | `npm run verify`: 15 files / 184 tests, 0 lint errors (3 pre-existing `react-hooks/exhaustive-deps` warnings, unrelated); `bash quality/verify_pages.sh` 32/0 — both unchanged from P11, as expected for a docs-only phase | Docs only, no `frontend/src` or `amplify/` changes: `ARCHITECTURE.md` got a new "Frontend layout (final tree)" summary section ahead of the existing incremental old→new Path map (kept for history) + confirmed the Feature→Lambda index, Routes, Key Components/Hooks tables, `@/` alias note, and eslint dependency-direction note are all current; `README.md` frontend-source row now points at the feature layout; root `CLAUDE.md`'s Layout section gained a short `app/`/`shared/`/`features/<name>` + dependency-rule line; `project-docs/INDEX.md`'s two restructure rows flipped to EXECUTED/DONE. Final live-doc sweep (`grep -rn "src/components/\|src/hooks/\|src/utils/\|src/services/" --include=*.md`) found no stale hits in `project-docs/playbooks`, `agent-kit`, `.claude`, or any `_active` architecture/ops doc; the only remaining hits are in `_shipped`/`_reference`/`_proposed`/`_legacy` docs, dated point-in-time snapshots (`*_2026-09-11.md`, `ARCHITECTURE_VERIFICATION_LOG.md`), `SYSTEM_WIRING.md` (explicitly self-disclaimed as a stale 2026-05-18 snapshot deferring to `ARCHITECTURE.md`), and the pair-arcs relocation docs (`PAIR_ARCS_RELOCATION_PLAN.md`/its task file) citing the already-**deleted** (not moved) `WorldMapV2.jsx` from an unrelated, separately-cancelled cleanup — none require a fix under this programme. §0(a)'s full-P0–P12 scope, all §0 defaults, and the "no deploy during the programme" rule (§0(f)) were honored throughout; no deploy occurred. |
