...
**Status:** ✅ SHIPPED — live, deployed & monetized. [Corrected 2026-09-08 doc-staleness sweep — the "DESIGN / not started" header went stale: the feature is live (see `components/AnalysisStudio.jsx`, `newsAnalyze` Lambda, and the follow-on `ANALYSIS_PRO_STRUCTURE_PLAN.md` / `ANALYZE_SURFACING_PLAN.md`).] Originally created 2026-06-10 as DESIGN / not started.
**Goal:** A page where a signed-in user **picks real stories from our data and gets a deep, cited analysis** — turning a passive reader into an active analyst. This is the feature that the future Polar **credits** will meter (see [`POLAR_BILLING_PLAN.md`](../../billing/_active/POLAR_BILLING_PLAN.md) Decision #5). **Build the feature first, monetize later.**
...
3. **Later (credits):** swap BYOK → our key behind the `newsAnalyze` Lambda + Polar credits, per [`POLAR_BILLING_PLAN.md`](../../billing/_active/POLAR_BILLING_PLAN.md).
