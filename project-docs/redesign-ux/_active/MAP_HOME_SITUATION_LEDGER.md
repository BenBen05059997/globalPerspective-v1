...
**Follow-up — S5.5·T3b (identity pass 2):** given-name aliasing + cross-axis consolidation for the same event, to actually cut the single-outlet fragmentation the conservative pass 1 left. Needs its own design (avoid over-merge: an explicit actor-alias map is safer than surname extraction, which would collide e.g. North/South Korea). 🔭 todo.
