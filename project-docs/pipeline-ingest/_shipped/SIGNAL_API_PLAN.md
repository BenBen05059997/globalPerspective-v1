...
**Status:** ✅ DEPLOYED (live-verified 2026-09-08) — `newsSignals` Lambda Active since 2026-06-23; `GlobalPerspectiveSignals` table populated (~5,720 signals, daily build cron running); `GlobalPerspectiveApiKeys` ACTIVE but **0 keys minted** (the one real remaining gap). 45 unit-test claim accurate. ⚠️ Deployed bytes predate the 2026-08-01 `main` merge — diff before changing. [Corrected 2026-09-08 doc-staleness sweep — prior "NOT deployed" / "tables not created" was stale.] This doc is the contract to hand a design partner.
