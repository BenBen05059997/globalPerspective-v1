...
Posts a daily AI-written summary article to [Dev.to](https://dev.to) — **and, despite the name, is also the sole producer of the Daily Intelligence Brief behind `/daily`.**

> ⚠️ **Misleading name — two independent jobs on two different providers** (corrected 2026-07-29; the doc previously described only the Dev.to half). `/daily` has no other producer, so a failure here silently empties the Daily Brief page while the Dev.to post is unaffected (and vice versa). When triaging `/daily`, look at *this* Lambda, not a "daily brief" function — there isn't one.
> - **`generateAndStoreDailyBrief` → `callGrok`** uses **DeepSeek** via `GROK_MODEL`/`GROK_API_URL`/`XAI_API_KEY` and writes `DAILY_BRIEF#{dateKey}` to `SUMMARIZE_PREDICT_TABLE`. Migrated to `deepseek-v4-flash` + `thinking:{type:'disabled'}` on 2026-07-27 (main `c232624`) — it was the last straggler on the retired `deepseek-chat` alias.
> - **The Dev.to article** uses **OpenRouter** via `AI_MODEL` — a completely separate call path and key.
...
3. Calls OpenRouter AI (`AI_MODEL`, default `deepseek/deepseek-v4-flash:free`) to generate a long-form Dev.to article
4. Checks `SOCIAL_POSTS_TABLE` to skip if already posted today
5. Posts to Dev.to via API; records post with 90-day TTL

**Key env vars:** `DEVTO_API_KEY`, `OPENROUTER_API_KEY`, `AI_MODEL` (default `deepseek/deepseek-v4-flash:free`), `XAI_API_KEY` / `GROK_MODEL` (=`deepseek-v4-flash`) / `GROK_API_URL` (the **daily-brief** path — legacy names holding DeepSeek values), `TOPICS_DDB_TABLE`, `SUMMARIZE_PREDICT_TABLE`, `SOCIAL_POSTS_TABLE`, `SITE_URL`
...
### Signal-API + impact-first ingest — source MERGED to `main` 2026-08-01

Four Lambdas **deployed and ENABLED in prod** whose source lived only on the unmerged `signal-api` branch until **2026-08-01**, when that branch was merged (`59054ec`) — closing the "Prod infra NOT on `main`" gap the 2026-07-25 staleness audit flagged. They pair with the 6 DDB tables below. See [[project_signal_api_deployed]], [[project_impact_first_redesign]].

> ⚠️ **Merged ≠ reconciled.** The merge landed the branch's *source*; it does **not** prove the source matches the **deployed bytes**, which drifted independently for ~5 weeks while only the deployed copy was live. Before changing any of these four, still diff against live AWS (`aws lambda get-function --query 'Code.Location'`). Known drift class: the merge brought `newsInvokeGemini`'s `captureIngestion` harness onto `main` alongside `main`'s own V4 `thinking:{type:'disabled'}` patch (both verified present post-merge, different regions of the file) — but whether prod's deployed `newsInvokeGemini` carries the harness is unverified. Two conflicts in `newsBreakingAlert` were resolved as a union: `significance.js` genuinely holds **both** the branch's `RISK_CAP=50` / risk-weight-2.0→1.0 rework **and** `main`'s `axisForCategory`/`regionRiskScore` category-axis dominance fix; the test suite was reconciled to the capped behaviour (50/50 pass).
...
| **`newsImpactAudit`** | `TriggerImpactAudit` `cron(0 9 * * ? *)` | 120s / 256MB | Coverage dead-man's-switch — DeepSeek audit of whether high-impact events were MISSED; SNS→`GlobalPerspectiveAlerts` when misses ≥ `MISS_ALERT_THRESHOLD=2`. **S8·T2 (2026-09-09):** reads GDACS + GDELT from S3 corpus (`corpus/gdacs/latest.json`, `corpus/gdelt/<day>.json`) and writes its verdict to S3 `audit/impact/<date>.json`+`latest.json` (the `ImpactAudit` DDB table was dropped). Still reads the latest `IngestCapture` row from DDB (`INGEST_CAPTURE_TABLE`) until T2b. |
...
These exist in prod DynamoDB; their producers' source lived on the unmerged `signal-api` branch until the 2026-08-01 merge (see "Signal-API + impact-first ingest" under the Lambda inventory — note the merged-≠-reconciled caveat). Key schemas confirmed via `describe-table` 2026-07-25:
...
| `GlobalPerspectiveIngestCapture` | (see describe-table) | newsInvokeGemini | Selector saw-vs-chose capture (audited by newsImpactAudit). **S8·T2b (pending):** migrate → S3 `audit/ingest-capture/`; deferred (touches the live content pipeline) |
