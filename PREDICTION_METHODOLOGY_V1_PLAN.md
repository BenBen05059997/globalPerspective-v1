...
**Status:** Phase 1 **DEPLOYED 2026-07-04** (era-cut date) — `NewsProjectInvokeAgentLambda-dev` live on v1 code; verified in prod (27 `methodologyVersion:1` snapshots, gates firing live: G4 caught 3 relative-window triggers, 0 retrodictions). Phase 2 **tooling built** (`predictions/resolve-v1-*.js` + runbook; awaits ~2026-07-11 deadlines). Phase 3 **SHIPPED 2026-07-05** — `/track-record` → "Accountability" hub live (§5). Phase 4 **SHIPPED 2026-07-06** — "Living forecast" board on ThreadPage (§5). **All four phases shipped.** Resolution is LIVE — first run 2026-07-07 (16/21 scored) + second run 2026-07-08 (all 21 due scored, queue drained): `/track-record` shows resolved 35, fired 9, Brier 0.157, three populated calibration buckets. G6 sports-scope capture gate added 2026-07-07 (§4). **Phase 5 (calibration-digest feedback loop) DESIGNED, data-gated → `CALIBRATION_DIGEST_PLAN.md`** — compacts the resolved-trigger trace into a ~200-token digest injected into generation (`methodologyVersion:2`); build nothing before its CP-1 (~50 resolved) / CP-2 (~150 resolved) gates.
