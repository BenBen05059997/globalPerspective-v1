...
| M4 · Radar mode | Flat radar, sweep, flare per pass, "scanned" mark in the feed (no focus theft); GLOBE / RADAR switch; auto radar without 3D | ✅ | see git log (branch) | verify 25 files / 246 tests; browser: GLOBE → RADAR switch, real coastlines, sweep visible, Mexico marker + callout; agent verified flare at the beam crossing, the feed "◉ scanned" mark (no scroll/focus change), marker click → `?focus=`, round trip to GLOBE. Old deck.gl flat mode removed (the approved design has 2 modes). Known issue for M6: panels + callout crowd the map's left side |
...
| M6 · Legend + layout | Compact "?" legend; max 3 movers / 8 pulses; GDACS text badge; **fix the crowding noted in M2–M4** (panels vs callout; the brief repeats the lede; the globe is small at default zoom) | **Now** | — | — |
