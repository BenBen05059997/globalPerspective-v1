...
let inputPath = path.join('quality', 'severity_gold_set.json');
let rubricVersion = 'v1 (pre-Phase-4 prompt)';

// Parse positional argument (if it doesn't look like a flag)
if (process.argv[2] && !process.argv[2].startsWith('--')) {
  inputPath = process.argv[2];
}

// Parse named arguments
const rubricVersionIdx = process.argv.indexOf('--rubric-version');
if (rubricVersionIdx !== -1 && rubricVersionIdx + 1 < process.argv.length) {
  rubricVersion = process.argv[rubricVersionIdx + 1];
}
...
function main() {
  ...
  lines.push(`Rubric version: ${rubricVersion}. Rubric: [\`SEVERITY_CODEBOOK.md\`](../../project-docs/architecture/SEVERITY_CODEBOOK.md).`);
  ...
  lines.push('---');
  lines.push('');
  lines.push('Reports must state the rubric/prompt version that produced the graded model_dimensions, so different rubric eras are never blended into one agreement number.');
  lines.push('');
