...
| `WeeklyPage.jsx` | 3-col EditorialShell: **tier-based front-page hierarchy** (`RISK_TIERS_PLAN.md` P3, 2026-07-03) above the category river — **1 LEAD** (tier `high` AND events ≤24h; Techmeme-style text-forward card: rule label, serif headline, why-it-matters, evidence row, arc dots, `↳ What changed` drift-delta line from the living-analysis loop) + **≤3 DEVELOPING** (fresh drift note OR ≥elevated & ≤48h; half-width rows). Then narrative threads grouped **by category** in the river (region is a left-rail filter). Hierarchy renders **only on the default unfiltered view** (search/filter = work mode = flat river); promoted threads are removed from the river **and** the right rail "Rising This Week" (no double-show); honest-empty if nothing qualifies. StatusStrip, left rail (search/period/sort/region/view-toggle); lazy-loaded `WeeklyMap` view mode |
