...
| **ACLED** | **fatalities** + actors + locations, real-time, 200+ countries | political violence + protests | ❌ **DROPPED 2026-06-25 — not openly accessible.** Requires a registered myACLED account + accepted Terms of Use to mint a key; access is **tiered** (personal email = limited) and **commercial use is heavily restricted** (a few downloads company-wide). Unsuitable for an automated, potentially-commercial product. (Our creds authenticate but `/api/acled/read` = 403, no scopes — and even fixed, the licensing kills it.) → use the open conflict alternatives below. |
| **GDELT** (open conflict signal) | actor→action→actor events + **Goldstein** impact + global mention volume, 15-min, 100+ langs | conflict / protest / violence (and everything else) | ✅ **truly open** — free file downloads + BigQuery, no account. The realistic conflict feed in place of ACLED (less precise on fatalities, but open + real-time). Also the wide attention net. **NEXT BUILD.** |
| **ReliefWeb** (UN OCHA) | curated humanitarian crises + reports (conflict-driven + disaster) | humanitarian | ✅ open API, no key, 1000/day. Secondary open conflict/humanitarian feed. |
