...
| N1 Navigation | Site navigation, APPROVED 26 Sep: Map · Stories · Briefings · Studio · Track record; same URLs (only `/briefings` new); computed "paused since" line; phone bottom tab bar; search later |
| P1 PhonePattern | Phone pattern, APPROVED 26 Sep: READ first (Map page opens on the radar), MAP tab with a bottom sheet (peek/half/full) that loads WebGL only then, third tab TIMELINE / EDITIONS / LOG; one switch per screen; 44px targets |
