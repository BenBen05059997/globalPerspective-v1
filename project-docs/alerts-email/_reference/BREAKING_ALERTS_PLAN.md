...
**Relationship:** Component 4 of [`RECOMMENDATIONS_AND_DIGEST_PLAN.md`](../_shipped/RECOMMENDATIONS_AND_DIGEST_PLAN.md). Shares the `GlobalPerspectiveUserPrefs` table, the email-sending path, and the double-opt-in / one-click-unsubscribe compliance scaffolding with the digest. This doc covers only what is *different*: an event-driven trigger and a global significance selection rule.
