...
> - **Backfill ("keep it not drifted") — ✅ SHIPPED + PROVEN 2026-07-02.** GOAL RECLARIFIED with operator: the product IS the auto-correcting system (success = "did it catch & ground every material move?"), NOT the track record. Corrector was grounding only the single newest move per run → `findAllDrifts` now grounds **every** ungrounded move in the window (`processCountry`/`processThread`); self-heals a missed cron day / pre-live history. **Last-week backtest: 11/11 moves grounded (100%, was 2/18%)** — grounded in real events, honest "no single driver" preserved. Re-runnable coverage check: `/tmp/backtest-drift.mjs` (moves-vs-notes via proxy).
> - **Correction-history chain (UI) — ✅ SHIPPED + verified live 2026-07-02** (`9eac88f`). `CountryWhatChanged` gained an expandable "N earlier corrections" chain (date / risk from→to / cited event) below the latest-move band — the *continuous* self-correction is now visible/auditable. Playwright-verified on prod Japan (5-row chain).
> - Decisions below.
>
