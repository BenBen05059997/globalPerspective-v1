...
**S5.5·T3c (identity pass 3 — axis-flip consolidation) — 🔭 DEFERRED (needs design).** The same real event still fragments across axes (`political#USA#trump` vs `conflict#USA#trump` vs `economic#USA#trump`). Cannot be fixed by the (axis, iso3, entity) key alone: merging by (iso3, entity) across axes would wrongly fuse genuinely different same-actor stories (the $5k-pledge political/economic story SHOULD merge, but the Iran-war conflict story must NOT). Safe consolidation needs content comparison (shared entity SET / headline similarity) — the fuzzy-merge machinery deliberately excluded so far. Highest-value remaining churn-reduction item; own design pass before building (over-merge hides situations, worse than fragmenting).
