...
**Phase 1d — bounded BACKFILL sidecar — ✅ DONE + UPLOADED 2026-09-11.** Sonnet tagged the ~3-day window (204 topics + 890 stories; production-tagged items passed through with `provenance:"production"`, only pre-deploy gaps agent-tagged; conservative rule held — 130 stories left `actors:[]` rather than guessed after the agent caught its own regex fallback emitting junk). Orchestrator review removed 6 bad actor tags pre-upload (Anak-Krakatoa volcano, DRC-as-actor, GDP/HIV/IPO/ISS). Self-consistency on known same-event pairs: iso3 any-overlap 100%, no disagreement rising to false-link risk. Uploaded to **`s3://…/tags/backfill/2026-09-11.json`** (240KB; NOT Worker-exposed — internal matcher data; originals untouched, per §3.5 sidecar rule). Gives the matcher full launch-day coverage instead of forward-only.

**Phase 1 BUILD SPEC — ✅ DRAFTED + REVIEWED 2026-09-11 → `EVENT_REGISTRY_MATCHER_SPEC.md` (this dir).** Matcher runs INSIDE `NewsProjectInvokeAgentLambda`'s existing non-fatal threadId block (has topics+threadIds in memory; tracker keeps zero DDB IAM; failure can't break generation or the sweep); writes `threads/story-map.json` full-replace; tracker stamps threadId with **LATCHING** (`opts.threadId || prev.threadId || null` — orchestrator correction: pure mirroring would flicker links every 4h topic rotation; latched links can't outlive their 90-day-TTL thread pages). IAM deltas specified (agent role needs its first S3 grants; tracker +threads/ read). **Tier 1 (URL-exact) has no pending gate and can ship on operator yes; Tier 2 (R3) behind an `ENABLE_R3_TIER` flag, default off, until the real-production-tag re-measurement (~2026-09-13/14) re-confirms 0 FP.**
