...
| R3 · Accessibility + phone | F1.8, F2.9–F2.17 | ✅ 2026-09-27 · verify 52 files / 437 tests; Playwright: footer clears the bar on 5 pages (tab bar top 786, footer bottom 776), 0 / 40 sheet Tab escapes, radar buttons in the a11y tree. Monitor scoped `.btn-gp` 44px to phones. Old-page controls under 44px left as is (out of scope) |
| R4a · Design | full-bleed console, approved legend, desktop alert stack, dark top bar on /map | ✅ 2026-09-27 · verify 55 files / 470 tests; screenshots (live + fixture) reviewed. **R1 regression found + fixed here:** the globe spin / pulse had silently stopped since R1 (the monitor's R1 check only confirmed the globe rendered, not that it moved) |
| R4b · Design | SITUATIONS / COUNTRY RISK layer switch | **Now** |
